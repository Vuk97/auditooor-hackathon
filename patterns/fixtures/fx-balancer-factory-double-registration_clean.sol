...
// Fixture: fixed — create() only calls _registerPoolWithVault (no duplicate).
...
contract Gyro2CLPPoolFactory is BasePoolFactory {
    // FIXED: removed duplicate _registerPoolWithFactory call
    function create(
        address tokenA,
        address tokenB
    ) external returns (address pool) {
        pool = address(new MockPool(tokenA, tokenB));
        _registerPoolWithVault(pool); // already handles factory registration internally
    }
}
