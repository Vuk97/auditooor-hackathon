...
contract WorkerFrFeeAccountingGuardMissingClean {
    ...
    IWorkerFrProtocolConfigClean public protocolConfig;
    ...
    function burn(address to) external returns (uint256 amount0, uint256 amount1) {
        ...
        IWorkerFrTokenClean(token0).transfer(to, amount0);
        IWorkerFrTokenClean(token1).transfer(to, amount1);
    }

    function swap(uint256 amount0Out, uint256 amount1Out, address to) external {
        uint256 bal0 = IWorkerFrTokenClean(token0).balanceOf(address(this)) - accruedFee;
        uint256 bal1 = IWorkerFrTokenClean(token1).balanceOf(address(this));
        ...
        IWorkerFrTokenClean(token0).transfer(to, amount0Out);
        IWorkerFrTokenClean(token1).transfer(to, amount1Out);
    }
