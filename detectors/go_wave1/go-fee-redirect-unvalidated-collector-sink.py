"""
go-fee-redirect-unvalidated-collector-sink.py

Shared Go fee-redirect class detector.

This detector generalizes the two narrow Go fee-redirect fixtures:

1. A fee, rebate, commission, reward, or accrual amount is transferred to a
   message signer or signer-derived collector.
2. A fee, rebate, commission, or accrual amount is transferred to an explicit
   user-controlled sink such as msg.FeeRecipient.

The detector suppresses paths where the sink is configured by params, module
accounts, or a canonical collector, and paths where the user-derived sink is
compared to a configured collector before the transfer.
...
DETECTOR_ID = "go_wave1.go-fee-redirect-unvalidated-collector-sink"
...
_USER_PARAM_NAME_RE = re.compile(
    ...
    r"|collector|feeCollector|rewardRecipient|rewardAddr)$",
    re.IGNORECASE,
)
...
_CONFIGURED_SINK_RE = re.compile(
    ...
    r"|RewardCollector|RewardModule|GetFeeCollector|GetCollector"
    r"|GetTreasury|GetModuleAddress|FeeCollectorName|ModuleAccount"
    r"|ModuleName|AllowedFeeRecipient|AllowedRewardRecipient"
    r"|AllowlistedSink)",
    re.IGNORECASE,
)
...
def _is_configured_sink(expr: str) -> bool:
    ...
    if _USER_FIELD_RE.search(expr) or _SIGNER_SOURCE_RE.search(expr):
        return False
    return bool(_CONFIGURED_SINK_RE.search(expr))


def _seed_user_terms(fn_text: str, body_text: str) -> set[str]:
    terms = {match.group(0).strip() for match in _SIGNER_SOURCE_RE.finditer(body_text)}
    terms.update(_USER_FIELD_RE.findall(body_text))
...
def _expand_user_aliases(body_text: str, terms: set[str]) -> set[str]:
    ...
    for line in body_text.splitlines():
        ...
        if _is_configured_sink(rhs):
            ...
            continue
        if (
            _SIGNER_SOURCE_RE.search(rhs)
            or _USER_FIELD_RE.search(rhs)
            or _expr_mentions_any(rhs, aliases)
        ):
            aliases.add(lhs)
...
def _has_configured_guard(body_text: str, user_terms: set[str]) -> bool:
    for line in body_text.splitlines():
        if _SINK_CALL_PREFIX_RE.search(line):
            continue
        if not _expr_mentions_any(line, user_terms) and not _SIGNER_SOURCE_RE.search(line):
            continue
        if _NAMED_GUARD_RE.search(line):
            return True
        if _CONFIGURED_SINK_RE.search(line) and _COMPARISON_RE.search(line):
            return True
        if _CONFIGURED_SINK_RE.search(line) and re.search(r"\bif\b", line):
            return True
    return False
...
def _fee_redirect_reason(body_text: str, terms: set[str]) -> str | None:
    for call_text in _call_spans(body_text):
        ...
        if recipients:
            return (
                f"{call_name} routes fee-like value to unvalidated "
                f"user-derived sink `{recipients[0]}`"
            )
    return None
        ...
        hits.append(
            {
                ...
                "message": (
                    f"`{name}` has a shared fee-redirect sink gap: {reason} "
                    f"without checking it against the configured collector, "
                    f"treasury, module account, or canonical recipient. "
