"""
reward-checkpoint-or-failed-dispatch-fire20

Solidity same-class recall detector for rewards-distribution-skew shapes where
reward or checkpoint accounting is asymmetric around dispatch failure, branch
idempotency flags are updated in only one reward arm, or bulk burn reward math
uses a pre-burn supply denominator.
...
_CONTEXT_RE = re.compile(
    r"\b(reward\w*|claim\w*|relayer\w*|dispatch\w*|checkpoint\w*|settle\w*|"
    r"bounty\w*|burn\w*|redeem\w*|withdraw\w*|totalSupply|_totalSupply|"
    r"processed|consumed|paid|settled|claimed|delivered)\b",
    re.IGNORECASE,
)
...
_FLAG_WRITE_RE = re.compile(
    ...
    r"Redeemed|Paid|Settled|Withdrawn|Dispatched|Finalized|Delivered)|claimed|"
    r"processed|consumed|redeemed|paid|settled|withdrawn|dispatched|"
    r"finalized|delivered)\b\s*(?:\[[^\]]+\]\s*)*(?:=|:=)\s*true\b|"
...
_REWARD_SIDE_EFFECT_RE = re.compile(
    ...
    r"bounty|refund|checkpoint\w*\s*\()",
    re.IGNORECASE,
)
_SYMMETRIC_BRANCH_HINT_RE = re.compile(
    r"\b(?:checkpointBothBranches|markAllBranchesProcessed|symmetric|"
...
_CHECKPOINT_CALL_RE = re.compile(
    r"\b(?:checkpoint\w*|_checkpoint\w*|settle\w*|_settle\w*|"
    r"record\w*|_record\w*|mark\w*|_mark\w*)\s*\(",
    re.IGNORECASE,
)
_SUCCESS_CHECKPOINT_RE = re.compile(
    r"\bif\s*\(\s*success\s*\)\s*\{[^{}]*(?:checkpoint\w*|_checkpoint\w*|"
    r"settle\w*|_settle\w*|record\w*|_record\w*|mark\w*|_mark\w*)\s*\(",
    re.IGNORECASE | re.DOTALL,
)
_FAILURE_CHECKPOINT_RE = re.compile(
    r"\b(?:else|catch)\b(?:\s*\([^)]*\))?\s*\{[^{}]*(?:checkpoint\w*|"
    r"_checkpoint\w*|settle\w*|_settle\w*|record\w*|_record\w*|"
    r"mark\w*|_mark\w*)\s*\(",
    re.IGNORECASE | re.DOTALL,
)
_UNCONDITIONAL_AFTER_SUCCESS_RE = re.compile(
    r"\bif\s*\(\s*success\s*\)\s*\{[^{}]*(?:checkpoint\w*|_checkpoint\w*|"
    r"settle\w*|_settle\w*|record\w*|_record\w*|mark\w*|_mark\w*)\s*\("
    r"[^{}]*\}\s*(?:;|\s)*"
    r"(?:checkpoint\w*|_checkpoint\w*|settle\w*|_settle\w*|record\w*|"
    r"_record\w*|mark\w*|_mark\w*)\s*\(",
    re.IGNORECASE | re.DOTALL,
)
...
def _branch_asymmetric_flag(fn: FunctionSlice) -> tuple[str, re.Match[str]] | None:
    ...
    if not flag_blocks or not unflagged_reward_blocks:
        return None
    return (
        "updates reward or idempotency state in only one reward-processing branch",
...
def _failed_dispatch_pays_relayer(fn: FunctionSlice) -> tuple[str, re.Match[str]] | None:
    ...
    if _SUCCESS_GATED_PAYOUT_RE.search(fn.body):
        return None
    return (
        "pays relayer reward or refund after a catch path marks dispatch unsuccessful",
        payout,
    )


def _failed_dispatch_skips_checkpoint(fn: FunctionSlice) -> tuple[str, re.Match[str]] | None:
    if _DISPATCH_FAILURE_RE.search(fn.body) is None:
        return None
    if _CATCH_REVERT_RE.search(fn.body):
        return None
    checkpoint = _SUCCESS_CHECKPOINT_RE.search(fn.body)
    if checkpoint is None:
        return None
    if _FAILURE_CHECKPOINT_RE.search(fn.body):
        return None
    if _UNCONDITIONAL_AFTER_SUCCESS_RE.search(fn.body):
        return None
    if len(_CHECKPOINT_CALL_RE.findall(fn.body)) < 1:
        return None
    return (
        "checkpoints reward or dispatch state only on the successful dispatch branch",
        checkpoint,
    )
...
def _stale_supply_bulk_burn(fn: FunctionSlice) -> tuple[str, re.Match[str]] | None:
    ...
    if not (snapshot.start() < reward_calc.start() < burn.start()):
        return None
    return (
        "calculates bulk burn rewards from a pre-burn totalSupply snapshot",
        snapshot,
    )
...
def _branch_denominator_asymmetry(fn: FunctionSlice) -> tuple[str, re.Match[str]] | None:
    ...
    if len(denominator_blocks) == len(blocks):
        return None
    return (
        "changes burn or supply denominator in only one reward branch",
...
def _first_reason(fn: FunctionSlice) -> tuple[str, re.Match[str]] | None:
    for check in (
        ...
        lambda: _failed_dispatch_skips_checkpoint(fn),
        ...
        findings.append(
            Finding(
                ...
                message=(
                    ...
                    "failed dispatch payouts and keep reward flags, checkpoints, "
                    "and supply denominators symmetric across branches."
                ),
            )
        )
    return findings
