"""
fund-loss-value-route-or-registration-fire20

Flags Solidity fund-loss-via-arithmetic recall shapes where arithmetic value
is routed through a caller supplied sink, a factory registers value-bearing
state twice, or a signed delta is cast to uint128 without a negativity guard.
"""
...
DETECTOR_NAME = "fund-loss-value-route-or-registration-fire20"
...
_VALUE_CONTEXT_RE = re.compile(
    ...
    r"payout|proceeds|claimable|credit|amount|value|share|rate|price|BPS|bps)\b"
)
...
def scan(source: str, file_path: str):
    ...
    The Slither detector below remains the canonical richer path. This source
    scan intentionally encodes the same three Fire20 fixture semantics so the
    held-out recall scoreboard can include this detector.
    """
...
class FundLossValueRouteOrRegistrationFire20(AbstractDetector):
    ARGUMENT = DETECTOR_NAME
    HELP = (
        "Flags caller supplied arithmetic value routes, duplicate value-bearing "
        "factory registration, and unchecked int128-to-uint128 casts."
    )
    IMPACT = DetectorClassification.MEDIUM
    CONFIDENCE = DetectorClassification.LOW
    WIKI = "https://github.com/Vuk97/auditooor"
    WIKI_TITLE = "Fund-loss arithmetic via value route or registration"
    WIKI_DESCRIPTION = (
        "Value-moving arithmetic must bind recipients to configured sinks, "
        "enforce one registration per value-bearing entity, and reject negative "
        "signed deltas before converting them to unsigned amounts."
    )
    WIKI_EXPLOIT_SCENARIO = (
        "A caller directs a computed referral fee to an arbitrary sink, a "
        "factory create path registers the same pool through two registries, "
        "or a negative int128 delta wraps into a huge uint128 credit."
    )
    WIKI_RECOMMENDATION = (
        "Use canonical recipients or allowlists, guard registration uniqueness, "
...
__all__ = [
    "FundLossValueRouteOrRegistrationFire20",
    "DETECTOR_NAME",
