"""
reward-skew-branch-or-failed-dispatch-fire19

Solidity same-class recall detector for rewards-distribution-skew shapes where
reward/idempotency accounting is asymmetric across branches, a relayer reward
is paid after a failed dispatch, or bulk burn reward math uses a stale supply
denominator.
...
_CONTEXT_RE = re.compile(
    r"\b(reward\w*|claim\w*|relayer\w*|dispatch\w*|refund\w*|bounty\w*|"
...
_DISPATCH_FAILURE_RE = re.compile(
    r"\btry\b[\s\S]{0,2200}?\bcatch\b(?:\s*\([^)]*\))?\s*\{[\s\S]{0,500}?"
...
def _failed_dispatch_pays_relayer(fn: FunctionSlice) -> tuple[str, re.Match[str]] | None:
    failed = _DISPATCH_FAILURE_RE.search(fn.body)
    if failed is None:
        return None
