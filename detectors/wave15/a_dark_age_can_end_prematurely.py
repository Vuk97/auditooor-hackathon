"""
a-dark-age-can-end-prematurely — generated from skeleton `name_match_missing_require`
Ported from: Solodit #60862 (Quantstamp/Doomsday: Settlers of the Wasteland)
Severity: HIGH
Pattern: function whose name matches the target regex writes a state var
matching the write regex, but does NOT contain a `require`/`assert` that
references the guard variable regex. Flag the function.

Generated by tools/gen-detector.py — DO NOT EDIT BY HAND.
Spec: detectors/_specs/a-dark-age-can-end-prematurely.yaml
"""

import re
import sys
from pathlib import Path as _Path

sys.path.insert(0, str(_Path(__file__).resolve().parent.parent))
from _template_utils import is_vendored_or_test_contract

from slither.core.cfg.node import NodeType
from slither.detectors.abstract_detector import AbstractDetector, DetectorClassification


class ADarkAgeCanEndPrematurely(AbstractDetector):
    ARGUMENT = "a-dark-age-can-end-prematurely"
    HELP = "A Dark Age Can End Prematurely"
    IMPACT = DetectorClassification.HIGH
    CONFIDENCE = DetectorClassification.MEDIUM
    WIKI = "https://github.com/Vuk97/auditooor/blob/main/reference/bug_patterns_observed.md"
    WIKI_TITLE = "A Dark Age Can End Prematurely"
    WIKI_DESCRIPTION = "**Update**\nMarked as \"Fixed\" by the client. Addressed in: `7736eb06074e1de55cccebfc2cbf572e892c08bc`. The client provided the following explanation: Uses supply at mint of the last minted token rather than current supply to calculate isDarkAge.\n\n**File(s) affected:**`DoomsdaySettlers.sol`\n\n**Descrip"
    WIKI_EXPLOIT_SCENARIO = "Per Solodit #60862: **Update**\nMarked as \"Fixed\" by the client. Addressed in: `7736eb06074e1de55cccebfc2cbf572e892c08bc`. The client provided the following explanation: Uses supply at mint of the last minted token rather"
    WIKI_RECOMMENDATION = "See source audit report for recommended fix."

    _FN_NAME_REGEX = re.compile(r".*(isDarkAge|supply).*", re.IGNORECASE)
    _WRITE_VAR_REGEX = re.compile(r".*(isDarkAge|supply).*", re.IGNORECASE)
    _GUARD_VAR_REGEX = re.compile(r".*(isDarkAge|supply).*", re.IGNORECASE)

    def _detect(self):
        results = []
        for c in self.contracts:
            if is_vendored_or_test_contract(c):
                continue
            for f in c.functions_and_modifiers_declared:
                if not self._FN_NAME_REGEX.search(f.name):
                    continue

                writes_target = False
                for sv in f.state_variables_written:
                    if self._WRITE_VAR_REGEX.search(sv.name):
                        writes_target = True
                        break
                if not writes_target:
                    continue

                has_guard = False
                for node in f.nodes:
                    if node.type not in (NodeType.IF, NodeType.EXPRESSION):
                        continue
                    if not node.contains_require_or_assert():
                        continue
                    expr_text = str(node.expression) if node.expression else ""
                    if self._GUARD_VAR_REGEX.search(expr_text):
                        has_guard = True
                        break

                if has_guard:
                    continue

                info = [
                    f,
                    f" — a-dark-age-can-end-prematurely: writes `.*(isDarkAge|supply).*`-matching state without a `require` on `.*(isDarkAge|supply).*`. ",
                    f"Ported from Solodit #60862 (Quantstamp/Doomsday: Settlers of the Wasteland).",
                ]
                results.append(self.generate_result(info))
        return results
