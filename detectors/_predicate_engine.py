...
_DATAFLOW_ENGINE_CACHE = {}


def _dataflow_engine_for(function):
    """Return a DataFlowEngine bound to the function's Slither compilation unit, or
    None if Slither / the engine module is unavailable. Cached per compilation unit.

    Reuses tools/dataflow-slice.py:DataFlowEngine (wraps
    slither.analyses.data_dependency: is_tainted / get_dependencies). The engine is
    a construction-time fixpoint, so we cache by compilation-unit id."""
    try:
        cu = getattr(function, "compilation_unit", None)
        if cu is None:
        ...
        if cu is None:
            return None
        cu_key = id(cu)
        if cu_key in _DATAFLOW_ENGINE_CACHE:
            return _DATAFLOW_ENGINE_CACHE[cu_key]
        import importlib.util
        import os
        here = os.path.dirname(os.path.abspath(__file__))
        mod_path = os.path.join(here, "..", "tools", "dataflow-slice.py")
        mod_path = os.path.normpath(mod_path)
        if not os.path.exists(mod_path):
            _DATAFLOW_ENGINE_CACHE[cu_key] = None
            return None
        spec = importlib.util.spec_from_file_location("_dataflow_slice_mod", mod_path)
        mod = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(mod)
        # Build an engine over the single CU the function belongs to.
        slither_obj = getattr(cu, "core", None)
        if slither_obj is None:
            # DataFlowEngine iterates .compilation_units; wrap the single CU.
            class _OneCU:
                compilation_units = [cu]
            slither_obj = _OneCU()
        eng = mod.DataFlowEngine(slither_obj)
        # stash the module-level value-moving classifier on the engine for reuse
        eng._is_value_moving_fn = mod._is_value_moving
        _DATAFLOW_ENGINE_CACHE[cu_key] = eng
        return eng
    except Exception:
        return None


def _taints_param_to_semantic(function, tainted_param_names, target_rx, guard_rx):
    """data_dependency-backed answer for function.taints_param_to.

    Returns:
      True  - an UNGUARDED tainted-param flow reaches a value-moving sink whose
              callee matches target_rx (confidence=semantic-ssa).
      False - the IR slice exists but every matching flow is guarded, or no flow
              reaches the target.
      None  - the semantic engine is unavailable -> caller falls back to the
              heuristic name-walk (confidence=heuristic, degraded=True).
    """
    eng = _dataflow_engine_for(function)
    if eng is None:
        return None
    try:
        fn_id = getattr(function, "canonical_name", getattr(function, "name", None))
        found_any_target = False
        # Enumerate value-moving sinks across the whole compilation unit and keep
        # only those whose backward slice ORIGINATES in `function` (inter-procedural).
        for sink_fn, sink_var, sink_ir, sink_node in eng._enumerate_sinks():
            callee = None
            fnobj = getattr(sink_ir, "function", None)
            if fnobj is not None:
                callee = getattr(fnobj, "name", None)
            callee = callee or getattr(sink_ir, "function_name", None)
            if not (callee and target_rx.search(callee)):
                continue
            rec = eng.backward_df_recursive(sink_fn, sink_var, sink_ir, sink_node)
            if rec is None:
                continue
            if (rec.get("source") or {}).get("fn") != fn_id:
                continue
            found_any_target = True
            src_var = (rec.get("source") or {}).get("var") or ""
            matches_param = any(p == src_var for p in tainted_param_names) or \
                any(target_param_in_hops(rec, p) for p in tainted_param_names)
            _closure_correct_rec(eng, rec)  # D-connect mirror (default-off)
            if matches_param and rec.get("unguarded") is True:
                return True
        return found_any_target and False
    except Exception:
        return None


def target_param_in_hops(rec, param_name):
    for h in rec.get("hops", []) or []:
        if h.get("from_var") == param_name or h.get("to_var") == param_name:
            return True
    return False


def _closure_unguarded_enabled():
    """D-connect mirror (P0) default-off switch. When enabled, the predicate
    engine consults the inter-procedural guard closure (the same correction
    tools/dataflow-slice.py applies) before trusting a slice-local
    `unguarded=True`. Off by default so existing detector verdicts are
    byte-identical; opt in via AUDITOOOR_DATAFLOW_CLOSURE_UNGUARDED."""
    import os
    return bool(os.environ.get("AUDITOOOR_DATAFLOW_CLOSURE_UNGUARDED"))


def _closure_correct_rec(eng, rec):
    """Mirror dataflow-slice's D-connect: if `rec` is slice-local unguarded but the
    sink's inter-procedural guard closure shows a guard (up the call graph / in a
    modifier), flip it to guarded. Additive + R80-degrade-safe: any failure leaves
    `rec` untouched. No-op unless _closure_unguarded_enabled()."""
    if not _closure_unguarded_enabled() or eng is None or rec is None:
        return rec
    if rec.get("unguarded") is not True:
        return rec
    try:
        eng.apply_closure_unguarded([rec])
    except Exception:
        pass  # R80: never let the closure layer change the answer on error
    return rec
...
def _check_contract_pred(c, key, val):
    ...
    if key == "contract.has_external_call_to":
        ...
        def _hl_call_fn_name(dest_fn) -> str:
            # Real Slither IR puts a HighLevelCall *operation* in tuple slot [1],
            # whose callee name lives at `.function_name` or `.function.name`
            # (the op has no `.name`). The Wave-2 unit-test mocks instead put a
            # plain object exposing `.name`. Resolve all shapes so the predicate
            # works against both the SimpleNamespace mocks AND live Slither IR.
            if dest_fn is None:
                return ""
            nm = getattr(dest_fn, "name", "") or ""
            if not nm:
                nm = getattr(dest_fn, "function_name", "") or ""
            if not nm:
                inner = getattr(dest_fn, "function", None)
                nm = getattr(inner, "name", "") if inner is not None else ""
            # Slither may hand back a non-str (e.g. a Constant) for unusual
            # callees (selector reads, library refs). Coerce so the regex
            # search below never raises into the broad except (which would
            # silently fail the whole predicate to False).
            ...
            for f in getattr(c, "functions", []) or []:
                for n in getattr(f, "nodes", []) or []:
                    for call in getattr(n, "high_level_calls", []) or []:
                        ...
                        if dest_name and not isinstance(dest_name, str):
                            dest_name = getattr(dest_name, "name", "") or ""
                        fn_name = _hl_call_fn_name(dest_fn)
                        if (not tgt_contract or tgt_contract == dest_name) and fn_name and fn_rx.search(fn_name):
                            return True
                    # Low-level edges (`.call{value:}` / `.send` / `.transfer` /
                    # `.delegatecall`) carry no callee contract name; their
                    # callee selector is `call`/`send`/`transfer`/`delegatecall`.
                    # Only consult them when the target regex is UNqualified
                    # (no `Contract.fn` form) so a qualified high-level target
                    # cannot be spuriously satisfied by an unrelated low-level
                    # send. This keeps the predicate satisfiable for callback-
                    # receiver contracts whose only external edge is low-level.
                    if not tgt_contract:
                        for lc in getattr(n, "low_level_calls", []) or []:
                            ll_str = str(lc)
                            for sel in ("call", "send", "transfer", "delegatecall"):
                                if (("function:%s" % sel) in ll_str or ("LOW_LEVEL" in ll_str and sel in ll_str.lower())) and fn_rx.search(sel):
                                    return True
        except Exception:
            pass
        return False
...
def _check_function_pred(function, key, val):
    ...
    if key == "function.taints_param_to":
        try:
            ...
            # --- Semantic path: Slither data_dependency-backed slice (R75/R76). ---
            # When a Slither context is available, reconstruct the real def-use slice
            # via tools/dataflow-slice.py's DataFlowEngine (wraps is_tainted /
            # get_dependencies over the SSA dependency closure + call-hop crossing).
            # This is exact where the heuristic name-walk is approximate. Falls back
            # gracefully to the substring walk below if the engine is unavailable.
            sem = _taints_param_to_semantic(function, tainted, target_rx, guard_rx)
            if sem is not None:
                # sem == True  -> unguarded tainted flow reaches target (confidence=semantic-ssa)
                # sem == False -> no such unguarded flow per the IR slice
                return sem
            # else: engine unavailable -> degrade to heuristic walk (confidence=heuristic)
    ...
    # data_dependency-backed value-flow predicates (Phase 1 EVM data-flow slice).
    #   function.value_flow_path:        {to: transferFrom}  -> True if ANY tainted
    #       slice (guarded or not) reaches a value-moving sink matching `to`.
    #   function.unguarded_value_flow_to: {to: transferFrom, from: amount}  -> True
    #       only if the recovered slice is UNGUARDED (no require/assert dominating).
    # Both use the semantic engine when available and degrade to the same heuristic
    # walk as taints_param_to so no detector goes silent (confidence=heuristic).
    if key in ("function.value_flow_path", "function.unguarded_value_flow_to"):
        try:
            spec = val if isinstance(val, dict) else ({"to": val} if isinstance(val, str) else {})
            target_rx = re.compile(spec.get("to", ".*"), re.IGNORECASE)
            param_rx = re.compile(spec.get("from", ".*"), re.IGNORECASE)
            guard_rx = re.compile(spec.get("guard", "require|assert"), re.IGNORECASE)
            param_names = {p.name for p in (getattr(function, "parameters", []) or [])
                           if getattr(p, "name", None)}
            tainted = {n for n in param_names if param_rx.search(n)} or param_names
            eng = _dataflow_engine_for(function)
            if eng is not None:
                fn_id = getattr(function, "canonical_name", getattr(function, "name", None))
                any_path = False
                for sink_fn, sink_var, sink_ir, sink_node in eng._enumerate_sinks():
                    ...
                    if (rec.get("source") or {}).get("fn") != fn_id:
                        continue
                    src_var = (rec.get("source") or {}).get("var") or ""
                    if tainted and not (any(p == src_var for p in tainted) or
                                        any(target_param_in_hops(rec, p) for p in tainted)):
                        continue
                    any_path = True
                    if key == "function.value_flow_path":
                        return True
                    _closure_correct_rec(eng, rec)  # D-connect mirror (default-off)
                    if key == "function.unguarded_value_flow_to" and rec.get("unguarded") is True:
                        return True
                # semantic engine ran: definitive answer
                if key == "function.value_flow_path":
                    return any_path
                return False
            # engine unavailable -> degrade to the heuristic taints_param_to walk
            return _check_function_pred(
                function, "function.taints_param_to",
                {"from": spec.get("from", ".*"), "to": spec.get("to", ".*"),
                 "guard": spec.get("guard", "require|assert"), "depth": spec.get("depth", 3)},
            )
        except Exception:
            return False
