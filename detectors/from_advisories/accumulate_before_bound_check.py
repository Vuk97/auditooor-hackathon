...
# CLASS-LEVEL detector (mechanism: accumulate_before_bound_check); anti-overfit: token-class anchors.
"""accumulate_before_bound_check - A money/balance accumulator adds credits before subtracting debits (or before a MAX bound) without checked arithmetic - the intermediate may transiently overflow a bounded type.

Class tag: ordering-arithmetic-overflow   Language: rust   Severity: MEDIUM
Source advisories: https://github.com/ZcashFoundation/zebra/security/advisories/GHSA-w834-cf6p-9m9w
...
DETECTOR_ID = 'accumulate_before_bound_check'
CLASS_TAG = 'ordering-arithmetic-overflow'
LANGUAGE = 'rust'
SEVERITY = 'MEDIUM'

_SKIP_DIRS = {"target", ".git", "node_modules", "vendor", "_archive", "tests", "test"}
_POSITIVE = re.compile('\\b[\\w.]*(?:balance|total|sum|acc|amount)\\b[^;]*\\+=[^;]*;')
_NEGATIVE = re.compile('(?:checked_add|saturating_add|\\.checked\\(|overflowing_add|ensure!\\s*\\([^)]*MAX|<=\\s*[\\w:.]*MAX)') if '(?:checked_add|saturating_add|\\.checked\\(|overflowing_add|ensure!\\s*\\([^)]*MAX|<=\\s*[\\w:.]*MAX)' else None
