...
# CLASS-LEVEL detector (mechanism: fork_replace_pinned_at_sha); anti-overfit: token-class anchors.
"""fork_replace_pinned_at_sha - go.mod pins a forked upstream at a frozen pseudo-version (date-SHA). If the fork HEAD == audit-pin, upstream security fixes shipped after the pin are not backported - fork-divergence. Triage: diff fork vs upstream since merge-base for fix/panic/consensus/cve/dos/overflow/nil/race commits absent from the fork.

Class tag: fork-divergence-missing-upstream-fix   Language: go   Severity: HIGH
Source advisories: cometbft/cometbft #5757/#5753/#5711/#5629/#5718 (silently-shipped blocksync hardening, no GHSA) | fork-pin 904204b11c9e ; cosmos/ibc-go 120/125 commits behind v8.5.x/v8.6.x; 5 missing patches touch wi
...
DETECTOR_ID = 'fork_replace_pinned_at_sha'
CLASS_TAG = 'fork-divergence-missing-upstream-fix'
LANGUAGE = 'go'
SEVERITY = 'HIGH'

_SKIP_DIRS = {"target", ".git", "node_modules", "vendor", "_archive", "tests", "test"}
_POSITIVE = re.compile('replace\\s+[\\w./-]+\\s*=>\\s*github\\.com/[\\w-]+/[\\w.-]+\\s+v[\\w.+-]*?[0-9a-f]{12}\\b')
