...
# CLASS-LEVEL detector (mechanism: state_mutation_before_guard); anti-overfit: token-class anchors.
"""state_mutation_before_guard - State index/map is mutated BEFORE the validity/rejection guard for the same untrusted content runs in the same function - a rejected item leaves a poisoned index entry (TOCTOU / ordering).

Class tag: ordering-validation-toctou   Language: rust   Severity: MEDIUM
Source advisories: https://github.com/ZcashFoundation/zebra/security/advisories/GHSA-hhm7-qrv5-h4r6
...
DETECTOR_ID = 'state_mutation_before_guard'
CLASS_TAG = 'ordering-validation-toctou'
LANGUAGE = 'rust'
SEVERITY = 'MEDIUM'

_SKIP_DIRS = {"target", ".git", "node_modules", "vendor", "_archive", "tests", "test"}
_POSITIVE = re.compile('(?s)\\.(?:insert|push)\\s*\\([^;]*\\)\\s*;[\\s\\S]{0,200}?(?:check_|validate_|ensure_|reject|duplicate)\\w*\\s*\\(')
