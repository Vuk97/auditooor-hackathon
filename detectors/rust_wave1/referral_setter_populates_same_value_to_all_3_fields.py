"""
referral_setter_populates_same_value_to_all_3_fields

Auto-generated from r94 phase7 overnight pipeline.
Pattern class: referral-setter-populates-same-value-to-all-3-fields
Platform: solana
Source: phase7_rust_fixture_referral_setter_populates_same_value_to_all_3_fields.json
...
_INDICATOR_PATTERNS = ['\\bupdate_referrer_info\\b', 'ReferralInfo\\s*\\{[^}]*:\\s*(\\w+)[^}]*:\\s*\\1[^}]*:\\s*\\1', 'insert\\(\\s*(\\w+)\\s*,\\s*ReferralInfo\\s*\\{[^}]*\\}\\s*\\)']
...
def run(tree, source: bytes, filepath: str):  # noqa: ARG001
    ...
    hits.append({
        ...
        "message": (
            f"{filepath}: pattern 'referral_setter_populates_same_value_to_all_3_fields' detected "
