"""
liquidator_rounds_up_collateral_seize_rounds_down_debt_repay

Auto-generated from r94 phase7 overnight pipeline.
Pattern class: liquidator-rounds-up-collateral-seize-rounds-down-debt-repay
Platform: solana
Source: phase7_rust_fixture_liquidator_rounds_up_collateral_seize_rounds_down_debt_repay.json
...
_INDICATOR_PATTERNS = ['\\+\\s*\\w+\\s*\\*\\s*\\d+\\s*-\\s*1\\s*\\)\\s*\\/\\s*\\w+', 'ceiling_div', '\\.div\\([^)]*\\).*\\.div\\(', 'liquidat.*collateral.*\\+.*-.*1.*\\/', 'round.*up.*collateral|ceil.*collateral']
...
def run(tree, source: bytes, filepath: str):  # noqa: ARG001
    ...
    hits.append({
        ...
        "message": (
            f"{filepath}: pattern 'liquidator_rounds_up_collateral_seize_rounds_down_debt_repay' detected "
