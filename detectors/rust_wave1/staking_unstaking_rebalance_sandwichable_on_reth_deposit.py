"""
staking_unstaking_rebalance_sandwichable_on_reth_deposit

Auto-generated from r94 phase7 overnight pipeline.
Pattern class: staking-unstaking-rebalance-sandwichable-on-reth-deposit
Platform: solana
Source: phase7_rust_fixture_staking_unstaking_rebalance_sandwichable_on_reth_deposit.json
...
_INDICATOR_PATTERNS = ['fn\\s+(stake|unstake|rebalance)[^}]*oracle\\.get_rate\\s*\\([^)]*\\)(?!\\s*[,;\\s]*(?:assert|min_out|slippage))', '\\.get_rate\\s*\\([^)]*\\)\\s*[,;\\s]*(?:let|mut)?\\s*\\w+\\s*[,;\\s]*[^a-zA-Z]*(?:balances|balance)']
...
def run(tree, source: bytes, filepath: str):  # noqa: ARG001
    ...
    hits.append({
        ...
        "message": (
            f"{filepath}: pattern 'staking_unstaking_rebalance_sandwichable_on_reth_deposit' detected "
