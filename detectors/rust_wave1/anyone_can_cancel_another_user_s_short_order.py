"""
anyone_can_cancel_another_user_s_short_order

Auto-generated from r94 phase7 overnight pipeline.
Pattern class: anyone-can-cancel-another-user-s-short-order
Platform: solana
Source: phase7_rust_fixture_anyone_can_cancel_another_user_s_short_order.json
...
_INDICATOR_PATTERNS = ['fn.*cancel.*order.*\\(.*order_id', 'get_mut\\(&order_id\\)', 'transfer.*short.*record.*cancel', 'cancelOrder\\(.*shortOrderId']
...
def run(tree, source: bytes, filepath: str):  # noqa: ARG001
    ...
    if re.search(r"fn\s+cancel_order\b[\s\S]*?order\.owner\s*!=\s*caller", code):
        return hits
    ...
    hits.append({
        ...
        "message": (
            f"{filepath}: pattern 'anyone_can_cancel_another_user_s_short_order' detected "
