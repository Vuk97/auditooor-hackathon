"""
multiple_vote_checkpoints_per_block_lead_to_incorrect_accounting

Auto-generated from r94 phase7 overnight pipeline.
Pattern class: multiple-vote-checkpoints-per-block-lead-to-incorrect-accounting
Platform: solana
Source: phase7_rust_fixture_multiple_vote_checkpoints_per_block_lead_to_incorrect_accounting.json
...
_INDICATOR_PATTERNS = ['fn\\s+(write_checkpoint|_writeCheckpoint|writeCheckpoint)', '\\.push\\s*\\(\\s*Checkpoint\\s*\\{', 'checkpoints\\.push\\s*\\(']
...
def run(tree, source: bytes, filepath: str):  # noqa: ARG001
    ...
    if re.search(r"last\.timestamp\s*==\s*timestamp[\s\S]*?last\.votes\s*=\s*votes[\s\S]*?return", code):
        return hits
    ...
    hits.append({
        ...
        "message": (
            f"{filepath}: pattern 'multiple_vote_checkpoints_per_block_lead_to_incorrect_accounting' detected "
