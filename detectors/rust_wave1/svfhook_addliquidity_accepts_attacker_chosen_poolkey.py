"""
svfhook_addliquidity_accepts_attacker_chosen_poolkey

Auto-generated from r94 phase7 overnight pipeline.
Pattern class: svfhook-addliquidity-accepts-attacker-chosen-poolkey
Platform: solana
Source: phase7_rust_fixture_svfhook_addliquidity_accepts_attacker_chosen_poolkey.json
...
_INDICATOR_PATTERNS = ['fn\\s+(add_liquidity|addLiquidity)\\s*\\([^)]*pool_key\\s*:\\s*&\\s*PoolKey', 'PoolKey\\s*\\{[^}]*\\}', 'add_liquidity|addLiquidity']
...
def run(tree, source: bytes, filepath: str):  # noqa: ARG001
    ...
    if re.search(r"if\s+pool_key\s*!=\s*&self\.canonical_pool_key\s*\{[\s\S]*?return\s+Err", code):
        return hits
    ...
    hits.append({
        ...
        "message": (
            f"{filepath}: pattern 'svfhook_addliquidity_accepts_attacker_chosen_poolkey' detected "
