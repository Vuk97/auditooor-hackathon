"""
jit_liquidity_penalty_bypassed_via_multi_position_split

Auto-generated from r94 phase7 overnight pipeline.
Pattern class: jit-liquidity-penalty-bypassed-via-multi-position-split
Platform: solana
Source: phase7_rust_fixture_jit_liquidity_penalty_bypassed_via_multi_position_split.json
...
_INDICATOR_PATTERNS = ['HashMap.*salt', 'jit_records.*insert.*salt', 'penalty.*salt.*tuple', 'owner.*salt.*HashMap']
...
def run(tree, source: bytes, filepath: str):  # noqa: ARG001
    ...
    if match_count < _MIN_MATCH and re.search(
        r"penalty_key\s*=\s*\(\s*key\.owner\s*,\s*key\.salt\s*\)"
        r"[\s\S]*?pool_entry\.get\s*\(\s*&penalty_key\s*\)"
        r"[\s\S]*?pool_entry\.insert\s*\(\s*penalty_key\s*,",
        code,
        re.IGNORECASE,
    ):
    ...
    hits.append({
        ...
        "message": (
            f"{filepath}: pattern 'jit_liquidity_penalty_bypassed_via_multi_position_split' detected "
