"""
remote_transfer_wrong_parameter_steals_usdo

Auto-generated from r94 phase7 overnight pipeline.
Pattern class: remote-transfer-wrong-parameter-steals-usdo
Platform: solana
Source: phase7_rust_fixture_remote_transfer_wrong_parameter_steals_usdo.json
...
_INDICATOR_PATTERNS = ['fn\\s+\\w*remote\\w*transfer\\w*\\s*\\([^)]*from\\s*:\\s*\\[u8;\\s*32\\][^)]*payload\\s*:\\s*\\w*Payload', 'let\\s+from\\s*=\\s*from_\\w+', 'fn\\s+\\w*transfer\\w*\\([^)]*from_\\w+\\s*:\\s*\\[u8;\\s*32\\][^)]*Payload']
...
def run(tree, source: bytes, filepath: str):  # noqa: ARG001
    ...
    hits.append({
        ...
        "message": (
            f"{filepath}: pattern 'remote_transfer_wrong_parameter_steals_usdo' detected "
