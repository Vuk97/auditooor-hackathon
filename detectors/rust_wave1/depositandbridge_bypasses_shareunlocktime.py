"""
depositandbridge_bypasses_shareunlocktime

Auto-generated from r94 phase7 overnight pipeline.
Pattern class: depositandbridge-bypasses-shareunlocktime
Platform: solana
Source: phase7_rust_fixture_depositandbridge_bypasses_shareunlocktime.json
...
_INDICATOR_PATTERNS = ['fn\\s+(deposit_and_bridge|depositAndBridge)', 'mint\\s*\\([^)]*\\)\\s*;\\s*\\n?\\s*[^}]*bridge\\.(send|bridge)', 'mint\\s*\\([^)]*\\)\\s*;\\s*\\n?\\s*[^}]*\\.send\\s*\\(']
...
def run(tree, source: bytes, filepath: str):  # noqa: ARG001
    ...
    hits.append({
        ...
        "message": (
            f"{filepath}: pattern 'depositandbridge_bypasses_shareunlocktime' detected "
