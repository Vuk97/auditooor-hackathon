...
impl VeToken {
    ...
    pub fn get_total_voting_power(&self, current_time: u64) -> u64 {
        let mut total = 0u64;
        for &account in self.locked_balances.keys() {
            total = total.saturating_add(self.get_voting_power_for_account(account, current_time));
        }
        total
    }
...
fn main() {
    ...
    assert!(voting_power <= supply, "voting power should not exceed weighted sum");
    println!("voting_power={} supply={}", voting_power, supply);
}