...
#[contractimpl]
impl Bad {
    // VULN: seizes collateral FROM the liquidator instead of the borrower.
    pub fn liquidate(env: Env, liquidator: Address, borrower: Address, collateral: Address, amount: i128) {
        ...
        c_token.transfer_from(liquidator.clone(), liquidator.clone(), env.current_contract_address(), amount);
    }
}
