...
/// Simulated UniV3 swap parameters
#[derive(Clone, Debug)]
...
/// Vulnerable fee harvester with zero slippage protection
...
impl UniV3Utils {
    pub fn swap(params: ExactInputSingleParams) -> u128 {
        // Simulated swap execution - no validation of amount_out_minimum
        params.amount_in
    }
}
...
impl<T> StrategyPassiveManagerUniswap<T> {
    ...
    /// VULNERABLE: Charge fees with amountOutMinimum=0 (no slippage protection)
    /// This allows MEV sandwich attacks on every harvest cycle
    pub fn _charge_fees(&self, fee0: u128, fee1: u128) -> (u128, u128) {
        let swap_params = ExactInputSingleParams {
            ...
            deadline: u64::MAX,
            amount_in: fee0,
            amount_out_minimum: 0, // CRITICAL: No slippage protection
