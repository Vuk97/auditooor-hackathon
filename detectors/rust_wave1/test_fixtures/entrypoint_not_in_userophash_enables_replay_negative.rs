...
/// EntryPoint address included in hash to prevent replay across deployments
...
impl UserOpHasher {
    ...
    /// Compute userOpHash including entryPoint address (EIP-4337 compliant)
    pub fn hash_user_op(&self, user_op: &UserOperation) -> B256 {
        ...
        // maxPriorityFeePerGas, hash(paymasterAndData), chainId, entryPoint
        ...
        // CRITICAL: Include both chainId AND entryPoint in the hash
        hasher.update(&self.chain_id.to_be_bytes());
        hasher.update(self.entry_point.as_slice());
        
        hasher.finalize()
    }

    /// Verify signature was signed over correct domain
...
mod tests {
    ...
    #[test]
    fn test_hash_includes_entrypoint() {
        ...
        // Different entryPoints MUST produce different hashes (same chain)
        assert_ne!(hash1, hash2, "EntryPoint must affect hash");
    }
}