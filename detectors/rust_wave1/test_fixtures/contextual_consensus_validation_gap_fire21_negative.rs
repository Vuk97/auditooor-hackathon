...
fn validate_anchors_against_parent_chain_and_finalized_state(
    finalized_state: &ZebraDb,
    parent_chain: Option<&Arc<Chain>>,
    transaction: &Transaction,
) -> Result<(), ValidateContextError> {
    for anchor in transaction.sapling_anchors() {
        let found_in_parent = parent_chain
            .map(|chain| chain.sapling_anchors.contains(&anchor))
            .unwrap_or(false);

        if !found_in_parent && !finalized_state.contains_sapling_anchor(&anchor) {
...
fn accept_parent_chain_treestate_with_root_binding(
    ...
    for joinsplit in transaction.sprout_groth16_joinsplits() {
        ...
        if let Some(input_tree) = input_tree {
            assert_eq!(input_tree.root(), joinsplit.anchor);
