...
impl ConvictionScore {
    ...
    pub fn update_conviction_score(&mut self, user: u64, current_block: u64) {
        // BUG: Mutate state BEFORE reading for delta computation
        self.is_governance.insert(user, false);
        
        // STALE READ: uses pre-mutation score but AFTER governance flag was cleared
        let prior_score = self.get_prior_conviction_score(user, current_block.saturating_sub(1));
        
        // Delta computed with stale score — is_governance is now false,
        // but prior_score reflects governance contribution that should have been counted
        let delta = prior_score;
        
        // Drift: total_conviction not properly adjusted because delta uses stale view
