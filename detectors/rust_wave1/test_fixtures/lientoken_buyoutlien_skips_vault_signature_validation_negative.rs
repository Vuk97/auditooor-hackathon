...
impl<'a> LienToken<'a> {
    ...
    /// CLEAN: Always validates vault signature before buyout
    pub fn buyout_lien(
        &mut self,
        action: LienActionBuyout,
        signature: Signature,
    ) -> Result<(), String> {
        // Validate vault signature on every path
        self.vault_impl._validate_commitment(&action, &signature)?;
