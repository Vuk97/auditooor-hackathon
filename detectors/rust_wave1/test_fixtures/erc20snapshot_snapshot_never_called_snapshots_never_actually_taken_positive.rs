...
fn main() {
    ...
    token.mint(1, 100);
    token.mint(2, 200);
    // _snapshot is never called - snapshots are never taken
    // balance_of_at will always return None for any snapshot_id
    assert_eq!(token.balance_of_at(1, 1), None);
}