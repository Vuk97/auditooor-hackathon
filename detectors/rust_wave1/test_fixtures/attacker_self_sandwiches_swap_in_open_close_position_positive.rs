...
fn main() {
    ...
    let attacker_controlled_swap = SwapParams {
        amount_in: 1000,
        min_amount_out: 0,
        max_slippage_bps: 10000,
    };

    let pos_id = vault.open_position(5000, 3000, attacker_controlled_swap.clone());
    vault.close_position(pos_id, attacker_controlled_swap);
}