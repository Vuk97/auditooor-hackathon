...
impl Marketplace {
    ...
    fn is_listing_valid(&self, listing: &Listing) -> bool {
        let seller_balance = self.get_balance(listing.token_id, listing.seller);
        seller_balance >= listing.amount
    }

    fn get_total_amount_for_token(&self, token_id: u64) -> u64 {
        let mut total = 0u64;
    ...
    fn validate_all_listings_for_token(&self, token_id: u64) -> Result<(), String> {
        for (listing_id, listings) in &self.listings {
            for listing in listings {
                if listing.token_id == token_id {
                    if !self.is_listing_valid(listing) {
                        return Err(format!(
                            "Listing {} invalid: seller lacks balance",
                            listing_id
                        ));
                    }
                }
    ...
    fn execute_listing(&self, listing_id: u64, listing_index: usize) -> Result<u64, String> {
...
fn main() {
    ...
    marketplace.set_balance(1, seller, 100);
    ...
    let result = marketplace.execute_listing(1, 0);
    assert!(result.is_ok());
    
    let valid = marketplace.validate_all_listings_for_token(1);
    assert!(valid.is_ok());
}