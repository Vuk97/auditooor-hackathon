...
Pattern class: convictionscore-updateconvictionscore-uses-stale-score-for-delta
Platform: solana
Source: phase7_rust_fixture_convictionscore_updateconvictionscore_uses_stale_score_for_delta.json
...
_INDICATOR_PATTERNS = [r'is_governance\s*\.\s*insert\s*\([^)]*false[^)]*\)[\s\S]*get_prior\w*_score']
...
def run(tree, source: bytes, filepath: str):  # noqa: ARG001
    ...
    hits.append({
        ...
        "message": (
            f"{filepath}: pattern 'convictionscore_updateconvictionscore_uses_stale_score_for_delta' detected "
