...
settle_reward BEFORE the mutation. A settlement call after the mutation
is still unsafe because accrued rewards since the last update get
retroactively multiplied at the new boost.
...
_SETTLES_REWARDS_RE = re.compile(
...
def run(tree, source: bytes, filepath: str):
    ...
    for fn, _impl in functions_in_contractimpl(tree.root_node, source):
        ...
        mutation = _first_match(_MUTATES_BOOST_RE, body_nc)
        if mutation is None:
            continue
        settlement = _first_match(_SETTLES_REWARDS_RE, body_nc)
