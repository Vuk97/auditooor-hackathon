...
contract AaveFlashloanPremiumOnDebtBearingLegClean {
    ...
    function executeFlashLoan(
        ...
        for (uint256 i = 0; i < amounts.length; ++i) {
            ...
            uint256 mode = interestRateModes[i];

            totalPremiums[i] = mode == 0 ? amount.percentMul(flashloanPremiumTotal) : 0;

            if (mode == uint256(InterestRateMode.NONE)) {
