...
contract GovValidatorVotingPowerClean {
    ...
    mapping(address => bool) internal validatorSuspended;
    ...
    function vote(uint256 proposalId) external returns (bool) {
        _refreshValidatorVotePower(msg.sender);
    ...
    function _refreshValidatorVotePower(address validator) internal {
        if (validatorSuspended[validator]) {
            votePower[validator] = 0;
        }
    }
}
