...
contract VotePowerForwardedBalancePlusDelegateDoubleCountPositive {
    struct Period {
    ...
    mapping(uint256 => uint256) public delegatedPower;
    ...
    function carryVoteForward(uint256 _tokenId, uint256 _fromPeriod) public {
        ...
        for (uint256 i; i < _poolList.length; i++) {
            ...
            uint256 carriedWeight = ps.tokenIdVotes[_gauge][_tokenId];
            _weightList[i] = carriedWeight + delegatedPower[_tokenId];
        }
