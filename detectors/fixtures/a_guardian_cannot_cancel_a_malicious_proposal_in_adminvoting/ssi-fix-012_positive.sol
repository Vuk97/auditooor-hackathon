...
contract AdminVotingVulnerable {
    struct Action {
        address target;
        bytes data;
    }
    ...
    function cancelProposal(uint256 id) external {
        ...
        require(!_isSetGuardianPayload(payload[0]), "Guardian replacement not cancellable");
    ...
    function _isSetGuardianPayload(Action memory action) internal view returns (bool) {
        if (action.target == address(prismaCore)) {
