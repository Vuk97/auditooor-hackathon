...
contract TokenizedBondSharesClean {
    ...
    function _update(address from, address to, uint256 amount) internal {
        _moveValidatorBondShares(from, to, amount);
        _balances[from] -= amount;
        _balances[to] += amount;
    }

    function _moveValidatorBondShares(address from, address to, uint256 amount) internal {
        uint256 validatorId = primaryValidator[from];
        validatorBondShares[validatorId][from] -= amount;
        validatorBondShares[validatorId][to] += amount;
        if (primaryValidator[to] == 0) {
            primaryValidator[to] = validatorId;
        }
    }
