#!/usr/bin/env bash
# auditooor-kimi-wrapper.sh — MCP-gated Moonshot Kimi CLI wrapper.
# Lane 7 of MCP harness review (PR #658).
# Wave-6 E-2: freshness gate added (AUDITOOOR_RECALL_MAX_AGE_S, default 1800 s).
#
# Gates: query, chat, run (POST-to-API / write class)
# Pass-through: --help, --version, models list (read-only)
#
# Token resolution:
...
if [ -z "$REAL_KIMI" ] || [ ! -x "$REAL_KIMI" ]; then
  echo "[auditooor-kimi-wrapper.sh] ERROR: real kimi binary not found. Set AUDITOOOR_REAL_KIMI=/path/to/kimi." >&2
  exit 127
fi
...
# Gated subcommands: anything that POSTs to the Kimi API
GATED_SUBCMDS_RE='^(query|chat|run)$'
...
# Detect "models list" 2-arg read-only pair; pass through immediately
ARG_BUF=()
for arg in "$@"; do
...
if [ "$GATE_PASS" -eq 0 ]; then
  ...
  if [ "$BYPASS" = "0" ]; then
    ...
    cat >&2 <<EOF
[mcp-gate:$WRAPPER_NAME] BLOCKED: 'kimi $SUB' requires a valid MCP session token (scope=$REQUIRE_SCOPE)
   ...
   Or override (audit-logged):
     AUDITOOOR_MCP_REQUIRED=0 kimi $SUB ...
EOF
    exit 1
  fi
fi

exec "$REAL_KIMI" "${FILTERED[@]}"
