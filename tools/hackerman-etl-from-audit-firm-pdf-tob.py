#!/usr/bin/env python3
"""Wave-2 W2.4 Phase-1 deep-mine ETL (Trail-of-Bits subset).

Walks the Wave-1 listing tree, filters to the
``trailofbits-publications`` firm subset (422 PDFs), fetches the PDF blob
into a local gitignored cache, and emits one
``auditooor.hackerman_record.v1``-style per-finding record per
Trail-of-Bits finding to::

    audit/corpus_tags/tags/audit_firm_findings_tob/<firm>__<project>__<finding_id>/record.{json,yaml}

Spec ref: ``docs/WAVE2_W24_PDF_DEEPMINE_SPEC_2026-05-16.md`` (Phase 1,
ToB-only subset per W2.4 task brief).

Real-source-only / M14-trap discipline:

* PDFs are fetched via ``urllib.request`` to the canonical
  ``raw.githubusercontent.com`` URL Wave-1 already validated; no scraping,
  no firm-website crawling.
...
CLI::
    ...
    python3 tools/hackerman-etl-from-audit-firm-pdf-tob.py \\
        ...
        --out-dir audit/corpus_tags/tags/audit_firm_findings_tob \\
...
FIRM_PREFIX = "trailofbits-publications"
PARSER_FIRM_VARIANT = "trailofbits"
...
def parse_listing(record_path: Path) -> Optional[ListingHandle]:
    ...
    if not pdf_url:
        return None
    if not project_label:
        # Fall back to slug from filename stem.
...
def iter_tob_listings(listings_dir: Path) -> Iterable[ListingHandle]:
...
def _attack_class_from_title(title: str) -> str:
    ...
    This is a heuristic; downstream W2.2 detector auto-gen re-derives
    structured attack-class tags from the body. We pick a coarse class
    so the record is searchable.
    """
    lowered = title.lower()
    for needle, klass in (
...
def build_finding_record(
    handle: ListingHandle,
    finding: pdf_finding_extractor.ToBFinding,
    blob_sha256: str,
    *,
    parser_version: str,
) -> Dict[str, Any]:
    finding_id = f"{finding.finding_index:03d}-{pdf_finding_extractor.slugify_title(finding.title)}"
    ...
    record: Dict[str, Any] = {
        ...
        "attacker_action_sequence": (
            ...
            f"Audit-firm finding extracted from Trail-of-Bits PDF '{handle.filename}'."
        ),
        ...
        "required_preconditions": [
            ...
            f"verification_tier={VERIFICATION_TIER}",
            f"Trail-of-Bits finding #{finding.finding_index}",
        ],
...
def write_record(
    ...
    *,
    ...
    (rec_dir / "record.json").write_text(json.dumps(record, indent=2, sort_keys=True), encoding="utf-8")
    (rec_dir / "record.yaml").write_text(yaml.safe_dump(record, sort_keys=True, default_flow_style=False), encoding="utf-8")
    return rec_dir
...
def process_listing(
    ...
    *,
    ...
    findings = pdf_finding_extractor.extract_trailofbits_findings(extraction)
    if not findings:
        return {
            "listing_record_id": handle.record_id,
            "pdf_url": handle.pdf_url,
...
def run(args: argparse.Namespace) -> int:
    ...
    handles = list(iter_tob_listings(listings_dir))
    ...
    if args.json_summary:
        ...
        sys.stdout.write(
            f"[w24-tob] listings_seen={summary['listings_seen']} "
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument(
        "--out-dir",
        default="audit/corpus_tags/tags/audit_firm_findings_tob",
