#!/usr/bin/env python3
"""Wave-2 W2.4 Zellic-firm deep-mine ETL.

Walks the Wave-1 listing tree, filters to the ``zellic-publications`` firm
subset (375 PDFs), fetches the PDF blob into a local gitignored cache, and
emits one ``auditooor.hackerman_record.v1``-style per-finding record per
Zellic finding to::

    audit/corpus_tags/tags/audit_firm_findings_zellic/<firm>__<project>__<finding_id>/record.{json,yaml}

Spec ref: ``docs/WAVE2_W24_PDF_DEEPMINE_SPEC_2026-05-16.md`` §5.2 (Zellic
variant). Mirror of ``hackerman-etl-from-audit-firm-pdf-tob.py`` with the
firm prefix, parser variant, and output tree swapped.
...
* No edits to existing tools. W2.4 Zellic is purely additive.
...
CLI::
    ...
    python3 tools/hackerman-etl-from-audit-firm-pdf-zellic.py \\
        ...
        --out-dir audit/corpus_tags/tags/audit_firm_findings_zellic \\
...
FIRM_PREFIX = "zellic-publications"
PARSER_FIRM_VARIANT = "zellic"
...
def iter_zellic_listings(listings_dir: Path) -> Iterable[ListingHandle]:
...
def _attack_class_from_title(title: str) -> str:
    ...
    attack-class tags from the body. We pick a coarse class so the record
    is searchable.
    """
    lowered = title.lower()
    for needle, klass in (
...
def build_finding_record(
    handle: ListingHandle,
    finding: "pdf_finding_extractor.ZellicFinding",
    blob_sha256: str,
    *,
    parser_version: str,
) -> Dict[str, Any]:
    ...
    record: Dict[str, Any] = {
        ...
        "attacker_action_sequence": (
            ...
            f"Audit-firm finding extracted from Zellic PDF '{handle.filename}'."
        ),
        ...
        "required_preconditions": [
            ...
            f"verification_tier={VERIFICATION_TIER}",
            f"Zellic finding #{finding.finding_index}",
        ],
...
def process_listing(
    ...
    *,
    ...
    findings = pdf_finding_extractor.extract_zellic_findings(extraction)
    if not findings:
        return {
            "listing_record_id": handle.record_id,
            "pdf_url": handle.pdf_url,
...
def run(args: argparse.Namespace) -> int:
    ...
    handles = list(iter_zellic_listings(listings_dir))
    ...
    if args.json_summary:
        ...
        sys.stdout.write(
            f"[w24-zellic] listings_seen={summary['listings_seen']} "
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument(
        "--out-dir",
        default="audit/corpus_tags/tags/audit_firm_findings_zellic",
