#!/usr/bin/env python3
"""Wave-2 W2.4 ChainSecurity-firm PDF deep-mine ETL.

Walks the Wave-1 listing tree, filters to the ``chainsecurity-audits``
firm subset (open-source ChainSecurity audit-reports), fetches the PDF
blob into a local gitignored cache, and emits one
``auditooor.hackerman_record.v1``-style per-finding record per
ChainSecurity finding to::

    audit/corpus_tags/tags/firm-chainsecurity-audits/<sanitized-report-slug>/<finding-id>.yaml
...
``hackerman-etl-from-audit-firm-pdf-spearbit.py``. ChainSecurity reports
use a bracketed sequential finding-ID layout (``[CS-1]``, ``[CS-2]``,
...) with a separate severity label. Findings carry Description /
Acceptance Criteria (unique to ChainSecurity) / Recommendation /
Acknowledgement subsections, with verbatim resolution wording such as
``Code Corrected`` / ``Acknowledged`` / ``Risk Accepted``.

See ``tools/lib/pdf_finding_extractor.py`` ``extract_chainsecurity_findings``
for the parser implementation.

Spec ref: ``docs/WAVE2_W24_PDF_DEEPMINE_SPEC_2026-05-16.md``
(ChainSecurity section, PR-Wave2-B task brief).
...
* PDFs fetched via ``urllib.request`` to the canonical
  ...
  scraping, no firm-website crawling. The URL path is percent-encoded
...
* No edits to existing tools. W2.4 ChainSecurity is purely additive.
...
CLI::
    ...
    python3 tools/hackerman-etl-from-audit-firm-pdf-chainsecurity.py \\
        --source tools/tests/fixtures/audit_firm_pdf_samples/chainsecurity \\
        --out /tmp/chainsec_test_emit --dry-run --json-summary
    ...
    python3 tools/hackerman-etl-from-audit-firm-pdf-chainsecurity.py \\
        ...
        --out-dir audit/corpus_tags/tags/firm-chainsecurity-audits \\
...
FIRM_PREFIX = "chainsecurity-audits"
PARSER_FIRM_VARIANT = "chainsecurity"
...
def _percent_encode_path(url: str) -> str:
    ...
    ChainSecurity filenames can carry spaces / parentheses / commas.
...
def iter_chainsecurity_listings(listings_dir: Path) -> Iterable[ListingHandle]:
...
def iter_source_dir_listings(source_dir: Path) -> Iterable[ListingHandle]:
    ...
    for child in sorted(source_dir.iterdir()):
        if child.is_file() and child.suffix.lower() == ".pdf":
            ...
            yield ListingHandle(
                record_id=f"source-mode:chainsecurity:{child.stem}",
...
def build_finding_record(
    handle: ListingHandle,
    finding: "pdf_finding_extractor.ChainSecurityFinding",
    blob_sha256: str,
    *,
    parser_version: str,
    ...
    record: Dict[str, Any] = {
        ...
        "attacker_action_sequence": (
            ...
            f"Audit-firm finding extracted from ChainSecurity PDF '{handle.filename}'."
        ),
        ...
        "required_preconditions": [
            ...
            f"verification_tier={VERIFICATION_TIER}",
            f"ChainSecurity finding {finding.finding_id}",
        ],
...
def process_listing(
    ...
    *,
    ...
    findings = pdf_finding_extractor.extract_chainsecurity_findings(extraction)
    if not findings:
        return {
            "listing_record_id": handle.record_id,
            "pdf_url": handle.pdf_url,
...
def run(args: argparse.Namespace) -> int:
    ...
    if args.source:
        ...
        handles = list(iter_chainsecurity_listings(listings_dir))
    ...
    if args.json_summary:
        ...
        sys.stdout.write(
            f"[w24-chainsecurity] listings_seen={summary['listings_seen']} "
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument(
        ...
        default="",
        help=(
            "Directory of ChainSecurity PDFs to mine directly (bypasses "
    ...
    p.add_argument(
        "--out-dir",
        default="audit/corpus_tags/tags/firm-chainsecurity-audits",
