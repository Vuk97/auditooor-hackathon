#!/usr/bin/env python3
"""Wave-2 W2.4 OpenZeppelin-firm PDF deep-mine ETL.

Walks the Wave-1 listing tree, filters to the ``openzeppelin-audits`` firm
subset, fetches the PDF blob into a local gitignored cache, and emits one
``auditooor.hackerman_record.v1``-style per-finding record per
OpenZeppelin finding to::

    audit/corpus_tags/tags/firm-openzeppelin-audits/<sanitized-report-slug>/<finding-id>.yaml
...
``hackerman-etl-from-audit-firm-pdf-zellic.py``,
``hackerman-etl-from-audit-firm-pdf-cyfrin.py``, and
``hackerman-etl-from-audit-firm-pdf-spearbit.py``.

OpenZeppelin reports use a bracketed severity-coded finding-ID layout
(``[C-01]`` / ``[H-01]`` / ``[M-01]`` / ``[L-01]`` / ``[N-01]`` /
``[I-01]``) where ``N`` is the OZ-specific Note / "None" tier. Findings
carry Description / Recommendation / (optional Mitigation) /
(optional Resolution) subsections. Resolution lines reference a fix PR#
(canonical OZ shape) or a commit SHA.

See ``tools/lib/pdf_finding_extractor.py`` ``extract_openzeppelin_findings``
...
* PDFs fetched via ``urllib.request`` to the canonical
  ...
  scraping, no firm-website crawling. Filenames are percent-encoded
  before fetch.
...
* No edits to existing tools. W2.4 OpenZeppelin is purely additive.
...
CLI::
    ...
    python3 tools/hackerman-etl-from-audit-firm-pdf-openzeppelin.py \\
        --source tools/tests/fixtures/audit_firm_pdf_samples/openzeppelin \\
        --out /tmp/oz_test_emit --dry-run --json-summary
    ...
    python3 tools/hackerman-etl-from-audit-firm-pdf-openzeppelin.py \\
        ...
        --out-dir audit/corpus_tags/tags/firm-openzeppelin-audits \\
...
FIRM_PREFIX = "openzeppelin-contracts-audits"  # must match the Wave-1 listing dir prefix (was "openzeppelin-audits" - drift -> 0 listings)
PARSER_FIRM_VARIANT = "openzeppelin"
...
def _percent_encode_path(url: str) -> str:
    ...
    OpenZeppelin filenames can carry spaces / parentheses / commas.
...
def iter_openzeppelin_listings(listings_dir: Path) -> Iterable[ListingHandle]:
...
def iter_source_dir_listings(source_dir: Path) -> Iterable[ListingHandle]:
    ...
    for child in sorted(source_dir.iterdir()):
        if child.is_file() and child.suffix.lower() == ".pdf":
            ...
            yield ListingHandle(
                record_id=f"source-mode:openzeppelin:{child.stem}",
...
def build_finding_record(
    handle: ListingHandle,
    finding: "pdf_finding_extractor.OpenZeppelinFinding",
    blob_sha256: str,
    *,
    parser_version: str,
    ...
    record: Dict[str, Any] = {
        ...
        "fix_anti_pattern_avoided": (
            ...
            "applying OpenZeppelin's recommended mitigation."
        ),
        ...
        "attacker_action_sequence": (
            ...
            f"Audit-firm finding extracted from OpenZeppelin PDF '{handle.filename}'."
        ),
        ...
        "required_preconditions": [
            ...
            f"verification_tier={VERIFICATION_TIER}",
            f"OpenZeppelin finding {finding.finding_id}",
        ],
        ...
        "record_extensions": {
            ...
            "resolution_ref": finding.resolution_ref,
...
def process_listing(
    ...
    *,
    ...
    findings = pdf_finding_extractor.extract_openzeppelin_findings(extraction)
    if not findings:
        return {
            "listing_record_id": handle.record_id,
            "pdf_url": handle.pdf_url,
...
def run(args: argparse.Namespace) -> int:
    ...
    if args.source:
        ...
        handles = list(iter_openzeppelin_listings(listings_dir))
    ...
    if args.json_summary:
        ...
        sys.stdout.write(
            f"[w24-openzeppelin] listings_seen={summary['listings_seen']} "
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument(
        ...
        default="",
        help=(
            "Directory of OpenZeppelin PDFs to mine directly (bypasses "
            "Wave-1 listings tree). One synthesised listing handle per *.pdf."
        ),
    )
    p.add_argument(
        "--cache-dir",
    ...
    p.add_argument(
        "--out-dir",
        default="audit/corpus_tags/tags/firm-openzeppelin-audits",
