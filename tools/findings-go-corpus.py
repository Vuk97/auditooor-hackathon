#!/usr/bin/env python3
"""findings-go-corpus.py — validator for reference/findings_go.jsonl.

Asserts every row in the Go-language findings corpus has the required
fields and that language=="go". Exit non-zero on any violation. Stdlib-only.

Usage:
    python3 tools/findings-go-corpus.py [--path PATH] [--json]

Used by Makefile target ``findings-go-validate`` and by the regression test
``tools/tests/test_findings_go_corpus.py``.
"""
...
DEFAULT_PATH = REPO / "reference" / "findings_go.jsonl"
...
def validate_row(row: dict[str, Any], idx: int) -> list[str]:
    ...
    if "language" in row and row["language"] != "go":
        errors.append(
            f"row {idx}: language must be 'go' (got {row['language']!r}); "
            f"corpus is Go-scoped"
        )
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        ...
        help="Path to findings_go.jsonl (default: reference/findings_go.jsonl)",
    )
    parser.add_argument(
        "--json",
        action="store_true",
    ...
    if args.json:
        ...
        if not errors:
            print("PASS: all rows have required fields and language=='go'")
