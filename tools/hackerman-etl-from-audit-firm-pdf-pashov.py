#!/usr/bin/env python3
"""Wave-2 W2.4 Phase-1 deep-mine ETL (Pashov subset).

Walks the Wave-1 listing tree, filters to the ``pashov-audits`` firm
subset (286 PDFs), fetches the PDF blob into a local gitignored cache,
and emits one ``auditooor.hackerman_record.v1``-style per-finding
record per Pashov finding to::

    audit/corpus_tags/tags/audit_firm_findings_pashov/<firm>__<project>__<finding_id>/record.{json,yaml}

Sibling driver to ``hackerman-etl-from-audit-firm-pdf-tob.py`` and
``hackerman-etl-from-audit-firm-pdf-sherlock.py``. Pashov reports use a
bracketed severity-coded finding-ID layout (``[H-1]``, ``[M-2]``,
``[L-3]``, ``[C-1]``, ``[I-1]``) with Markdown-ish
``Description`` / ``Recommendation`` / optional ``Proof of Concept``
subsections. Older pre-2024 Pashov PDFs use a numeric
``N. Title`` heading + body ``Severity:`` label; the parser falls back
to that template when the bracketed pass yields zero candidates. See
``tools/lib/pdf_finding_extractor.py`` ``extract_pashov_findings`` for
the parser implementation.

Spec ref: ``docs/WAVE2_W24_PDF_DEEPMINE_SPEC_2026-05-16.md`` §5.6
(Phase 1, Pashov parser variant per PR-Wave2-B task brief).
...
* PDFs are fetched via ``urllib.request`` to the canonical
  ...
  scraping, no firm-website crawling. Pashov filenames occasionally
...
FIRM_PREFIX = "pashov-audits"
PARSER_FIRM_VARIANT = "pashov"
...
# real Pashov filenames can contain spaces.
...
def _percent_encode_path(url: str) -> str:
    ...
    Pashov filenames can contain spaces / parentheses / commas (e.g.
...
def iter_pashov_listings(listings_dir: Path) -> Iterable[ListingHandle]:
...
def _record_quality_score(finding: pdf_finding_extractor.PashovFinding) -> float:
...
def _fix_pattern_for_finding(
    handle: ListingHandle,
    finding: pdf_finding_extractor.PashovFinding,
) -> str:
    if finding.recommendation:
        return finding.recommendation[:600]
    return (
        f"Review and address the Pashov audit finding '{finding.title}' in "
...
def build_finding_record(
    ...
    *,
    ...
    severity_code = finding.severity_code or schema_severity[:1].upper() or "U"
    ...
    record: Dict[str, Any] = {
        ...
        "record_source_url": handle.pdf_url,
        ...
        "attacker_action_sequence": (
            ...
            (finding.summary[:1200] if finding.summary else
             f"Audit-firm finding extracted from Pashov PDF '{handle.filename}'.")
        ),
        ...
        "required_preconditions": [
            ...
            f"verification_tier={VERIFICATION_TIER}",
            f"Pashov finding {severity_code}-{finding.finding_index}",
        ],
...
def process_listing(
    ...
    *,
    ...
    for finding in findings:
        ...
        title_slug = pdf_finding_extractor.slugify_title(finding.title)
        severity_code = finding.severity_code or _schema_severity(finding.severity)[:1].upper() or "U"
...
def run(args: argparse.Namespace) -> int:
    ...
    handles = list(iter_pashov_listings(listings_dir))
    ...
    if args.json_summary:
        ...
        sys.stdout.write(
            f"[w24-pashov] listings_seen={summary['listings_seen']} "
