...
PATTERN = "partially-implemented-two-step-ownership-transfer-17"
DETECTOR = (
    ROOT
    / "detectors"
    / "wave_graveyard"
    / "wave14_broken"
    / "partially_implemented_two_step_ownership_transfer_17.py"
)
...
FIXTURE_DIR = (
    ...
    / "detectors"
    / "fixtures"
    / "partially_implemented_two_step_ownership_transfer_17"
)
ALT_FIXTURE_DIR = (
    ROOT
    / "detectors"
    / "fixtures"
    / "partially-implemented-two-step-ownership-transfer-17"
)
...
class PartiallyImplementedTwoStepOwnershipTransfer17Test(unittest.TestCase):
    ...
    def test_detector_reference_and_fixture_scope_stay_aligned(self) -> None:
        ...
        self.assertIn("_PENDING_SENDER_GUARD", detector_text)
        self.assertIn("NOT_SUBMIT_READY", detector_text)
        self.assertIn("pending nominee", detector_text)
        ...
        self.assertIn("function transferOwnership(address newOwner) external onlyOwner", positive_text)
        self.assertIn("function acceptOwnership() external onlyOwner", positive_text)
        self.assertIn("owner = pendingOwner;", positive_text)
        self.assertNotIn("msg.sender == pendingOwner", positive_text)

        self.assertIn("function acceptOwnership() external", clean_text)
        self.assertIn('require(msg.sender == pendingOwner, "not pending owner");', clean_text)
        self.assertIn("owner = pendingOwner;", clean_text)
        ...
        self.assertIn("pendingOwner", payload["limitation_note"])
