...
PATTERN = "perp-liquidation-unwrap-native-ignores-cross-chain-recipient"
DETECTOR = ROOT / "detectors" / "wave17" / "perp_liquidation_unwrap_native_ignores_cross_chain_recipient.py"
REFERENCE = ROOT / "reference" / "patterns.dsl" / f"{PATTERN}.yaml"
FIXTURE_DIR = ROOT / "detectors" / "fixtures" / "perp_liquidation_unwrap_native_ignores_cross_chain_recipient"
ALT_FIXTURE_DIR = ROOT / "detectors" / "fixtures" / "perp-liquidation-unwrap-native-ignores-cross-chain-recipient"
...
class PerpLiquidationUnwrapNativeIgnoresCrossChainRecipientTest(unittest.TestCase):
    ...
    def test_detector_reference_and_fixture_scope_stay_aligned(self) -> None:
        ...
        self.assertIn("shouldUnwrapNativeToken", detector_text)
        self.assertIn("positionLastSrcChainId|srcChainId|multichainBalance", detector_text)
        ...
        self.assertIn("contract LiquidationUtilsPositive", positive_text)
        self.assertIn("function createLiquidationOrder(IPosition position)", positive_text)
        self.assertIn("bool shouldUnwrapNativeToken = true;", positive_text)
        self.assertIn("flags = Order.Flags(position.isLong(), true);", positive_text)
        self.assertNotIn("srcChainId", positive_text)

        self.assertIn("contract LiquidationUtilsClean", clean_text)
        self.assertIn("uint256 srcChainId = 42161;", clean_text)
        self.assertIn("bool shouldUnwrapNativeToken = srcChainId == 0 ? true : false;", clean_text)
        self.assertIn("flags = Order.Flags(position.isLong(), shouldUnwrapNativeToken);", clean_text)
        ...
        self.assertIn("srcChainId", payload["limitation_note"])
