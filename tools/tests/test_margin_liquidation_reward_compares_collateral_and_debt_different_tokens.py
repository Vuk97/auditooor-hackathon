...
PATTERN = "margin-liquidation-reward-compares-collateral-and-debt-different-tokens"
DETECTOR = ROOT / "detectors" / "wave17" / "margin_liquidation_reward_compares_collateral_and_debt_different_tokens.py"
REFERENCE = ROOT / "reference" / "patterns.dsl" / f"{PATTERN}.yaml"
FIXTURE_DIR = ROOT / "detectors" / "fixtures" / "margin_liquidation_reward_compares_collateral_and_debt_different_tokens"
ALT_FIXTURE_DIR = ROOT / "detectors" / "fixtures" / "margin-liquidation-reward-compares-collateral-and-debt-different-tokens"
...
class MarginLiquidationRewardComparesCollateralAndDebtDifferentTokensTest(unittest.TestCase):
    ...
    def test_detector_reference_and_fixture_metadata_stay_scoped(self) -> None:
        ...
        self.assertIn("debtFutureValue * rewardPct", detector_text)
        self.assertIn("convertToCollateral", detector_text)
        ...
        self.assertIn("function executeLiquidate", positive_text)
        self.assertIn("liquidatorReward = Math.min(collateralSurplus, debtFutureValue * liquidatorRewardPercent / PERCENT);", positive_text)
        self.assertIn("collateralToken.transfer(liquidator, liquidatorReward);", positive_text)

        self.assertIn("function executeLiquidate", clean_text)
        self.assertIn("uint256 debtRewardInCollateral = convertToCollateral", clean_text)
        self.assertIn("liquidatorReward = Math.min(collateralSurplus, debtRewardInCollateral);", clean_text)
        ...
        self.assertIn("source-shape proof only", smoke["limitation_note"])
