...
DETECTOR = ROOT / "detectors" / "go_wave1" / "go-bridge-destination-settlement-unproven-source-commitment.py"
FIXTURE_DIR = ROOT / "detectors" / "go_wave1" / "test_fixtures"
PATTERN = "go-bridge-destination-settlement-unproven-source-commitment"
DETECTOR_ID = "go_wave1.go-bridge-destination-settlement-unproven-source-commitment"
...
class GoBridgeDestinationSettlementUnprovenSourceCommitmentTest(unittest.TestCase):
    def test_detector_ids_route_to_bridge_proof_domain_bypass(self) -> None:
        detector_map = yaml.safe_load(DETECTOR_TO_AC_MAP.read_text(encoding="utf-8"))["mappings"]
        self.assertEqual(detector_map[PATTERN][0], "bridge-proof-domain-bypass")
    ...
    def _hits(self, fixture: Path) -> tuple[int, str]:
        ...
        with tempfile.NamedTemporaryFile(prefix=".go_bridge_source_commit_", suffix=".log") as tmp:
            proc = subprocess.run(
                [
                    python_ast,
                    str(TOOL),
