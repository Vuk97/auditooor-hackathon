...
PATTERN = "native-asset-mint-without-msgvalue-equality-check"
DETECTOR = ROOT / "detectors" / "wave17" / "native_asset_mint_without_msgvalue_equality_check.py"
REFERENCE = ROOT / "reference" / "patterns.dsl" / f"{PATTERN}.yaml"
FIXTURE_DIR = ROOT / "detectors" / "fixtures" / "native_asset_mint_without_msgvalue_equality_check"
ALT_FIXTURE_DIR = ROOT / "detectors" / "fixtures" / "native-asset-mint-without-msgvalue-equality-check"
...
class NativeAssetMintWithoutMsgvalueEqualityCheckTest(unittest.TestCase):
    ...
    def test_detector_reference_and_fixture_scope_stay_aligned(self) -> None:
        ...
        self.assertIn("assets == msg.value", detector_text)
        self.assertIn("depositEth", detector_text)
        self.assertIn("depositRequest", detector_text)
        ...
        self.assertIn("function requestDeposit(", positive_text)
        self.assertIn("pendingSilo.depositEth{value: msg.value}(controller);", positive_text)
        self.assertIn("epochs[currentEpoch].depositRequest[controller] += assets;", positive_text)
        self.assertNotIn("require(assets == msg.value", positive_text)

        self.assertIn("function requestDeposit(", clean_text)
        self.assertIn('require(assets == msg.value, "native/assets mismatch");', clean_text)
        self.assertIn("pendingSilo.depositEth{value: msg.value}(controller);", clean_text)
        ...
        self.assertIn("depositRequest[controller] += assets;", payload["limitation_note"])
        self.assertIn("assets == msg.value", payload["limitation_note"])
