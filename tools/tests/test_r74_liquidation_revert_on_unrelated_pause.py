...
PATTERN = "r74-liquidation-revert-on-unrelated-pause"
DETECTOR = ROOT / "detectors" / "wave17" / "r74_liquidation_revert_on_unrelated_pause.py"
REFERENCE = ROOT / "reference" / "patterns.dsl" / f"{PATTERN}.yaml"
FIXTURE_DIR = ROOT / "detectors" / "fixtures" / "r74_liquidation_revert_on_unrelated_pause"
...
class R74LiquidationRevertOnUnrelatedPauseTest(unittest.TestCase):
    ...
    def test_pattern_compile_round_trip_matches_generated_detector(self) -> None:
        ...
        with tempfile.TemporaryDirectory(
            prefix=".pattern_compile_r74_liquidation_pause_",
            dir=ROOT,
        ) as tmp:
    ...
    def test_detector_reference_and_fixture_metadata(self) -> None:
        ...
        self.assertIn("assets?|reserves?|collaterals?", detector_text)
        ...
        self.assertIn("contract MultiAssetLiquidationPositive", positive_text)
        self.assertIn("function liquidate(", positive_text)
        self.assertIn("borrowerAssets.length", positive_text)
        self.assertIn("require(!reserve.paused", positive_text)
        self.assertNotIn("if (paused) continue;", positive_text)

        self.assertIn("contract MultiAssetLiquidationClean", clean_text)
        self.assertIn("if (paused) continue;", clean_text)
        self.assertIn("if (reserve.price == 0) continue;", clean_text)
