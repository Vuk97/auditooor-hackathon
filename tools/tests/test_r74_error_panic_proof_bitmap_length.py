...
PATTERN = "r74-error-panic-proof-bitmap-length"
DETECTOR = ROOT / "detectors" / "wave17" / "r74_error_panic_proof_bitmap_length.py"
REFERENCE = ROOT / "reference" / "patterns.dsl" / f"{PATTERN}.yaml"
FIXTURE_DIR = ROOT / "detectors" / "fixtures" / "r74_error_panic_proof_bitmap_length"
...
class R74ErrorPanicProofBitmapLengthTest(unittest.TestCase):
    ...
    def test_pattern_compile_round_trip_matches_generated_detector(self) -> None:
        ...
        with tempfile.TemporaryDirectory(
            prefix=".pattern_compile_r74_error_panic_proof_bitmap_length_",
            dir=ROOT,
        ) as tmp:
    ...
    def test_detector_reference_and_fixture_metadata(self) -> None:
        ...
        self.assertIn("bitmap\\\\.length", detector_text)
        self.assertIn("siblings\\\\s*\\\\[\\\\s*\\\\w+", detector_text)
        self.assertIn("MAX_(?:BITMAP|DEPTH|SIBLINGS)", detector_text)
        ...
        self.assertIn("contract BitmapProofVerifierPositive", positive_text)
        self.assertIn("byteIndex < bitmap.length", positive_text)
        self.assertIn("siblings[siblingIndex]", positive_text)
        self.assertNotIn("MAX_BITMAP_BYTES", positive_text)

        self.assertIn("contract BitmapProofVerifierClean", clean_text)
        self.assertIn("require(bitmap.length <= MAX_BITMAP_BYTES", clean_text)
        self.assertIn("require(siblings.length <= MAX_DEPTH", clean_text)
        self.assertIn("require(leafKeys.length <= bitmap.length * 8", clean_text)
