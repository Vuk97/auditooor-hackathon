...
PATTERN = "fund-loss-value-math-external-transfer-fire10"
...
for contract in slither.contracts:
    ...
    for function in contract.functions_and_modifiers_declared:
        if eval_function_match(function, spec.get("match", [])):
            hits.append(f"{contract.name}.{function.name}")
print("HITS=" + str(len(hits)))
print("HIT_NAMES=" + ",".join(hits))
"""
...
class FundLossValueMathExternalTransferFire10Test(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def test_dsl_metadata_and_fixtures_are_source_backed(self) -> None:
        ...
        self.assertEqual(spec["attack_class"], "fund-loss-via-arithmetic")
        ...
        self.assertTrue(
            all(
                ref["verification_tier"] in {"tier-2-verified-public-archive", "source-state-validated-pre-fix"}
                for ref in spec["references"]
            )
        )

        self.assertIn("returns (uint128 amountValueUnclamped)", positive)
        self.assertIn("amountValueUnclamped > holding_.assetAmountValue", positive)
        self.assertIn("ledger.postValueDecrease(holder, value);", positive)

        self.assertIn("returns (uint128 amountValueBooked)", negative)
        self.assertIn("amountValueBooked = amountValueUnclamped > holding_.assetAmountValue", negative)
        self.assertIn("holding_.assetAmountValue -= amountValueBooked;", negative)

    def test_dsl_compiles_to_detector_syntax_without_checked_in_emission(self) -> None:
        compiler = _load_module(COMPILE, "pattern_compile_fire10_value_math")
    ...
    def test_positive_fixture_fires_and_negative_fixture_stays_quiet(self) -> None:
        ...
        self.assertEqual(positive_hits, 1, positive_output)
        self.assertIn("HIT_NAMES=Fire10Holdings.decrease", positive_output)
