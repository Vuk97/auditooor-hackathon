...
PATTERN = "multi-signer-consensus-mutation-of-observed-state-breaks-digest"
DETECTOR = (
    ROOT
    / "detectors"
    / "wave17"
    / "multi_signer_consensus_mutation_of_observed_state_breaks_digest.py"
)
...
FIXTURE_DIR = (
    ...
    / "detectors"
    / "fixtures"
    / "multi_signer_consensus_mutation_of_observed_state_breaks_digest"
)
ALT_FIXTURE_DIR = (
    ROOT
    / "detectors"
    / "fixtures"
    / "multi-signer-consensus-mutation-of-observed-state-breaks-digest"
)
...
class MultiSignerConsensusMutationOfObservedStateBreaksDigestTest(unittest.TestCase):
    ...
    def test_pattern_compile_round_trip_matches_generated_detector(self) -> None:
        ...
        with tempfile.TemporaryDirectory(
            prefix=".pattern_compile_multi_signer_digest_",
            dir=ROOT,
        ) as tmp:
    ...
    def test_detector_reference_and_fixture_scope_stay_aligned(self) -> None:
        ...
        self.assertIn("ConsistencyLevel", detector_text)
        ...
        self.assertIn("detector_fixture_smoke_only", reference_text)

        self.assertIn("function cclHandleMessage", positive_text)
        self.assertIn("pe.message.ConsistencyLevel = readContract", positive_text)
        self.assertIn("message.ConsistencyLevel", positive_text)

        self.assertIn("function cclHandleMessage", clean_text)
        self.assertIn("pe.effectiveCL = readContract", clean_text)
        self.assertNotIn("pe.message.ConsistencyLevel = readContract", clean_text)
    ...
    def test_hyphenated_fixture_mirror_stays_in_sync(self) -> None:
        self.assertEqual(
            ...
            ALT_POSITIVE.read_text(encoding="utf-8"),
        )
