...
PATTERN = "pausable-contract-which-exposed-whennotpaused-only-exposed-pause-thus"
DETECTOR = (
    ROOT
    / "detectors"
    / "wave17"
    / "pausable_contract_which_exposed_whennotpaused_only_exposed_pause_thus.py"
)
...
FIXTURE_DIR = (
    ...
    / "detectors"
    / "fixtures"
    / "pausable_contract_which_exposed_whennotpaused_only_exposed_pause_thus"
)
ALT_FIXTURE_DIR = (
    ROOT
    / "detectors"
...
class PausableContractWhichExposedWhennotpausedOnlyExposedPauseThusTest(unittest.TestCase):
    ...
    def test_detector_reference_and_fixture_scope_stay_aligned(self) -> None:
        ...
        self.assertIn("public/external wrapper that unpauses it", detector_text)
        self.assertIn("_pause()", detector_text)
        self.assertIn("_unpause()", detector_text)
        ...
        self.assertIn("pause()", reference_text)
        self.assertIn("unpause()", reference_text)

        self.assertIn("function pause() external onlyOwner", positive_text)
        self.assertIn("function deposit(uint256 amount) external whenNotPaused", positive_text)
        self.assertNotIn("function unpause() external onlyOwner", positive_text)

        self.assertIn("function pause() external onlyOwner", clean_text)
        self.assertIn("function unpause() external onlyOwner", clean_text)
        ...
        self.assertIn("no public/external `_unpause()` wrapper", payload["limitation_note"])
