...
PATTERN = "liquidation-seized-assets-to-shares-down-no-readjust"
REFERENCE = ROOT / "reference" / "patterns.dsl" / f"{PATTERN}.yaml"
DETECTOR = ROOT / "detectors" / "wave17" / "liquidation_seized_assets_to_shares_down_no_readjust.py"
...
class LiquidationSeizedAssetsToSharesDownNoReadjustTest(unittest.TestCase):
    ...
    def test_pattern_metadata_is_source_backed_and_bounded(self) -> None:
        spec = yaml.safe_load(REFERENCE.read_text(encoding="utf-8"))

        self.assertEqual(spec["pattern"], PATTERN)
        self.assertEqual(spec["source"], "auditooor-fire4-lt-solodit-40874")
        self.assertEqual(spec["severity"], "MEDIUM")
        self.assertIn("liquidation-trigger-poison", spec["tags"])
        self.assertIn("source-backed-cantina-morpho-40874", spec["tags"])
        ...
        self.assertIn("Solodit/Cantina Morpho #40874", REFERENCE.read_text(encoding="utf-8"))

    def test_pattern_compile_round_trip_matches_generated_detector(self) -> None:
        compiler = _load_pattern_compile()
        with tempfile.TemporaryDirectory(prefix=".pattern_compile_lt_", dir=ROOT) as tmp:
    ...
    def test_detector_imports_and_fixture_shapes_are_distinct(self) -> None:
        py_compile.compile(str(DETECTOR), doraise=True)
        vuln_text = VULN.read_text(encoding="utf-8")
        clean_text = CLEAN.read_text(encoding="utf-8")

        self.assertIn("repaidShares = toSharesDown", vuln_text)
        self.assertNotIn("repaidAssets = toAssetsUp(repaidShares", vuln_text)
        self.assertIn("repaidShares = toSharesUp", clean_text)
        self.assertIn("repaidAssets = toAssetsUp(repaidShares", clean_text)

    def test_vulnerable_fixture_fires_and_clean_fixture_is_silent(self) -> None:
        self.assertGreaterEqual(self._hits(VULN), 1)
