...
PATTERN = "preapproval-signature-bypass-meta"
DETECTOR = ROOT / "detectors" / "wave17" / "preapproval_signature_bypass_meta.py"
REFERENCE = ROOT / "reference" / "patterns.dsl" / f"{PATTERN}.yaml"
FIXTURE_DIR = ROOT / "detectors" / "fixtures" / "preapproval_signature_bypass_meta"
...
class PreapprovalSignatureBypassMetaTest(unittest.TestCase):
    ...
    def test_pattern_compile_round_trip_matches_generated_detector(self) -> None:
        ...
        with tempfile.TemporaryDirectory(
            prefix=".pattern_compile_preapproval_signature_",
            dir=ROOT,
        ) as tmp:
    ...
    def test_detector_reference_and_fixture_metadata(self) -> None:
        ...
        self.assertIn("signature\\\\.length", detector_text)
        self.assertIn("preapproved\\\\[", detector_text)
        ...
        self.assertIn("contract PreapprovalExchangePositive", positive_text)
        self.assertIn("if (signature.length == 0)", positive_text)
        self.assertIn("require(preapproved[orderHash]", positive_text)
        self.assertNotIn("deadline", positive_text)
        self.assertNotIn("block.timestamp", positive_text)

        self.assertIn("contract PreapprovalExchangeClean", clean_text)
        self.assertIn("uint256 deadline", clean_text)
        self.assertIn("require(block.timestamp <= deadline", clean_text)
        self.assertIn("require(preapproved[orderHash]", clean_text)
        ...
        self.assertIn("empty-signature preapproval shape", payload["limitation_note"])
