...
PATTERN = "r74-auth-deauthorized-keeps-shares"
DETECTOR = ROOT / "detectors" / "wave17" / "r74_auth_deauthorized_keeps_shares.py"
REFERENCE = ROOT / "reference" / "patterns.dsl" / f"{PATTERN}.yaml"
FIXTURE_DIR = ROOT / "detectors" / "fixtures" / "r74_auth_deauthorized_keeps_shares"
MIRROR_DIR = ROOT / "detectors" / "fixtures" / "r74-auth-deauthorized-keeps-shares"
...
class R74AuthDeauthorizedKeepsSharesTest(unittest.TestCase):
    ...
    def test_detector_reference_and_smoke_posture_stay_aligned(self) -> None:
        ...
        self.assertIn("function.has_param_of_type", detector_text)
        ...
        self.assertIn("function removeFromWhitelist(address account) external", positive_text)
        self.assertIn("authorized[account] = false;", positive_text)
        self.assertNotIn("_burn(account", positive_text)

        self.assertIn("uint256 userShares = shares[account];", clean_text)
        self.assertIn("_burn(account, userShares);", clean_text)
        self.assertIn("authorized[account] = false;", clean_text)
        ...
        self.assertTrue(payload["positive_command"])
        self.assertTrue(payload["clean_command"])
