...
PATTERN = "liquidation-transferfrom-market-not-liquidator"
DETECTOR = ROOT / "detectors" / "wave17" / "liquidation_transferfrom_market_not_liquidator.py"
REFERENCE = ROOT / "reference" / "patterns.dsl" / f"{PATTERN}.yaml"
FIXTURE_DIR = ROOT / "detectors" / "fixtures" / "liquidation_transferfrom_market_not_liquidator"
ALT_FIXTURE_DIR = ROOT / "detectors" / "fixtures" / "liquidation-transferfrom-market-not-liquidator"
...
class LiquidationTransferfromMarketNotLiquidatorTest(unittest.TestCase):
    ...
    def test_detector_reference_and_fixtures_capture_not_submit_ready_scope(self) -> None:
        ...
        self.assertIn("address(this)", detector_text)
        self.assertIn("msg.sender", detector_text)
        ...
        self.assertIn("function executeLiquidate(int256 remainingMargin) external", positive_text)
        self.assertIn("address(this)", positive_text)
        self.assertNotIn("msg.sender", positive_text)
        self.assertIn("function executeLiquidate(int256 remainingMargin) external", clean_text)
        self.assertIn("msg.sender", clean_text)
