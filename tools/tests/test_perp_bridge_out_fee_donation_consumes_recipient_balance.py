...
PATTERN = "perp-bridge-out-fee-donation-consumes-recipient-balance"
DETECTOR = ROOT / "detectors" / "wave17" / "perp_bridge_out_fee_donation_consumes_recipient_balance.py"
REFERENCE = ROOT / "reference" / "patterns.dsl" / f"{PATTERN}.yaml"
FIXTURE_DIR = ROOT / "detectors" / "fixtures" / "perp_bridge_out_fee_donation_consumes_recipient_balance"
ALT_FIXTURE_DIR = ROOT / "detectors" / "fixtures" / "perp-bridge-out-fee-donation-consumes-recipient-balance"
...
class PerpBridgeOutFeeDonationConsumesRecipientBalanceTest(unittest.TestCase):
    ...
    def test_detector_reference_and_fixture_scope_stay_aligned(self) -> None:
        ...
        self.assertIn("_has_recipient_fee_donation_shape", detector_text)
        self.assertIn("account != receiver", detector_text)
        ...
        self.assertIn("contract PerpBridgeOutFeeDonationConsumesRecipientBalancePositive", positive_text)
        self.assertIn("mintMarketTokens(receiver, marketTokens);", positive_text)
        self.assertIn("bridgeOut(receiver, marketTokens, receiver);", positive_text)
        self.assertNotIn("if (account != receiver)", positive_text)

        self.assertIn("contract PerpBridgeOutFeeDonationConsumesRecipientBalanceClean", clean_text)
        self.assertIn("if (account != receiver)", clean_text)
        self.assertIn("bridgeOut(receiver, marketTokens, account);", clean_text)
