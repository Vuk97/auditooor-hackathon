...
PATTERN = "voting-power-self-delegation-double-count"
DETECTOR = ROOT / "detectors" / "wave68" / "voting_power_self_delegation_double_count.py"
REFERENCE = ROOT / "reference" / "patterns.dsl" / f"{PATTERN}.yaml"
FIXTURE_DIR = ROOT / "detectors" / "fixtures" / "voting_power_self_delegation_double_count"
...
class VotingPowerSelfDelegationDoubleCountRecallLiftTest(unittest.TestCase):
    ...
    def test_pattern_compile_round_trip_matches_generated_detector(self) -> None:
        ...
        with tempfile.TemporaryDirectory(prefix=".pattern_compile_voting_power_self_", dir=ROOT) as tmp:
    ...
    def test_fixture_smoke_manifest_and_hits(self) -> None:
        ...
        self.assertEqual(payload["detector_slug"], "voting_power_self_delegation_double_count")
        self.assertEqual(payload["positive_hits"], 1)
        self.assertEqual(payload["clean_hits"], 0)
        self.assertEqual(payload["status"], "passed_vulnerable_clean_smoke")
        self.assertIn("self-delegates", payload["notes"])
        self.assertIn(f'ARGUMENT = "{PATTERN}"', detector_text)
        self.assertIn("delegatee", detector_text)
        self.assertIn("hasVoted", detector_text)
        self.assertIn("fixtures:", reference_text)
        self.assertIn("detectors/fixtures/voting_power_self_delegation_double_count/positive.sol", reference_text)
        self.assertIn("detectors/fixtures/voting_power_self_delegation_double_count/clean.sol", reference_text)
        self.assertIn("delegatee != msg.sender", clean_text)
        self.assertIn("delegates[msg.sender] = msg.sender", positive_text)
