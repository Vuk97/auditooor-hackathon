...
DETECTOR = "bridge_lightclient_domain_fire35"
...
class RustBridgeLightclientDomainFire35Tests(unittest.TestCase):
    def test_detector_compiles(self) -> None:
        py_compile.compile(str(DETECTOR_PATH), doraise=True)

    def test_positive_fixture_fires_on_lightclient_proof_domain_omission(self) -> None:
        hits, log_text = _run_fixture(POSITIVE)
        self.assertGreaterEqual(hits, 1, log_text)
        self.assertIn("submit_light_client_update", log_text)
        self.assertIn("bridge-proof-domain-bypass", log_text)
        self.assertIn("client_id", log_text)
        self.assertIn("gateway", log_text)
        self.assertIn("nonce_lane", log_text)
