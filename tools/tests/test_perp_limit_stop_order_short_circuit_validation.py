...
PATTERN = "perp-limit-stop-order-short-circuit-validation"
DETECTOR = ROOT / "detectors" / "wave17" / "perp_limit_stop_order_short_circuit_validation.py"
REFERENCE = ROOT / "reference" / "patterns.dsl" / f"{PATTERN}.yaml"
FIXTURE_DIR = ROOT / "detectors" / "fixtures" / "perp_limit_stop_order_short_circuit_validation"
...
class PerpLimitStopOrderShortCircuitValidationTest(unittest.TestCase):
    ...
    def test_detector_reference_and_fixture_metadata(self) -> None:
        ...
        self.assertIn('COVERAGE_CLAIM = "detector_fixture_smoke_only"', detector_text)
        self.assertIn("_has_bad_short_circuit_guard", detector_text)
        self.assertIn("_NEGATED_LIMIT_RE", detector_text)
        self.assertIn("_NEGATED_STOP_RE", detector_text)
        ...
        self.assertIn("contract PerpLimitStopOrderRouter", positive_text)
        self.assertIn("OrderType.LIMIT_STOP", positive_text)
        self.assertIn("!validateLimitPrice(order, fillPrice) && !validateStopPrice(order, oraclePrice)", positive_text)

        self.assertIn("contract PerpLimitStopOrderRouterClean", clean_text)
        self.assertIn("OrderType.LIMIT_STOP", clean_text)
        self.assertIn("!validateLimitPrice(order, fillPrice) || !validateStopPrice(order, oraclePrice)", clean_text)
        self.assertNotIn("!validateLimitPrice(order, fillPrice) && !validateStopPrice(order, oraclePrice)", clean_text)
