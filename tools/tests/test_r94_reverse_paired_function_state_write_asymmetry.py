...
PATTERN = "r94-reverse-paired-function-state-write-asymmetry"
DETECTOR = ROOT / "detectors" / "wave17" / "r94_reverse_paired_function_state_write_asymmetry.py"
REFERENCE = ROOT / "reference" / "patterns.dsl" / f"{PATTERN}.yaml"
FIXTURE_DIR = ROOT / "detectors" / "fixtures" / "r94_reverse_paired_function_state_write_asymmetry"
...
class R94ReversePairedFunctionStateWriteAsymmetryTest(unittest.TestCase):
    ...
    def test_detector_reference_and_fixture_scope_stay_aligned(self) -> None:
        ...
        self.assertIn("same-stem", detector_text)
        self.assertIn("operatorList", detector_text)
        ...
        self.assertIn("same-stem lifecycle shape", reference_text)

        self.assertIn("contract R94ReversePairedFunctionStateWriteAsymmetryPositive", positive_text)
        self.assertIn("operatorIndexPlusOne[operator] = operatorList.length + 1;", positive_text)
        self.assertIn("operatorList.push(operator);", positive_text)
        self.assertNotIn("operatorList.pop();", positive_text)

        self.assertIn("contract R94ReversePairedFunctionStateWriteAsymmetryClean", clean_text)
        self.assertIn("operatorList[index] = moved;", clean_text)
        self.assertIn("operatorList.pop();", clean_text)
        self.assertIn("delete operatorIndexPlusOne[operator];", clean_text)
        ...
        self.assertIn("same-stem add/remove registry shape", payload["limitation_note"])
