...
PATTERN = "vote-power-reassignment-old-source-not-debited"
REFERENCE = ROOT / "reference" / "patterns.dsl" / f"{PATTERN}.yaml"
DETECTOR = ROOT / "detectors" / "wave17" / "vote_power_reassignment_old_source_not_debited.py"
FIXTURE_DIR = ROOT / "detectors" / "fixtures" / "vote_power_reassignment_old_source_not_debited"
...
class VotePowerReassignmentOldSourceNotDebitedTest(unittest.TestCase):
    ...
    def test_pattern_compile_round_trip_matches_generated_detector(self) -> None:
        ...
        with tempfile.TemporaryDirectory(
            prefix=".pattern_compile_vote_power_old_source_",
            dir=ROOT,
        ) as tmp:
    ...
    def test_reference_yaml_and_smoke_metadata_stay_aligned(self) -> None:
        ...
        self.assertIn("vuln: detectors/fixtures/vote_power_reassignment_old_source_not_debited/positive.sol", yaml_text)
        self.assertIn("clean: detectors/fixtures/vote_power_reassignment_old_source_not_debited/clean.sol", yaml_text)
    ...
    def test_named_miss_and_adjacent_controls_have_expected_scope(self) -> None:
        ...
        self.assertEqual(hits, 0, log_text)
        hits, log_text = self._hits(LEGACY_ARRAY_POSITIVE)
        self.assertEqual(hits, 1, log_text)
