...
PATTERN = "fund-loss-arithmetic-fire22"
DETECTOR = ROOT / "detectors" / "wave17" / "fund_loss_arithmetic_fire22.py"
POSITIVE = (
    ROOT
    / "detectors"
    / "test_fixtures"
    / "positive"
    / "fund_loss_arithmetic_fire22.sol"
)
NEGATIVE = (
    ROOT
    / "detectors"
...
class FundLossArithmeticFire22Test(unittest.TestCase):
    ...
    def test_detector_and_fixture_sources_are_aligned(self) -> None:
        ...
        self.assertIn("balanceOf[msg.sender] += balanceOf[msg.sender] + amount;", positive_text)
        self.assertIn("claimableRewards[user] += claimableRewards[user] + rewardDelta;", positive_text)

        self.assertIn("balanceOf[msg.sender] += amount;", negative_text)
        self.assertIn("claimableRewards[user] = previousReward + rewardDelta;", negative_text)
