...
PATTERN = "r74-error-swap-both-directions-steal"
DETECTOR = ROOT / "detectors" / "wave17" / "r74_error_swap_both_directions_steal.py"
REFERENCE = ROOT / "reference" / "patterns.dsl" / f"{PATTERN}.yaml"
FIXTURE_DIR = ROOT / "detectors" / "fixtures" / "r74_error_swap_both_directions_steal"
...
class R74ErrorSwapBothDirectionsStealTest(unittest.TestCase):
    ...
    def test_pattern_compile_round_trip_matches_generated_detector(self) -> None:
        ...
        with tempfile.TemporaryDirectory(
            prefix=".pattern_compile_r74_error_swap_both_directions_steal_",
            dir=ROOT,
        ) as tmp:
    ...
    def test_detector_reference_and_fixture_metadata(self) -> None:
        ...
        self.assertIn("reserve0|reserve1|feeGrowth0|feeGrowth1", detector_text)
        self.assertIn("_checkInvariant", detector_text)
        ...
        self.assertIn("contract BidirectionalSwapPositive", positive_text)
        self.assertIn("function swap(bool zeroForOne, uint256 amountIn) external", positive_text)
        self.assertIn("feeGrowth0 += fee;", positive_text)
        self.assertIn("feeGrowth1 += fee;", positive_text)
        self.assertNotIn("_checkInvariant", positive_text)

        self.assertIn("contract BidirectionalSwapClean", clean_text)
        self.assertIn("uint256 kBefore = reserve0 * reserve1;", clean_text)
        self.assertIn("_checkInvariant(kBefore);", clean_text)
        self.assertIn('require(reserve0 * reserve1 >= kBefore, "K");', clean_text)
