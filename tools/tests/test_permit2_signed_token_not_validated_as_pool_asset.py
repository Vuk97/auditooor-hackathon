...
PATTERN = "permit2-signed-token-not-validated-as-pool-asset"
DETECTOR = ROOT / "detectors" / "wave17" / "permit2_signed_token_not_validated_as_pool_asset.py"
REFERENCE = ROOT / "reference" / "patterns.dsl" / f"{PATTERN}.yaml"
FIXTURE_DIR = ROOT / "detectors" / "fixtures" / "permit2_signed_token_not_validated_as_pool_asset"
ALT_FIXTURE_DIR = ROOT / "detectors" / "fixtures" / "permit2-signed-token-not-validated-as-pool-asset"
...
class Permit2SignedTokenNotValidatedAsPoolAssetTest(unittest.TestCase):
    ...
    def test_detector_reference_and_fixture_scope_stay_aligned(self) -> None:
        ...
        self.assertIn("_has_unvalidated_pool_asset_permit_shape", detector_text)
        ...
        self.assertIn("contract Permit2SignedTokenNotValidatedAsPoolAssetPositive", positive_text)
        self.assertIn("permit2.permitTransferFrom(permit, details, msg.sender, signature);", positive_text)
        self.assertIn("shares[msg.sender] += assets;", positive_text)
        self.assertNotIn("permit.permitted.token == asset()", positive_text)

        self.assertIn("contract Permit2SignedTokenNotValidatedAsPoolAssetClean", clean_text)
