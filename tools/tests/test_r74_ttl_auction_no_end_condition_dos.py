...
PATTERN = "r74-ttl-auction-no-end-condition-dos"
DETECTOR = ROOT / "detectors" / "wave17" / "r74_ttl_auction_no_end_condition_dos.py"
REFERENCE = ROOT / "reference" / "patterns.dsl" / f"{PATTERN}.yaml"
FIXTURE_DIR = ROOT / "detectors" / "fixtures" / "r74_ttl_auction_no_end_condition_dos"
...
class R74TtlAuctionNoEndConditionDosTest(unittest.TestCase):
    ...
    def test_pattern_compile_round_trip_matches_generated_detector(self) -> None:
        ...
        with tempfile.TemporaryDirectory(
            prefix=".pattern_compile_r74_ttl_auction_",
            dir=ROOT,
        ) as tmp:
    ...
    def test_detector_reference_and_fixture_metadata(self) -> None:
        ...
        self.assertIn("block.timestamp", detector_text)
        self.assertIn("ttl", detector_text)
        ...
        self.assertIn("contract TtlAuctionNoEndConditionPositive", positive_text)
        self.assertIn("function placeBid(uint256 auctionId) external payable", positive_text)
        self.assertIn("auction.endTime = block.timestamp + ttl;", positive_text)
        self.assertNotIn("hardEnd", positive_text)

        self.assertIn("contract TtlAuctionNoEndConditionClean", clean_text)
        self.assertIn("uint256 hardEnd;", clean_text)
        self.assertIn("if (extendedEnd > auction.hardEnd)", clean_text)
        self.assertIn("auction.endTime = extendedEnd;", clean_text)
