...
PATTERN = "r74-abi-writeoutputs-memory-corruption"
DETECTOR = ROOT / "detectors" / "wave17" / "r74_abi_writeoutputs_memory_corruption.py"
REFERENCE = ROOT / "reference" / "patterns.dsl" / f"{PATTERN}.yaml"
FIXTURE_DIR = ROOT / "detectors" / "fixtures" / "r74_abi_writeoutputs_memory_corruption"
MIRROR_DIR = ROOT / "detectors" / "fixtures" / "r74-abi-writeoutputs-memory-corruption"
...
class R74AbiWriteOutputsMemoryCorruptionTest(unittest.TestCase):
    ...
    def test_detector_reference_and_smoke_posture_stay_aligned(self) -> None:
        ...
        self.assertIn("function.assembly_block_matches", detector_text)
        ...
        self.assertIn("mstore(add(add(outputs, 0x20), mul(index, 0x20)), value)", positive_text)
        self.assertNotIn("require(index < outputs.length", positive_text)

        self.assertIn("mstore(add(add(outputs, 0x20), mul(index, 0x20)), value)", clean_text)
        self.assertIn('require(index < outputs.length, "output index out of bounds");', clean_text)
