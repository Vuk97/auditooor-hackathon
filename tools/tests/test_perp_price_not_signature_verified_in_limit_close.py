...
PATTERN = "perp-price-not-signature-verified-in-limit-close"
DETECTOR = ROOT / "detectors" / "wave17" / "perp_price_not_signature_verified_in_limit_close.py"
REFERENCE = ROOT / "reference" / "patterns.dsl" / f"{PATTERN}.yaml"
FIXTURE_DIR = ROOT / "detectors" / "fixtures" / "perp_price_not_signature_verified_in_limit_close"
ALT_FIXTURE_DIR = ROOT / "detectors" / "fixtures" / PATTERN
...
class PerpPriceNotSignatureVerifiedInLimitCloseTest(unittest.TestCase):
    ...
    def test_pattern_compile_round_trip_matches_generated_detector(self) -> None:
        ...
        with tempfile.TemporaryDirectory(
            prefix=".pattern_compile_perp_price_signature_",
            dir=ROOT,
        ) as tmp:
    ...
    def test_detector_reference_and_fixture_scope_stay_not_submit_ready(self) -> None:
        ...
        self.assertIn("_priceData\\\\.price", detector_text)
        self.assertIn("verified return value", detector_text)
        ...
        self.assertIn("owned fixture pair", reference_text)

        self.assertIn("function limitClose(", positive_text)
        self.assertIn("getVerifiedPrice(_priceData, _signature);", positive_text)
        self.assertIn("if (_priceData.price > 3_500)", positive_text)

        self.assertIn("function limitClose(", clean_text)
        self.assertIn("uint256 verifiedPrice = getVerifiedPrice(_priceData, _signature);", clean_text)
        self.assertIn("if (verifiedPrice > 3_500)", clean_text)
        self.assertNotIn("lastClosedPrice = _priceData.price;", clean_text)
        ...
        self.assertIn("owned limit-close shape", payload["limitation_note"])
