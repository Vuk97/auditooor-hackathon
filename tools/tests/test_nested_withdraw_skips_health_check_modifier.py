...
PATTERN = "nested-withdraw-skips-health-check-modifier"
DETECTOR = ROOT / "detectors" / "wave17" / "nested_withdraw_skips_health_check_modifier.py"
REFERENCE = ROOT / "reference" / "patterns.dsl" / f"{PATTERN}.yaml"
FIXTURE_DIR = ROOT / "detectors" / "fixtures" / "nested_withdraw_skips_health_check_modifier"
ALT_FIXTURE_DIR = ROOT / "detectors" / "fixtures" / "nested-withdraw-skips-health-check-modifier"
...
class NestedWithdrawSkipsHealthCheckModifierTest(unittest.TestCase):
    ...
    def test_detector_reference_and_fixture_scope_stay_aligned(self) -> None:
        ...
        self.assertIn("withdrawExactShares", detector_text)
        self.assertIn("healthStateCheck", detector_text)
        ...
        self.assertIn("function manualWithdraw(", positive_text)
        self.assertIn("wiseLending.withdrawExactShares(nftId, pendleChild, shares);", positive_text)
        self.assertNotIn("healthStateCheck", positive_text)

        self.assertIn("function manualWithdraw(", clean_text)
        self.assertIn("wiseLending.withdrawExactShares(nftId, pendleChild, shares);", clean_text)
        self.assertIn("wiseLending.healthStateCheck(nftId)", clean_text)
