...
PATTERN = "certora-aave-scaled-balance-consistency"
REFERENCE = ROOT / "reference" / "patterns.dsl" / f"{PATTERN}.yaml"
DETECTOR = ROOT / "detectors" / "wave17" / "certora_aave_scaled_balance_consistency.py"
FIXTURE_DIR = ROOT / "detectors" / "fixtures" / "certora_aave_scaled_balance_consistency"
...
class CertoraAaveScaledBalanceConsistencyTest(unittest.TestCase):
    ...
    def test_pattern_compile_round_trip_matches_generated_detector(self) -> None:
        ...
        with tempfile.TemporaryDirectory(
            prefix=".pattern_compile_certora_scaled_balance_",
            dir=ROOT,
        ) as tmp:
    ...
    def test_reference_yaml_points_at_owned_fixture_pair(self) -> None:
        ...
        self.assertIn(
            "vuln: detectors/fixtures/certora_aave_scaled_balance_consistency/ssi-fix-053_positive.sol",
            text,
        )
        self.assertIn(
            "clean: detectors/fixtures/certora_aave_scaled_balance_consistency/ssi-fix-053_clean.sol",
            text,
        )

    def test_fixture_shape_exercises_raw_and_scaled_paths(self) -> None:
        positive = POSITIVE.read_text(encoding="utf-8")
        clean = CLEAN.read_text(encoding="utf-8")

        self.assertIn("function credit", positive)
        self.assertIn("_balances[user] += amount;", positive)
        self.assertIn("_totalSupply += amount;", positive)
        self.assertNotIn("rayDiv", positive[positive.index("function credit"):])

        self.assertIn("uint256 scaledDelta = amount.rayDiv(liquidityIndex);", clean)
        self.assertIn("_scaledTotalSupply += scaledDelta;", clean)
        self.assertIn("_totalSupply = _scaledTotalSupply.rayMul(liquidityIndex);", clean)
