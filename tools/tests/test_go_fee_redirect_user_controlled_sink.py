...
DETECTOR = ROOT / "detectors" / "go_wave1" / "go-fee-redirect-user-controlled-sink.py"
FIXTURE_DIR = ROOT / "detectors" / "go_wave1" / "test_fixtures"
PATTERN = "go-fee-redirect-user-controlled-sink"
...
class GoFeeRedirectUserControlledSinkTest(unittest.TestCase):
    def _hits(self, fixture: Path) -> tuple[int, str]:
        ...
        with tempfile.NamedTemporaryFile(prefix=".go_fee_redirect_", suffix=".log") as tmp:
            proc = subprocess.run(
                [
                    python_ast,
                    str(TOOL),
    ...
    def test_fixtures_lock_user_sink_and_configured_collector_controls(self) -> None:
        positive = POSITIVE.read_text(encoding="utf-8")
        clean = CLEAN.read_text(encoding="utf-8")
        self.assertIn("msg.FeeRecipient", positive)
        self.assertIn("feeCoins", positive)
        self.assertIn("params.FeeCollector", clean)
        self.assertIn("Withdraw", clean)
