"""Tests for VaultQuery.vault_knowledge_gap_context callable (W3.8 coverage push).
...
Notes: no ledger -> error envelope
"""
...
class TestVaultKnowledgeGapContext(unittest.TestCase):

    def setUp(self):
        self.tmp = tempfile.TemporaryDirectory(prefix="w38-knowledge_gap_context-")
    ...
    def test_happy_path_returns_dict(self):
        # synthetic_fixture: true
        result = self.query.vault_knowledge_gap_context()
    ...
    def test_cli_dispatch_exits_zero(self):
        ...
        proc = subprocess.run(
            [sys.executable, str(MODULE_PATH), "--repo-root", str(self.root),
             "--vault-dir", str(self.vault), "--call", "vault_knowledge_gap_context",
             "--args", args_json],
