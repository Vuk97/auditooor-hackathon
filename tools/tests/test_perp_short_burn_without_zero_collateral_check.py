...
PATTERN = "perp-short-burn-without-zero-collateral-check"
DETECTOR = ROOT / "detectors" / "wave17" / "perp_short_burn_without_zero_collateral_check.py"
REFERENCE = ROOT / "reference" / "patterns.dsl" / f"{PATTERN}.yaml"
FIXTURE_DIR = ROOT / "detectors" / "fixtures" / "perp_short_burn_without_zero_collateral_check"
...
class PerpShortBurnWithoutZeroCollateralCheckTest(unittest.TestCase):
    ...
    def test_pattern_compile_round_trip_matches_generated_detector(self) -> None:
        ...
        with tempfile.TemporaryDirectory(
            prefix=".pattern_compile_perp_short_burn_zero_collateral_",
            dir=ROOT,
        ) as tmp:
    ...
    def test_detector_reference_and_fixture_metadata(self) -> None:
        ...
        self.assertIn("contract ShortTokenPositive", positive_text)
        self.assertIn("if (position.shortAmount == 0)", positive_text)
        self.assertIn("_burn(tokenId);", positive_text)
        self.assertNotIn("position.collateralAmount == 0", positive_text)

        self.assertIn("contract ShortTokenClean", clean_text)
        self.assertIn("position.shortAmount == 0 && position.collateralAmount == 0", clean_text)
