...
PATTERN = "match-orders-fee-reentrancy-via-1155-receiver"
DETECTOR = ROOT / "detectors" / "wave17" / "match_orders_fee_reentrancy_via_1155_receiver.py"
REFERENCE = ROOT / "reference" / "patterns.dsl" / f"{PATTERN}.yaml"
FIXTURE_DIR = ROOT / "detectors" / "fixtures" / "match_orders_fee_reentrancy_via_1155_receiver"
ALT_FIXTURE_DIR = ROOT / "detectors" / "fixtures" / "match-orders-fee-reentrancy-via-1155-receiver"
...
class MatchOrdersFeeReentrancyVia1155ReceiverTest(unittest.TestCase):
    ...
    def test_detector_reference_and_fixture_scope_stay_aligned(self) -> None:
        ...
        self.assertIn("_has_ordered_fee_reentrancy_shape", detector_text)
        ...
        self.assertIn("function matchOrders(", positive_text)
        self.assertIn("conditionalTokens.balanceOf(address(this), feeAssetId)", positive_text)
        self.assertIn("conditionalTokens.safeTransferFrom(", positive_text)
        self.assertIn("orderStatus[orderHash] = 1;", positive_text)
        self.assertNotIn("nonReentrant", positive_text)

        self.assertIn("modifier nonReentrant()", clean_text)
        self.assertIn("function matchOrders(", clean_text)
