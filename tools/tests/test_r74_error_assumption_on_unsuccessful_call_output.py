...
PATTERN = "r74-error-assumption-on-unsuccessful-call-output"
DETECTOR = ROOT / "detectors" / "wave17" / "r74_error_assumption_on_unsuccessful_call_output.py"
REFERENCE = ROOT / "reference" / "patterns.dsl" / f"{PATTERN}.yaml"
FIXTURE_DIR = ROOT / "detectors" / "fixtures" / "r74_error_assumption_on_unsuccessful_call_output"
...
class R74ErrorAssumptionOnUnsuccessfulCallOutputTest(unittest.TestCase):
    ...
    def test_pattern_compile_round_trip_matches_generated_detector(self) -> None:
        ...
        with tempfile.TemporaryDirectory(
            prefix=".pattern_compile_r74_error_assumption_",
            dir=ROOT,
        ) as tmp:
    ...
    def test_detector_reference_and_fixture_metadata(self) -> None:
        ...
        self.assertIn("0x08c379a0|0x4e487b71", detector_text)
        self.assertIn("verifyCallResultFromTarget", detector_text)
        ...
        self.assertIn("contract ErrorAssumptionOnUnsuccessfulCallOutputPositive", positive_text)
        self.assertIn("selector == 0x08c379a0", positive_text)
        self.assertIn("selector == 0x4e487b71", positive_text)
        self.assertIn('return "unknown revert";', positive_text)

        self.assertIn("contract ErrorAssumptionOnUnsuccessfulCallOutputClean", clean_text)
        self.assertIn("revert(add(returndata, 32), mload(returndata))", clean_text)
        self.assertNotIn("0x08c379a0", clean_text)
