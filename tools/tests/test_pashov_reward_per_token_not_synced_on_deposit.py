...
PATTERN = "pashov-reward-per-token-not-synced-on-deposit"
DETECTOR = ROOT / "detectors" / "wave17" / "pashov_reward_per_token_not_synced_on_deposit.py"
REFERENCE = ROOT / "reference" / "patterns.dsl" / f"{PATTERN}.yaml"
FIXTURE_DIR = ROOT / "detectors" / "fixtures" / "pashov_reward_per_token_not_synced_on_deposit"
ALT_FIXTURE_DIR = ROOT / "detectors" / "fixtures" / "pashov-reward-per-token-not-synced-on-deposit"
...
class PashovRewardPerTokenNotSyncedOnDepositTest(unittest.TestCase):
    ...
    def test_detector_reference_and_fixture_scope_stay_aligned(self) -> None:
        ...
        self.assertIn("_has_unsynced_reward_per_token_snapshot_shape", detector_text)
        self.assertIn("_CHECKPOINT_ASSIGN_RE", detector_text)
        self.assertIn("_UPDATE_CALL_RE", detector_text)
        ...
        self.assertIn("checkpoint.rewardPerTokenPaid[token] = extraRewardPerToken[token];", positive_text)
        self.assertNotIn("_updateExtraRewardState(rewardTokens);", positive_text)
        self.assertIn("checkpoint.balance += amount;", positive_text)
        self.assertIn("for (uint256 i = 0; i < rewardTokens.length; ++i)", positive_text)

        self.assertIn("_updateExtraRewardState(rewardTokens);", clean_text)
        self.assertIn("checkpoint.rewardPerTokenPaid[token] = extraRewardPerToken[token];", clean_text)
        self.assertIn("checkpoint.balance += amount;", clean_text)
