...
PATTERN = "oz-governor-queue-replay-after-timelock-rotation"
DETECTOR = ROOT / "detectors" / "wave17" / "oz_governor_queue_replay_after_timelock_rotation.py"
REFERENCE = ROOT / "reference" / "patterns.dsl" / f"{PATTERN}.yaml"
FIXTURE_DIR = ROOT / "detectors" / "fixtures" / "oz_governor_queue_replay_after_timelock_rotation"
ALT_FIXTURE_DIR = ROOT / "detectors" / "fixtures" / "oz-governor-queue-replay-after-timelock-rotation"
...
class OzGovernorQueueReplayAfterTimelockRotationTest(unittest.TestCase):
    ...
    def test_detector_reference_and_fixture_scope_stay_aligned(self) -> None:
        ...
        self.assertIn("_hash_proposal_excludes_timelock", detector_text)
        self.assertIn("_QUEUE_INVALIDATION_RE", detector_text)
        ...
        self.assertIn("function hashProposal(", positive_text)
        self.assertIn("keccak256(abi.encode(targets, values, calldatas, descriptionHash))", positive_text)
        self.assertIn("function updateTimelock(ITimelockController newTimelock) public", positive_text)
        self.assertIn("_timelockIds[proposalId] = operationId;", positive_text)
        self.assertNotIn("delete _timelockIds", positive_text)

        self.assertIn("uint256[] private pendingProposalIds", clean_text)
        self.assertIn("_timelock.cancel(operationId);", clean_text)
        self.assertIn("delete _timelockIds[proposalId];", clean_text)
        ...
        self.assertIn("No corpus-backed exploit evidence", payload["limitation_note"])
