"""Tests for VaultQuery.vault_dupe_advisory_check callable (W3.8 coverage push).
...
Notes: degraded envelope when no fixtures
"""
...
class TestVaultDupeAdvisoryCheck(unittest.TestCase):

    def setUp(self):
        self.tmp = tempfile.TemporaryDirectory(prefix="w38-dupe_advisory_check-")
    ...
    def test_happy_path_returns_dict(self):
        # synthetic_fixture: true
        result = self.query.vault_dupe_advisory_check()
    ...
    def test_schema_or_error_field_present(self):
        ...
        has_known = any(k in result for k in ['schema', 'context_pack_id', 'context_pack_hash'])
        self.assertTrue(
    ...
    def test_cli_dispatch_exits_zero(self):
        ...
        proc = subprocess.run(
            [sys.executable, str(MODULE_PATH), "--repo-root", str(self.root),
             "--vault-dir", str(self.vault), "--call", "vault_dupe_advisory_check",
             "--args", args_json],
    ...
    def test_callable_in_tool_schemas(self):
        ...
        self.assertIn("vault_dupe_advisory_check", names)
