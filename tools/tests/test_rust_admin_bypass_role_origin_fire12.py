...
DETECTOR = "admin_bypass_role_origin_fire12"
...
class RustAdminBypassRoleOriginFire12Tests(unittest.TestCase):
    def test_positive_fixture_fires_on_origin_and_delegation_overwrite(self) -> None:
        hits, log_text = _run_fixture(POSITIVE)
        self.assertEqual(hits, 2, log_text)
        self.assertIn("set_config", log_text)
        self.assertIn("RuntimeConfig", log_text)
        self.assertIn("update_user_boost", log_text)
        self.assertIn("delegations", log_text)

    def test_negative_fixture_is_silent_on_real_guards_and_zero_address_only(self) -> None:
        hits, log_text = _run_fixture(NEGATIVE)
        self.assertEqual(hits, 0, log_text)

    def test_scan_file_contract_includes_detector_metadata(self) -> None:
