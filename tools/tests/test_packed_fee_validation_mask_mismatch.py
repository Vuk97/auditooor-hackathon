...
PATTERN = "packed-fee-validation-mask-mismatch"
DETECTOR = ROOT / "detectors" / "wave17" / "packed_fee_validation_mask_mismatch.py"
REFERENCE = ROOT / "reference" / "patterns.dsl" / f"{PATTERN}.yaml"
FIXTURE_DIR = ROOT / "detectors" / "fixtures" / "packed_fee_validation_mask_mismatch"
ALT_FIXTURE_DIR = ROOT / "detectors" / "fixtures" / "packed-fee-validation-mask-mismatch"
...
class PackedFeeValidationMaskMismatchTest(unittest.TestCase):
    ...
    def test_detector_reference_and_fixture_scope_stay_aligned(self) -> None:
        ...
        self.assertIn("_has_packed_fee_validation_mask_mismatch_shape", detector_text)
        self.assertIn("_TOO_WIDE_SELF_MASK_RE", detector_text)
        ...
        self.assertIn("function calculateSwapFee(", positive_text)
        self.assertIn("and(self, 0xffff)", positive_text)
        self.assertIn("and(shr(12, self), 0xfff)", positive_text)
        self.assertIn('assembly ("memory-safe")', positive_text)

        self.assertIn("function calculateSwapFee(", clean_text)
        self.assertIn("and(self, 0xfff)", clean_text)
        self.assertNotIn("and(self, 0xffff)", clean_text)
