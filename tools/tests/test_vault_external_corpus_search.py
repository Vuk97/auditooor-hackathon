"""Tests for VaultQuery.vault_external_corpus_search callable (W3.8 coverage push).
...
Verifies:
  ...
  3. Sample kwargs payload accepted: query='test_synth'.
...
Notes: always emits structured response
"""
...
class TestVaultExternalCorpusSearch(unittest.TestCase):
    ...
    def setUp(self):
        self.tmp = tempfile.TemporaryDirectory(prefix="w38-external_corpus_search-")
    ...
    def test_happy_path_returns_dict(self):
        # synthetic_fixture: true
        result = self.query.vault_external_corpus_search(query='test_synth')
        self.assertIsInstance(result, dict)
        # At least one of the expected keys must be present
        expected = ['schema', 'corpus', 'query', 'query_terms', 'results']
    ...
    def test_no_kwargs_does_not_raise(self):
        # synthetic_fixture: true
        result = self.query.vault_external_corpus_search()
        self.assertIsInstance(result, dict)
    ...
    def test_schema_or_error_field_present(self):
        ...
        has_known = any(k in result for k in ['schema', 'corpus', 'query', 'query_terms', 'results'])
        self.assertTrue(
    ...
    def test_cli_dispatch_exits_zero(self):
        # synthetic_fixture: true
        args_json = json.dumps({ "query": 'test_synth' })
        proc = subprocess.run(
            [sys.executable, str(MODULE_PATH), "--repo-root", str(self.root),
             "--vault-dir", str(self.vault), "--call", "vault_external_corpus_search",
             "--args", args_json],
    ...
    def test_callable_in_tool_schemas(self):
        ...
        self.assertIn("vault_external_corpus_search", names)
