...
PATTERN = "protocol-fee-withdraw-no-claimed-flag"
DETECTOR = ROOT / "detectors" / "wave17" / "protocol_fee_withdraw_no_claimed_flag.py"
REFERENCE = ROOT / "reference" / "patterns.dsl" / f"{PATTERN}.yaml"
FIXTURE_DIR = ROOT / "detectors" / "fixtures" / "protocol_fee_withdraw_no_claimed_flag"
...
class ProtocolFeeWithdrawNoClaimedFlagTest(unittest.TestCase):
    ...
    def test_pattern_compile_round_trip_matches_generated_detector(self) -> None:
        ...
        with tempfile.TemporaryDirectory(
            prefix=".pattern_compile_protocol_fee_withdraw_",
            dir=ROOT,
        ) as tmp:
    ...
    def test_reference_yaml_and_detector_capture_fixture_smoke_posture(self) -> None:
        detector_text = DETECTOR.read_text(encoding="utf-8")
        reference_text = REFERENCE.read_text(encoding="utf-8")

        self.assertIn(f'ARGUMENT = "{PATTERN}"', detector_text)
        self.assertIn("function.body_not_contains_regex", detector_text)
        self.assertIn("delete\\\\s+(protocolFee|accruedFee|feeBalance|accumulatedFees|feesCollected|_fees)", detector_text)
    ...
    def test_fixture_pair_and_smoke_records_stay_in_sync(self) -> None:
        positive_text = POSITIVE.read_text(encoding="utf-8")
        clean_text = CLEAN.read_text(encoding="utf-8")
        payload = json.loads(SMOKE.read_text(encoding="utf-8"))
        mirror_payload = json.loads(MIRROR_SMOKE.read_text(encoding="utf-8"))

        self.assertIn("contract ProtocolFeeVaultPositive", positive_text)
        self.assertIn("uint256 fee = accruedFee;", positive_text)
        self.assertIn("token.transfer(treasury, fee);", positive_text)
        self.assertNotIn("accruedFee = 0;", positive_text)

        self.assertIn("contract ProtocolFeeVaultClean", clean_text)
        self.assertIn("uint256 fee = accruedFee;", clean_text)
        self.assertIn("accruedFee = 0;", clean_text)
        self.assertIn("token.transfer(treasury, fee);", clean_text)
