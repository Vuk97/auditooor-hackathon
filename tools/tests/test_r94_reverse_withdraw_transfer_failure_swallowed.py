...
PATTERN = "r94-reverse-withdraw-transfer-failure-swallowed"
DETECTOR = ROOT / "detectors" / "wave17" / "r94_reverse_withdraw_transfer_failure_swallowed.py"
REFERENCE = ROOT / "reference" / "patterns.dsl" / f"{PATTERN}.yaml"
FIXTURE_DIR = ROOT / "detectors" / "fixtures" / "r94_reverse_withdraw_transfer_failure_swallowed"
...
class R94ReverseWithdrawTransferFailureSwallowedTest(unittest.TestCase):
    ...
    def test_detector_reference_and_fixture_scope_stay_aligned(self) -> None:
        ...
        self.assertIn("contract R94ReverseWithdrawTransferFailureSwallowedPositive", positive_text)
        self.assertIn("(bool success, ) = msg.sender.call{value: amount}(\"\");", positive_text)
        self.assertIn("success;", positive_text)
        self.assertIn("balances[msg.sender] -= amount;", positive_text)

        self.assertIn("contract R94ReverseWithdrawTransferFailureSwallowedClean", clean_text)
        self.assertIn("(bool success, ) = msg.sender.call{value: amount}(\"\");", clean_text)
        self.assertIn("require(success, \"transfer failed\");", clean_text)
