...
PATTERN = "r74-auth-cross-contract-signature-replay"
DETECTOR = ROOT / "detectors" / "wave17" / "r74_auth_cross_contract_signature_replay.py"
REFERENCE = ROOT / "reference" / "patterns.dsl" / f"{PATTERN}.yaml"
FIXTURE_DIR = ROOT / "detectors" / "fixtures" / "r74_auth_cross_contract_signature_replay"
...
class R74AuthCrossContractSignatureReplayTest(unittest.TestCase):
    ...
    def test_pattern_compile_round_trip_matches_generated_detector(self) -> None:
        ...
        with tempfile.TemporaryDirectory(prefix=".pattern_compile_r74_auth_", dir=ROOT) as tmp:
    ...
    def test_reference_and_fixture_metadata_stay_not_submit_ready(self) -> None:
        ...
        self.assertIn("_DOMAIN_SEPARATOR", positive_text)
        self.assertIn("verifier.recover(digest, signature)", positive_text)
        self.assertNotIn("block.chainid", positive_text)
        self.assertNotIn("address(this)", positive_text)

        self.assertIn("block.chainid", clean_text)
        self.assertIn("address(this)", clean_text)
        self.assertIn("verifier.recover(digest, signature)", clean_text)
