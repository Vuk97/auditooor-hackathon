...
PATTERN = "pmm-quote-reads-external-oracle-without-deviation-band-vs-reserves"
DETECTOR = ROOT / "detectors" / "wave17" / "pmm_quote_reads_external_oracle_without_deviation_band_vs_reserves.py"
REFERENCE = ROOT / "reference" / "patterns.dsl" / f"{PATTERN}.yaml"
FIXTURE_DIR = ROOT / "detectors" / "fixtures" / "pmm_quote_reads_external_oracle_without_deviation_band_vs_reserves"
...
class PmmQuoteReadsExternalOracleWithoutDeviationBandVsReservesTest(unittest.TestCase):
    ...
    def test_pattern_compile_round_trip_matches_generated_detector(self) -> None:
        ...
        with tempfile.TemporaryDirectory(
            prefix=".pattern_compile_pmm_quote_external_oracle_",
            dir=ROOT,
        ) as tmp:
    ...
    def test_detector_reference_and_fixture_metadata(self) -> None:
        ...
        self.assertIn("baseReserve|quoteReserve|baseBalance|quoteBalance|reserveRatio|poolRatio|pmmState", detector_text)
        self.assertIn("deviationBps|deviationBand|chainlinkFallback|secondaryOracle", detector_text)
        ...
        self.assertIn("contract WooStylePmmQuotePositive", positive_text)
        self.assertIn("function querySwap(address baseToken, uint256 baseAmount) external view", positive_text)
        self.assertIn("uint256 oraclePrice = oracle.price(baseToken);", positive_text)
        self.assertIn("uint256 reserveRatio = quoteReserve * 1e18 / baseReserve;", positive_text)
        self.assertNotIn("secondaryOracle", positive_text)

        self.assertIn("contract WooStylePmmQuoteClean", clean_text)
        self.assertIn("secondaryOracle.price(baseToken)", clean_text)
        self.assertIn("deviationBps", clean_text)
        self.assertIn("reserveRatio", clean_text)
        ...
        self.assertIn("single external oracle price", payload["limitation_note"])
