...
PATTERN = "perp-glv-cancel-fee-attributed-to-keeper-not-account"
DETECTOR = ROOT / "detectors" / "wave17" / "perp_glv_cancel_fee_attributed_to_keeper_not_account.py"
REFERENCE = ROOT / "reference" / "patterns.dsl" / f"{PATTERN}.yaml"
FIXTURE_DIR = ROOT / "detectors" / "fixtures" / "perp_glv_cancel_fee_attributed_to_keeper_not_account"
...
class PerpGlvCancelFeeAttributedToKeeperNotAccountTest(unittest.TestCase):
    ...
    def test_detector_reference_and_fixture_metadata(self) -> None:
        ...
        self.assertIn("_has_keeper_refund_misattribution_shape", detector_text)
        self.assertIn("_KEEPER_MSG_SENDER_RE", detector_text)
        ...
        self.assertIn("contract GlvHandlerPositive", positive_text)
        self.assertIn("reason: GlvDepositUtils.USER_INITIATED_CANCEL", positive_text)
        self.assertIn("keeper: msg.sender", positive_text)
        self.assertNotIn("keeper: deposit.account()", positive_text)

        self.assertIn("contract GlvHandlerClean", clean_text)
        self.assertIn("reason: GlvDepositUtils.USER_INITIATED_CANCEL", clean_text)
        self.assertIn("keeper: deposit.account()", clean_text)
        self.assertNotIn("keeper: msg.sender", clean_text)
