...
PATTERN = "perp-reallocate-depends-on-slot0-not-twap"
DETECTOR = ROOT / "detectors" / "wave17" / "perp_reallocate_depends_on_slot0_not_twap.py"
REFERENCE = ROOT / "reference" / "patterns.dsl" / f"{PATTERN}.yaml"
FIXTURE_DIR = ROOT / "detectors" / "fixtures" / "perp_reallocate_depends_on_slot0_not_twap"
...
class PerpReallocateDependsOnSlot0NotTwapTest(unittest.TestCase):
    ...
    def test_pattern_compile_round_trip_matches_generated_detector(self) -> None:
        ...
        with tempfile.TemporaryDirectory(
            prefix=".pattern_compile_perp_reallocate_slot0_",
            dir=ROOT,
        ) as tmp:
    ...
    def test_detector_reference_and_fixture_metadata(self) -> None:
        ...
        self.assertIn("Fixture-smoke/source-shape proof only", detector_text)
        self.assertIn("NOT_SUBMIT_READY", detector_text)
        self.assertIn("currentTick\\\\s*<\\\\s*tickLower", detector_text)
        self.assertIn("_repositionLiquidity", detector_text)
        ...
        self.assertIn("contract PerpRangeReallocatorPositive", positive_text)
        self.assertIn("pool.slot0()", positive_text)
        self.assertIn("bool outOfRange = currentTick < tickLower || currentTick > tickUpper;", positive_text)
        self.assertIn("_repositionLiquidity(currentTick);", positive_text)
        self.assertNotIn("OracleLibrary.consult", positive_text)

        self.assertIn("contract PerpRangeReallocatorClean", clean_text)
        self.assertIn("OracleLibrary.consult(pool, 600)", clean_text)
        self.assertIn("pool.observe(secondsAgos);", clean_text)
        self.assertNotIn("pool.slot0()", clean_text)
