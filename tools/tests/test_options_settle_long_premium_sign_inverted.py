...
PATTERN = "options-settle-long-premium-sign-inverted"
DETECTOR = ROOT / "detectors" / "wave17" / "options_settle_long_premium_sign_inverted.py"
REFERENCE = ROOT / "reference" / "patterns.dsl" / f"{PATTERN}.yaml"
FIXTURE_DIR = ROOT / "detectors" / "fixtures" / "options_settle_long_premium_sign_inverted"
ALT_FIXTURE_DIR = ROOT / "detectors" / "fixtures" / "options-settle-long-premium-sign-inverted"
...
class OptionsSettleLongPremiumSignInvertedTest(unittest.TestCase):
    ...
    def test_detector_reference_and_fixtures_capture_not_submit_ready_scope(self) -> None:
        ...
        self.assertIn("realizedPremia", detector_text)
        self.assertIn("exercise", detector_text)
        ...
        self.assertIn("function settleLongPremium(address longOwner, uint256 tokenId, uint256 legIndex) external", positive_text)
        self.assertIn("exercise(longOwner, 0, 0, 0, realizedPremia)", positive_text)
        self.assertNotIn("exercise(longOwner, 0, 0, 0, -realizedPremia)", positive_text)
        self.assertIn("function settleLongPremium(address longOwner, uint256 tokenId, uint256 legIndex) external", clean_text)
        self.assertIn("exercise(longOwner, 0, 0, 0, -realizedPremia)", clean_text)
