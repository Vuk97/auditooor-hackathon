...
FIXTURE_DIR = ROOT / "detectors" / "fixtures" / "r74_abi_quorum_lost_after_manual_value_set"
MIRROR_DIR = ROOT / "detectors" / "fixtures" / "r74-abi-quorum-lost-after-manual-value-set"
...
class R74AbiQuorumLostAfterManualValueSetTest(unittest.TestCase):
    ...
    def test_detector_reference_and_smoke_posture_stay_aligned(self) -> None:
        ...
        self.assertIn("function setQuorum(uint256 newQuorum) external", positive_text)
        self.assertNotIn("_validateQuorumBounds", positive_text)

        self.assertIn("function setQuorum(uint256 newQuorum) external", clean_text)
        self.assertIn("_validateQuorumBounds(newQuorum);", clean_text)
        self.assertIn("members.length * MIN_QUORUM_BPS", clean_text)
