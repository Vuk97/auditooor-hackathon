...
PATTERN = "r94-loop-init-race-admin-takeover"
DETECTOR = ROOT / "detectors" / "wave17" / "r94_loop_init_race_admin_takeover.py"
REFERENCE = ROOT / "reference" / "patterns.dsl" / f"{PATTERN}.yaml"
FIXTURE_DIR = ROOT / "detectors" / "fixtures" / "r94_loop_init_race_admin_takeover"
...
class R94LoopInitRaceAdminTakeoverTest(unittest.TestCase):
    ...
    def test_detector_reference_and_fixture_scope_stay_aligned(self) -> None:
        ...
        self.assertIn("initialize", detector_text)
        self.assertIn("onlyFactory|onlyDeployer|onlyCreator", detector_text)
        ...
        self.assertIn("contract InitRaceAdminTakeoverPositive", positive_text)
        self.assertIn("function initialize(address newOwner) external", positive_text)
        self.assertIn("require(!initialized", positive_text)
        self.assertNotIn("msg.sender == deployer", positive_text)

        self.assertIn("contract InitRaceAdminTakeoverClean", clean_text)
        self.assertIn("address public immutable deployer;", clean_text)
        self.assertIn("require(msg.sender == deployer", clean_text)
