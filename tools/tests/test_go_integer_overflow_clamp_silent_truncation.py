...
DETECTOR = ROOT / "detectors" / "go_wave1" / "go-integer-overflow-clamp-silent-truncation.py"
FIXTURE_DIR = ROOT / "detectors" / "go_wave1" / "test_fixtures"
PATTERN = "go-integer-overflow-clamp-silent-truncation"
...
class GoIntegerOverflowClampSilentTruncationTest(unittest.TestCase):
    def _hits(self, fixture: Path) -> tuple[int, str]:
        ...
        with tempfile.NamedTemporaryFile(prefix=".go_integer_overflow_clamp_", suffix=".log") as tmp:
            proc = subprocess.run(
                [
                    python_ast,
                    str(TOOL),
