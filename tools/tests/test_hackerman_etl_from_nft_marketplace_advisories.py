"""Unit tests for ``tools/hackerman-etl-from-nft-marketplace-advisories.py``.
...
* Map severity / impact / actor correctly for NFT / marketplace domains.
...
TOOL_PATH = REPO_ROOT / "tools" / "hackerman-etl-from-nft-marketplace-advisories.py"
VALIDATOR_PATH = REPO_ROOT / "tools" / "hackerman-record-validate.py"


def _load_tool():
    name = "_hackerman_etl_from_nft_marketplace_advisories"
...
def _load_validator():
    name = "_hackerman_record_validate_for_nft_marketplace_test"
...
def _sample_advisory(
    *,
    ...
    severity: str = "high",
    summary: str = "Order replay in Seaport allows NFT theft",
    description: str = (
        "Improper nonce handling on a Seaport order allows an unprivileged "
        "attacker to replay a previously-cancelled signed order and steal "
        "the underlying ERC-721 token."
    ),
    cve_id: str = "CVE-2024-99999",
    package_name: str = "seaport",
    patched_versions: str = ">=1.5.0",
    html_url: str = "https://github.com/ProjectOpenSea/seaport/security/advisories/GHSA-aaaa-bbbb-cccc",
...
class HackermanEtlFromNftMarketplaceTests(unittest.TestCase):
    @classmethod
    ...
    def test_target_repos_cover_required_set(self) -> None:
        ...
        required = {
            ...
            "looksrare/contracts-exchange-v2",
            "blur-io/blur-poolswap",
            "thirdweb-dev/contracts",
            "thirdweb-dev/typescript-sdk",
            ...
            "enjin/enjin-platform-pcg-pack-contract",
            "spcoin-utils/spcoin-rewards",
        }
        ...
        valid_langs = {"solidity", "typescript-onchain"}
        valid_domains = {"nft", "gaming", "rpc-infra"}
    ...
    def test_critical_severity_maps_dollar_class_and_impact_actor(self) -> None:
        adv = _sample_advisory(
            severity="critical",
            summary="NFT theft via order replay in Seaport",
            description=(
                "An attacker replays a previously-cancelled signed order "
                "and steals the underlying ERC-721 from the listing owner."
            ),
        )
        summary = self._run({"ProjectOpenSea/seaport": [adv]})
        ...
        # "order replay" precedes "theft" in keyword routing -> priv-esc.
        self.assertEqual(rec["impact_class"], "privilege-escalation")
        # NFT-domain priv-esc -> specific-user (the listing owner).
        self.assertEqual(rec["impact_actor"], "specific-user")
        self.assertEqual(rec["target_domain"], "nft")
        self.assertEqual(rec["target_language"], "solidity")

    def test_royalty_bypass_maps_yield_redistribution(self) -> None:
        adv = _sample_advisory(
            severity="high",
            summary="Royalty bypass in marketplace exchange contract",
            description=(
                "A crafted listing path skips the EIP-2981 royalty hook, "
                "redirecting royalty fees away from the original creator."
            ),
        )
        self._run({"looksrare/contracts-exchange-v2": [adv]})
        sub = next(p for p in self.out_dir.iterdir() if p.is_dir())
        rec = json.loads((sub / "record.json").read_text())
    ...
    def test_moderate_severity_maps_medium_with_precision_keyword(self) -> None:
        adv = _sample_advisory(
            severity="moderate",
            summary="Precision rounding in ERC721A batch-mint counter",
            description=(
                "Rounding in the batch-mint share counter produces off-by-one "
                "errors when computing token-id ranges across owners."
            ),
        )
        self._run({"ERC721A/ERC721A": [adv]})
        sub = next(p for p in self.out_dir.iterdir() if p.is_dir())
        rec = json.loads((sub / "record.json").read_text())
    ...
    def test_indexer_griefing_routes_arbitrary_user_on_rpc_infra(self) -> None:
        adv = _sample_advisory(
            severity="medium",
            summary="Indexer poisoning via crafted metadata",
            description=(
                "A crafted metadata payload triggers indexer poisoning, "
                "causing the marketplace UI to display attacker-controlled "
                "content for unrelated tokens."
            ),
        )
        self._run({"reservoirprotocol/indexer": [adv]})
        sub = next(p for p in self.out_dir.iterdir() if p.is_dir())
        rec = json.loads((sub / "record.json").read_text())
        self.assertEqual(rec["impact_class"], "griefing")
        # rpc-infra + griefing -> arbitrary-user
    ...
    def test_gaming_domain_for_enjin_pack_contract(self) -> None:
        adv = _sample_advisory(
            severity="high",
            summary="Reentrancy in pack-open path leaks user assets",
            description=(
                "A reentrancy in the in-game pack-open flow lets an attacker "
                "drain the assets staked inside a player's pack."
            ),
        )
        self._run({"enjin/enjin-platform-pcg-pack-contract": [adv]})
        sub = next(p for p in self.out_dir.iterdir() if p.is_dir())
        rec = json.loads((sub / "record.json").read_text())
        self.assertEqual(rec["target_domain"], "gaming")
        # gaming + theft (via reentrancy) -> specific-user (the player).
    ...
    def test_honest_zero_repos_reported_not_fabricated(self) -> None:
        ...
        cache = {"ProjectOpenSea/seaport": [_sample_advisory()]}
        ...
        self.assertNotIn("ProjectOpenSea/seaport",
                         summary["repos_with_zero_advisories"])

    def test_filter_repo_restricts_output(self) -> None:
        cache = _build_cache(
            repos=("ProjectOpenSea/seaport", "looksrare/contracts-exchange-v2"),
        )
        self._write_cache(cache)
        summary = self.tool.convert(
            self.out_dir,
            cache_file=self.cache_path,
            filter_repo="ProjectOpenSea/seaport",
            dry_run=True,
        )
    ...
    def test_state_filter_skips_unpublished_advisories(self) -> None:
        ...
        draft["state"] = "draft"
        self._run({"ProjectOpenSea/seaport": [published, draft]})
        sub_ids = [
    ...
    def test_slug_uses_double_underscore_separator(self) -> None:
        ...
        # Owner lowercased by slugify.
        self.assertTrue(subs[0].startswith("projectopensea__seaport__"),
    ...
    def test_dedupe_collapses_same_ghsa(self) -> None:
        ...
        summary = self._run({"ProjectOpenSea/seaport": [adv, adv]})
        self.assertEqual(summary["records_emitted"], 1)
        self.assertEqual(summary["errors"], [])
