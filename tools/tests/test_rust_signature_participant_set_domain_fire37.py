...
DETECTOR = "signature_participant_set_domain_fire37"
...
class RustSignatureParticipantSetDomainFire37Tests(unittest.TestCase):
    def test_detector_compiles(self) -> None:
        py_compile.compile(str(DETECTOR_PATH), doraise=True)

    def test_positive_fixture_fires_on_unbound_participant_scope(self) -> None:
        hits, log_text = _run_fixture(POSITIVE)
        self.assertGreaterEqual(hits, 1, log_text)
        self.assertIn("execute_frost_threshold_release", log_text)
        self.assertIn("signature-replay-cross-domain", log_text)
        self.assertIn("participant_set", log_text)
        self.assertIn("signer_role", log_text)
        self.assertIn("threshold", log_text)
        self.assertIn("session_id", log_text)
        self.assertIn("chain_id", log_text)
        self.assertIn("purpose_domain", log_text)

    def test_negative_fixture_is_silent_when_participant_scope_is_bound(self) -> None:
