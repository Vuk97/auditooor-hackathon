...
PATTERN = "misleading-argument-name"
DETECTOR = (
    ROOT
    / "detectors"
    / "wave_graveyard"
    / "wave14_broken"
    / "misleading_argument_name.py"
)
REFERENCE = ROOT / "reference" / "patterns.dsl" / f"{PATTERN}.yaml"
FIXTURE_DIR = ROOT / "detectors" / "fixtures" / "misleading_argument_name"
ALT_FIXTURE_DIR = ROOT / "detectors" / "fixtures" / "misleading-argument-name"
...
class MisleadingArgumentNameTest(unittest.TestCase):
    ...
    def test_detector_reference_and_fixture_scope_stay_aligned(self) -> None:
        ...
        self.assertIn("first parameter is named `factory`", detector_text)
        ...
        self.assertIn("function verifyCallback(", positive_text)
        self.assertIn("address factory,", positive_text)
        self.assertIn("getPoolKey(poolDeployer, tokenA, tokenB)", positive_text)

        self.assertIn("function verifyCallback(", clean_text)
        self.assertIn("address poolDeployer_,", clean_text)
        self.assertIn("getPoolKey(poolDeployer_, tokenA, tokenB)", clean_text)
        ...
        self.assertIn("visibly routes through `poolDeployer`", payload["limitation_note"])
