...
PATTERN = "r94-reverse-rewards-emission-rate-retroactive"
DETECTOR = ROOT / "detectors" / "wave17" / "r94_reverse_rewards_emission_rate_retroactive.py"
REFERENCE = ROOT / "reference" / "patterns.dsl" / f"{PATTERN}.yaml"
FIXTURE_DIR = ROOT / "detectors" / "fixtures" / "r94_reverse_rewards_emission_rate_retroactive"
...
class R94ReverseRewardsEmissionRateRetroactiveTest(unittest.TestCase):
    ...
    def test_detector_reference_and_fixture_scope_stay_aligned(self) -> None:
        ...
        self.assertIn("rate setters write the new rate", detector_text)
        self.assertIn("_updateRewardIndex", detector_text)
        ...
        self.assertIn("reward-rate setter shape", reference_text)

        self.assertIn("contract R94ReverseRewardsEmissionRateRetroactivePositive", positive_text)
        self.assertIn("function setRewardRate(uint256 newRate) external", positive_text)
        self.assertIn("rewardRate = newRate;", positive_text)
        self.assertNotIn("_updateRewardIndex();", positive_text)

        self.assertIn("contract R94ReverseRewardsEmissionRateRetroactiveClean", clean_text)
        self.assertIn("_updateRewardIndex();", clean_text)
        self.assertIn("rewardRate = newRate;", clean_text)
        ...
        self.assertIn("reward-rate setter shape", payload["limitation_note"])
