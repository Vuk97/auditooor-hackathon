"""
test_rust_wave1_anchor_owner_check_missing_on_authority.py

Tests for anchor_owner_check_missing_on_authority detector.

Run with:
    python3 -m unittest tools.tests.test_rust_wave1_anchor_owner_check_missing_on_authority
"""
...
DETECTOR_NAME = "anchor_owner_check_missing_on_authority"
...
class TestAnchorOwnerCheckMissingOnAuthority(unittest.TestCase):
    ...
    def test_fires_on_positive_1(self):
        """Fires on authority field typed as AccountInfo without Signer."""
    ...
    def test_fires_on_positive_2(self):
        """Fires on admin field as UncheckedAccount without constraint."""
        hits = self._scan_fixture(f"{DETECTOR_NAME}_positive_2.rs")
        self.assertGreaterEqual(len(hits), 1, f"Expected >=1 hit, got: {hits}")

    def test_silent_on_negative_1(self):
        """Silent when authority is Signer<'info>."""
    ...
    def test_silent_on_negative_2(self):
        """Silent when authority constrained via has_one."""
        hits = self._scan_fixture(f"{DETECTOR_NAME}_negative_2.rs")
        self.assertEqual(len(hits), 0, f"Expected 0 hits, got: {hits}")

    def test_scan_file_emits_field_name(self):
        """scan_file() hit includes field_name for diagnostic context."""
    ...
    def test_scan_file_emits_util_fields(self):
        ...
        self.assertTrue(has_util, f"Expected util-style field: {h}")
