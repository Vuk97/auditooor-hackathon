"""Tests for VaultQuery.vault_language_patterns callable (W3.8 coverage push).
...
Verifies:
  ...
  3. Sample kwargs payload accepted: language='solidity'.
...
class TestVaultLanguagePatterns(unittest.TestCase):
    ...
    def setUp(self):
        self.tmp = tempfile.TemporaryDirectory(prefix="w38-language_patterns-")
    ...
    def test_happy_path_returns_dict(self):
        # synthetic_fixture: true
        result = self.query.vault_language_patterns(language='solidity')
    ...
    def test_no_kwargs_does_not_raise(self):
        # synthetic_fixture: true
        result = self.query.vault_language_patterns()
        self.assertIsInstance(result, dict)
    ...
    def test_cli_dispatch_exits_zero(self):
        # synthetic_fixture: true
        args_json = json.dumps({ "language": 'solidity' })
        proc = subprocess.run(
            [sys.executable, str(MODULE_PATH), "--repo-root", str(self.root),
             "--vault-dir", str(self.vault), "--call", "vault_language_patterns",
             "--args", args_json],
