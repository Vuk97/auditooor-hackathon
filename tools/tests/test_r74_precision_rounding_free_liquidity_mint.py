...
PATTERN = "r74-precision-rounding-free-liquidity-mint"
DETECTOR = ROOT / "detectors" / "wave17" / "r74_precision_rounding_free_liquidity_mint.py"
REFERENCE = ROOT / "reference" / "patterns.dsl" / f"{PATTERN}.yaml"
FIXTURE_DIR = ROOT / "detectors" / "fixtures" / "r74_precision_rounding_free_liquidity_mint"
...
class R74PrecisionRoundingFreeLiquidityMintTest(unittest.TestCase):
    ...
    def test_pattern_compile_round_trip_matches_generated_detector(self) -> None:
        ...
        with tempfile.TemporaryDirectory(prefix=".pattern_compile_r74_precision_", dir=ROOT) as tmp:
    ...
    def test_reference_yaml_detector_and_fixture_metadata_stay_not_submit_ready(self) -> None:
        ...
        self.assertIn("ceilDiv", detector_text)
        ...
        self.assertIn("contract FreeLiquidityMintPositive", positive_text)
        self.assertIn("amount = shares * reserve0 / totalSupply;", positive_text)
        self.assertIn("totalSupply += shares;", positive_text)
        self.assertNotIn("ceilDiv", positive_text)

        self.assertIn("contract FreeLiquidityMintClean", clean_text)
        self.assertIn("amount = ceilDiv(shares * reserve0, totalSupply);", clean_text)
        self.assertIn('require(amount > 0, "zero input");', clean_text)
