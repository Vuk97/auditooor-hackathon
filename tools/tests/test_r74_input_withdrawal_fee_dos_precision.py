...
PATTERN = "r74-input-withdrawal-fee-dos-precision"
DETECTOR = ROOT / "detectors" / "wave17" / "r74_input_withdrawal_fee_dos_precision.py"
REFERENCE = ROOT / "reference" / "patterns.dsl" / f"{PATTERN}.yaml"
FIXTURE_DIR = ROOT / "detectors" / "fixtures" / "r74_input_withdrawal_fee_dos_precision"
...
class R74InputWithdrawalFeeDosPrecisionTest(unittest.TestCase):
    ...
    def test_pattern_compile_round_trip_matches_generated_detector(self) -> None:
        ...
        with tempfile.TemporaryDirectory(prefix=".pattern_compile_r74_input_withdraw_", dir=ROOT) as tmp:
    ...
    def test_reference_yaml_detector_and_fixture_metadata_stay_not_submit_ready(self) -> None:
        ...
        self.assertIn("mulDivUp|mulDivRoundingUp|ceilDiv|roundUp", detector_text)
        ...
        self.assertIn("contract DustWithdrawalFeeDosPositive", positive_text)
        self.assertIn("uint256 fee = assets * withdrawalFeeBps / BPS;", positive_text)
        self.assertIn("protocolFeeDebt += assets * withdrawalFeeBps / BPS;", positive_text)
        self.assertNotIn("ceilDiv", positive_text)

        self.assertIn("contract DustWithdrawalFeeDosClean", clean_text)
        self.assertIn("uint256 fee = ceilDiv(assets * withdrawalFeeBps, BPS);", clean_text)
        self.assertIn("protocolFeeDebt += fee;", clean_text)
        self.assertIn("function ceilDiv(uint256 x, uint256 y)", clean_text)
