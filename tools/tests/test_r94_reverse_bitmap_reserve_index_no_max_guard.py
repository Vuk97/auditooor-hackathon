...
PATTERN = "r94-reverse-bitmap-reserve-index-no-max-guard"
DETECTOR = ROOT / "detectors" / "wave17" / "r94_reverse_bitmap_reserve_index_no_max_guard.py"
REFERENCE = ROOT / "reference" / "patterns.dsl" / f"{PATTERN}.yaml"
FIXTURE_DIR = ROOT / "detectors" / "fixtures" / "r94_reverse_bitmap_reserve_index_no_max_guard"
MIRROR_DIR = ROOT / "detectors" / "fixtures" / "r94-reverse-bitmap-reserve-index-no-max-guard"
...
class R94ReverseBitmapReserveIndexNoMaxGuardTest(unittest.TestCase):
    ...
    def test_detector_reference_and_fixture_scope_stay_aligned(self) -> None:
        ...
        self.assertIn("MAX_RESERVES", detector_text)
        self.assertIn("reserveIndex << 1", detector_text)
        ...
        self.assertIn("two-bits-per-reserve layout", reference_text)

        self.assertIn("contract UserConfigurationPositive", positive_text)
        self.assertIn("uint256 bit = uint256(1) << (reserveIndex << 1);", positive_text)
        self.assertNotIn("MAX_RESERVES", positive_text)

        self.assertIn("contract UserConfigurationClean", clean_text)
        self.assertIn("uint256 internal constant MAX_RESERVES = 64;", clean_text)
        self.assertIn('require(reserveIndex < MAX_RESERVES, "invalid reserve");', clean_text)
        ...
        self.assertIn("MAX_RESERVES guard", payload["limitation_note"])
    ...
    def test_hyphenated_fixture_mirror_stays_in_sync(self) -> None:
        ...
        self.assertEqual(mirror["fixture_id"], "r94-reverse-bitmap-reserve-index-no-max-guard")
