...
DETECTOR = "signature_domain_replay_fire36"
...
class RustSignatureDomainReplayFire36Tests(unittest.TestCase):
    def test_detector_compiles(self) -> None:
        py_compile.compile(str(DETECTOR_PATH), doraise=True)

    def test_positive_fixture_fires_on_unbound_transcript_scope(self) -> None:
        hits, log_text = _run_fixture(POSITIVE)
        self.assertGreaterEqual(hits, 1, log_text)
        self.assertIn("execute_frost_authorization", log_text)
        self.assertIn("signature-replay-cross-domain", log_text)
        self.assertIn("chain_id", log_text)
        self.assertIn("domain", log_text)
        self.assertIn("session", log_text)
        self.assertIn("purpose", log_text)
        self.assertIn("participant_set", log_text)

    def test_negative_fixture_is_silent_when_scope_is_bound(self) -> None:
