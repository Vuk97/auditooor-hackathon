...
PATTERN = "bridge-replay-key-omits-chain-domain"
DETECTOR = ROOT / "detectors" / "wave17" / "bridge_replay_key_omits_chain_domain.py"
REFERENCE = ROOT / "reference" / "patterns.dsl" / f"{PATTERN}.yaml"
FIXTURE_DIR = ROOT / "detectors" / "fixtures" / "bridge_replay_key_omits_chain_domain"
...
class BridgeReplayKeyOmitsChainDomainTest(unittest.TestCase):
    ...
    def test_pattern_compile_round_trip_matches_generated_detector(self) -> None:
        ...
        with tempfile.TemporaryDirectory(
            prefix=".pattern_compile_bridge_replay_key_",
            dir=ROOT,
        ) as tmp:
    ...
    def test_detector_reference_and_fixture_metadata(self) -> None:
        ...
        self.assertIn("Bridge replay/proof key stores consumed/processed state", detector_text)
        self.assertIn("source|src|origin|from|remote|destination|dest|dst|target|local", detector_text)
        self.assertIn("consumed|processed|used|spent|seen", detector_text)
        ...
        self.assertIn("Bridge receivers and proof consumers", reference_text)

        self.assertIn("bytes32 key = keccak256(abi.encode(sender, nonce, payload));", positive_text)
        self.assertIn("processedMessages[key] = true;", positive_text)
        self.assertNotIn("address(this)", positive_text)
        self.assertNotIn("destinationDomain == uint32(block.chainid)", positive_text)

        self.assertIn("destinationDomain == uint32(block.chainid)", clean_text)
        self.assertIn("abi.encode(sourceDomain, destinationDomain, address(this), sender, nonce, payload)", clean_text)
        self.assertIn("processedMessages[key] = true;", clean_text)
