...
PATTERN = "no-mechanism-to-remove-backing-tokens"
DETECTOR = (
    ROOT
    / "detectors"
    / "wave_graveyard"
    / "syntax_broken"
    / "no_mechanism_to_remove_backing_tokens.py"
)
REFERENCE = ROOT / "reference" / "patterns.dsl" / f"{PATTERN}.yaml"
FIXTURE_DIR = ROOT / "detectors" / "fixtures" / "no_mechanism_to_remove_backing_tokens"
ALT_FIXTURE_DIR = ROOT / "detectors" / "fixtures" / "no-mechanism-to-remove-backing-tokens"
...
class NoMechanismToRemoveBackingTokensTest(unittest.TestCase):
    ...
    def test_detector_reference_and_fixture_scope_stay_aligned(self) -> None:
        ...
        self.assertIn("addBackingToken(address _token, address _oracle)", detector_text)
        self.assertIn("_REMOVE_FUNCTION_NAME", detector_text)
        self.assertIn("_UPDATE_FUNCTION_NAME", detector_text)
        ...
        self.assertIn("source-shape proof only", reference_text)

        self.assertIn("function addBackingToken(address _token, address _oracle)", positive_text)
        self.assertIn("backingTokens.push(_token);", positive_text)
        self.assertIn("oracle: _oracle", positive_text)
        self.assertNotIn("function removeBackingToken(address _token)", positive_text)
        self.assertNotIn("function updateBackingTokenOracle(address _token, address _oracle)", positive_text)

        self.assertIn("function removeBackingToken(address _token)", clean_text)
        self.assertIn("backingTokens[i] = backingTokens[backingTokens.length - 1];", clean_text)
        self.assertIn("backingTokens.pop();", clean_text)
        self.assertIn("delete backingTokenDetailsForAddress[_token];", clean_text)
        self.assertIn("backingTokenDetailsForAddress[_token].oracle = _oracle;", clean_text)
        ...
        self.assertIn("removeBackingToken", payload["limitation_note"])
        self.assertIn("updateBackingTokenOracle", payload["limitation_note"])
