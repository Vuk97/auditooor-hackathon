...
PATTERN = "oracle-deviation-check-uses-mutable-cached-baseline"
DETECTOR = ROOT / "detectors" / "wave17" / "oracle_deviation_check_uses_mutable_cached_baseline.py"
REFERENCE = ROOT / "reference" / "patterns.dsl" / f"{PATTERN}.yaml"
FIXTURE_DIR = ROOT / "detectors" / "fixtures" / "oracle_deviation_check_uses_mutable_cached_baseline"
...
class OracleDeviationCheckUsesMutableCachedBaselineTest(unittest.TestCase):
    ...
    def test_pattern_compile_round_trip_matches_generated_detector(self) -> None:
        ...
        with tempfile.TemporaryDirectory(prefix=".pattern_compile_mutable_cached_oracle_", dir=ROOT) as tmp:
    ...
    def test_detector_reference_and_fixture_text(self) -> None:
        ...
        self.assertIn("cachedPrice|lastPrice|priceCache|referencePrice|baselinePrice|pegPrice", detector_text)
        ...
        self.assertIn("contract MutableCachedBaselinePositive", positive_text)
        self.assertIn("deviation = currentPrice > cachedPrice", positive_text)
        self.assertIn("cachedPrice = currentPrice;", positive_text)

        self.assertIn("contract MutableCachedBaselineClean", clean_text)
        self.assertIn("deviation = currentPrice > pegAnchorPrice", clean_text)
        self.assertIn("lastObservedPrice = currentPrice;", clean_text)
