"""Tests for VaultQuery.vault_bug_family_heatmap callable (W3.8 coverage push).
...
class TestVaultBugFamilyHeatmap(unittest.TestCase):
    ...
    def setUp(self):
        self.tmp = tempfile.TemporaryDirectory(prefix="w38-bug_family_heatmap-")
    ...
    def test_happy_path_returns_dict(self):
        # synthetic_fixture: true
        result = self.query.vault_bug_family_heatmap()
    ...
    def test_cli_dispatch_exits_zero(self):
        ...
        proc = subprocess.run(
            [sys.executable, str(MODULE_PATH), "--repo-root", str(self.root),
             "--vault-dir", str(self.vault), "--call", "vault_bug_family_heatmap",
             "--args", args_json],
