...
PATTERN = "referrer-code-transfers-overwrite-recipient-codes-and-misali-x"
DETECTOR = ROOT / "detectors" / "wave17" / "referrer_code_transfers_overwrite_recipient_codes_and_misali_x.py"
REFERENCE = ROOT / "reference" / "patterns.dsl" / f"{PATTERN}.yaml"
FIXTURE_DIR = ROOT / "detectors" / "fixtures" / "referrer_code_transfers_overwrite_recipient_codes_and_misali_x"
...
class ReferrerCodeTransfersOverwriteRecipientCodesAndMisaliXTest(unittest.TestCase):
    ...
    def test_detector_reference_fixture_and_smoke_metadata(self) -> None:
        ...
        self.assertIn("_CODE_OWNER_WRITE_TEMPLATE", detector_text)
        self.assertIn("_RECIPIENT_CODE_WRITE_TEMPLATE", detector_text)
        self.assertIn("_SENDER_CODE_DELETE_REGEX", detector_text)
        self.assertIn("_RECIPIENT_CODE_GUARD_TEMPLATE", detector_text)
        ...
        self.assertIn("function setCodeOwner(bytes32 _code, address _newAccount) external", positive_text)
        self.assertIn("codeOwners[_code] = _newAccount;", positive_text)
        self.assertIn("delete codes[account];", positive_text)
        self.assertIn("codes[_newAccount] = _code;", positive_text)
        self.assertNotIn("recipient already has code", positive_text)
        self.assertNotIn("referrerTiers[_newAccount] = 1;", positive_text)

        self.assertIn("function setCodeOwner(bytes32 _code, address _newAccount) external", clean_text)
        self.assertIn('require(codes[_newAccount] == bytes32(0), "recipient already has code");', clean_text)
        self.assertIn("referrerTiers[account] = 0;", clean_text)
        self.assertIn("referrerTiers[_newAccount] = 1;", clean_text)
