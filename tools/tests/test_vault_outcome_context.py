"""Tests for VaultQuery.vault_outcome_context callable (W3.8 coverage push).
...
Verifies:
  ...
  3. Sample kwargs payload accepted: workspace='x', platform='cantina'.
...
Notes: always returns rows[] list, never raises
"""
...
class TestVaultOutcomeContext(unittest.TestCase):

    def setUp(self):
        self.tmp = tempfile.TemporaryDirectory(prefix="w38-outcome_context-")
    ...
    def test_happy_path_returns_dict(self):
        # synthetic_fixture: true
        result = self.query.vault_outcome_context(workspace='x', platform='cantina')
        self.assertIsInstance(result, dict)
        # At least one of the expected keys must be present
        expected = ['workspace', 'platform', 'limit', 'rows', 'policy']
    ...
    def test_no_kwargs_does_not_raise(self):
        # synthetic_fixture: true
        result = self.query.vault_outcome_context()
        self.assertIsInstance(result, dict)
    ...
    def test_schema_or_error_field_present(self):
        ...
        has_known = any(k in result for k in ['workspace', 'platform', 'limit', 'rows', 'policy'])
        self.assertTrue(
    ...
    def test_cli_dispatch_exits_zero(self):
        # synthetic_fixture: true
        args_json = json.dumps({ "workspace": 'x', "platform": 'cantina' })
        proc = subprocess.run(
            [sys.executable, str(MODULE_PATH), "--repo-root", str(self.root),
             "--vault-dir", str(self.vault), "--call", "vault_outcome_context",
             "--args", args_json],
    ...
    def test_callable_in_tool_schemas(self):
        ...
        self.assertIn("vault_outcome_context", names)
