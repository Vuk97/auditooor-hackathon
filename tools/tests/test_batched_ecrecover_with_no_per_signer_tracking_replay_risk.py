...
PATTERN = "batched-ecrecover-with-no-per-signer-tracking-replay-risk"
REFERENCE = ROOT / "reference" / "patterns.dsl" / f"{PATTERN}.yaml"
DETECTOR = ROOT / "detectors" / "wave17" / "batched_ecrecover_with_no_per_signer_tracking_replay_risk.py"
FIXTURE_DIR = ROOT / "detectors" / "fixtures" / "batched_ecrecover_with_no_per_signer_tracking_replay_risk"
...
class BatchedEcrecoverWithNoPerSignerTrackingReplayRiskTest(unittest.TestCase):
    ...
    def test_pattern_compile_round_trip_matches_generated_detector(self) -> None:
        ...
        with tempfile.TemporaryDirectory(
            prefix=".pattern_compile_batched_ecrecover_",
            dir=ROOT,
        ) as tmp:
    ...
    def test_reference_yaml_points_at_owned_fixture_pair(self) -> None:
        ...
        self.assertIn(
            "vuln: detectors/fixtures/batched_ecrecover_with_no_per_signer_tracking_replay_risk/positive.sol",
            text,
        )
        self.assertIn(
            "clean: detectors/fixtures/batched_ecrecover_with_no_per_signer_tracking_replay_risk/clean.sol",
            text,
        )
