...
DETECTOR = "signature_transcript_domain_fire34"
...
class RustSignatureTranscriptDomainFire34Tests(unittest.TestCase):
    def test_detector_compiles(self) -> None:
        py_compile.compile(str(DETECTOR_PATH), doraise=True)

    def test_positive_fixture_fires_on_replayable_transcript_omission(self) -> None:
        hits, log_text = _run_fixture(POSITIVE)
        self.assertGreaterEqual(hits, 1, log_text)
        self.assertIn("execute_signed_permit_claim", log_text)
        self.assertIn("signed digest omits", log_text)
        self.assertIn("chain_id", log_text)
        self.assertIn("domain_separator", log_text)
        self.assertIn("contract_id", log_text)
        self.assertIn("nonce", log_text)
        self.assertIn("purpose", log_text)

    def test_negative_fixture_is_silent_when_transcript_binds_scope(self) -> None:
