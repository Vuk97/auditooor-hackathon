...
PATTERN = "one-account-can-register-multiple-referral-codes"
DETECTOR = (
    ROOT
    / "detectors"
    / "wave_graveyard"
    / "syntax_broken"
    / "one_account_can_register_multiple_referral_codes.py"
)
...
FIXTURE_DIR = (
    ...
    / "detectors"
    / "fixtures"
    / "one_account_can_register_multiple_referral_codes"
)
ALT_FIXTURE_DIR = (
    ROOT
    / "detectors"
...
class OneAccountCanRegisterMultipleReferralCodesTest(unittest.TestCase):
    ...
    def test_detector_reference_and_fixture_scope_stay_aligned(self) -> None:
        ...
        self.assertIn("codeOwners[_code] = msg.sender", detector_text)
        ...
        self.assertIn("accountCode[msg.sender]", reference_text)

        self.assertIn("function registerCode(bytes32 _code) external", positive_text)
        self.assertIn("require(codeOwners[_code] == address(0)", positive_text)
        self.assertNotIn("accountCode[msg.sender] == bytes32(0)", positive_text)

        self.assertIn("function registerCode(bytes32 _code) external", clean_text)
        self.assertIn("require(accountCode[msg.sender] == bytes32(0)", clean_text)
        self.assertIn("accountCode[msg.sender] = _code;", clean_text)
        ...
        self.assertIn("code uniqueness", payload["limitation_note"])
