...
PATTERN = "oz-multicall-value-splitting-lets-single-payment-authorize-many"
DETECTOR = (
    ROOT
    / "detectors"
    / "wave17"
    / "oz_multicall_value_splitting_lets_single_payment_authorize_many.py"
)
...
FIXTURE_DIR = (
    ...
    / "detectors"
    / "fixtures"
    / "oz_multicall_value_splitting_lets_single_payment_authorize_many"
)
ALT_FIXTURE_DIR = (
    ROOT
    / "detectors"
...
class OzMulticallValueSplittingLetsSinglePaymentAuthorizeManyTest(unittest.TestCase):
    ...
    def test_detector_reference_and_fixture_scope_stay_aligned(self) -> None:
        ...
        self.assertIn("seatAuthorized", detector_text)
        self.assertIn("accountedBalance", detector_text)
        ...
        self.assertIn("contract SinglePaymentTicketSale is Multicall", positive_text)
        self.assertIn('require(msg.value >= pricePerSeat, "price");', positive_text)
        self.assertIn("seatAuthorized[seatId] = true;", positive_text)

        self.assertIn("contract SinglePaymentTicketSaleSafe is Multicall", clean_text)
        self.assertIn(
            'require(address(this).balance >= accountedBalance + pricePerSeat, "price");',
            clean_text,
        )
        self.assertIn("accountedBalance += pricePerSeat;", clean_text)
        self.assertNotIn('require(msg.value >= pricePerSeat, "price");', clean_text)
        ...
        self.assertIn("pricePerSeat", payload["limitation_note"])
