...
PATTERN = "r74-oracle-median-reverts-on-single-feed"
DETECTOR = ROOT / "detectors" / "wave17" / "r74_oracle_median_reverts_on_single_feed.py"
REFERENCE = ROOT / "reference" / "patterns.dsl" / f"{PATTERN}.yaml"
FIXTURE_DIR = ROOT / "detectors" / "fixtures" / "r74_oracle_median_reverts_on_single_feed"
...
class R74OracleMedianRevertsOnSingleFeedTest(unittest.TestCase):
    ...
    def test_pattern_compile_round_trip_matches_generated_detector(self) -> None:
        ...
        with tempfile.TemporaryDirectory(
            prefix=".pattern_compile_r74_oracle_median_reverts_",
            dir=ROOT,
        ) as tmp:
    ...
    def test_detector_reference_and_fixture_metadata(self) -> None:
        ...
        self.assertIn("try\\\\s+\\\\w+\\\\.latestRoundData", detector_text)
        self.assertIn("latestRoundData\\\\s*\\\\(", detector_text)
        ...
        self.assertIn("contract OracleMedianSingleFeedPositive", positive_text)
        self.assertIn("MedianFeed[] public priceFeeds;", positive_text)
        self.assertIn("priceFeeds[i].latestRoundData()", positive_text)
        self.assertNotIn("try priceFeeds[i].latestRoundData()", positive_text)

        self.assertIn("contract OracleMedianSingleFeedClean", clean_text)
        self.assertIn("try priceFeeds[i].latestRoundData()", clean_text)
        self.assertIn('require(successes >= minSuccessfulFeeds, "insufficient live feeds");', clean_text)
        self.assertIn("continue;", clean_text)
        ...
        self.assertGreaterEqual(mirror_payload["positive_hits"], 1)
