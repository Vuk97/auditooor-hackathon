...
PATTERN = "r94-loop-zk-missing-constraint"
DETECTOR = ROOT / "detectors" / "wave17" / "r94_loop_zk_missing_constraint.py"
REFERENCE = ROOT / "reference" / "patterns.dsl" / f"{PATTERN}.yaml"
FIXTURE_DIR = ROOT / "detectors" / "fixtures" / "r94_loop_zk_missing_constraint"
...
class R94LoopZkMissingConstraintTest(unittest.TestCase):
    ...
    def test_detector_reference_and_fixture_scope_stay_aligned(self) -> None:
        ...
        self.assertIn("proverSupplied", detector_text)
        ...
        self.assertIn("contract R94LoopZkMissingConstraintVerifierPositive", positive_text)
        self.assertIn("function verifyProof(", positive_text)
        self.assertIn("uint256 proverSuppliedOpening", positive_text)
        self.assertIn("uint256 logBlowup", positive_text)
        self.assertIn("uint256 alphaPows = proverSuppliedOpening * (logBlowup + 1);", positive_text)
        self.assertNotIn("require(proverSuppliedOpening", positive_text)

        self.assertIn("contract R94LoopZkMissingConstraintVerifierClean", clean_text)
        self.assertIn('require(proverSuppliedOpening < FIELD_MODULUS, "opening");', clean_text)
        self.assertIn('require(logBlowup <= 4, "blowup");', clean_text)
        self.assertIn("uint256 alphaPows = proverSuppliedOpening * (logBlowup + 1);", clean_text)
        ...
        self.assertIn("prover-shaped input", payload["limitation_note"])
