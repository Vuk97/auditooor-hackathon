...
PATTERN = "remove-redundant-code-in-cashin-proof-of-concept-take-a-look-at"
DETECTOR = ROOT / "detectors" / "wave17" / "remove_redundant_code_in_cashin_proof_of_concept_take_a_look_at.py"
REFERENCE = ROOT / "reference" / "patterns.dsl" / f"{PATTERN}.yaml"
FIXTURE_DIR = ROOT / "detectors" / "fixtures" / "remove_redundant_code_in_cashin_proof_of_concept_take_a_look_at"
...
class RemoveRedundantCodeInCashinProofOfConceptTakeALookAtTest(unittest.TestCase):
    ...
    def test_detector_reference_fixture_and_smoke_metadata(self) -> None:
        ...
        self.assertIn("_FN_NAME_REGEX", detector_text)
        self.assertIn("_WRITE_VAR_REGEX", detector_text)
        self.assertIn("_GUARD_VAR_REGEX", detector_text)
        ...
        self.assertIn("function cashIn(uint256 amount) external", positive_text)
        self.assertIn("balance = amount;", positive_text)
        self.assertNotIn("require(amount <=", positive_text)

        self.assertIn("function cashIn(uint256 amount) external", clean_text)
        self.assertIn('require(amount <= totalSupplyCap, "amount cap");', clean_text)
