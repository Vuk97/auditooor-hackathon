...
PATTERN = "perp-baddebt-transfer-from-router-not-liquidator"
DETECTOR = ROOT / "detectors" / "wave17" / "perp_baddebt_transfer_from_router_not_liquidator.py"
REFERENCE = ROOT / "reference" / "patterns.dsl" / f"{PATTERN}.yaml"
FIXTURE_DIR = ROOT / "detectors" / "fixtures" / "perp_baddebt_transfer_from_router_not_liquidator"
...
class PerpBaddebtTransferFromRouterNotLiquidatorTest(unittest.TestCase):
    ...
    def test_detector_reference_and_fixture_metadata(self) -> None:
        ...
        self.assertIn("_has_router_baddebt_transfer_shape", detector_text)
        self.assertIn("_TRANSFER_FROM_MSG_SENDER_RE", detector_text)
        ...
        self.assertIn("function executeLiquidation(int256 remainingMargin)", positive_text)
        self.assertIn("safeTransferFrom(msg.sender, address(this), uint256(-remainingMargin));", positive_text)
        self.assertNotIn("safeTransferFrom(liquidator", positive_text)

        self.assertIn("function executeLiquidation(address liquidator, int256 remainingMargin)", clean_text)
        self.assertIn("safeTransferFrom(liquidator, address(this), uint256(-remainingMargin));", clean_text)
        self.assertNotIn("safeTransferFrom(msg.sender", clean_text)
