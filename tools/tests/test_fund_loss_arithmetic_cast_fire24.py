...
PATTERN = "fund-loss-arithmetic-cast-fire24"
DETECTOR = ROOT / "detectors" / "wave17" / "fund_loss_arithmetic_cast_fire24.py"
POSITIVE = (
    ROOT
    / "detectors"
    / "test_fixtures"
    / "positive"
    / "fund_loss_arithmetic_cast_fire24.sol"
)
NEGATIVE = (
    ROOT
    / "detectors"
...
class FundLossArithmeticCastFire24Test(unittest.TestCase):
    ...
    def test_detector_and_fixture_sources_are_aligned(self) -> None:
        ...
        self.assertIn("cast-after-value-math", detector_text)
        self.assertIn("signed-to-unsigned-value-cast", detector_text)
        self.assertIn("self-referencing-value-accumulator", detector_text)
        self.assertIn("NOT_SUBMIT_READY", detector_text)
        self.assertIn("PROMOTION_ALLOWED = False", detector_text)

        self.assertIn("uint128 credited = uint128(netAmount);", positive_text)
        self.assertIn("uint128 credit = uint128(rewardDelta);", positive_text)
        self.assertIn("rewardIndex += rewardIndex + rewardDelta;", positive_text)

        self.assertIn("uint128 credited = netAmount.toUint128();", negative_text)
        self.assertIn("uint128 credit = rewardDelta.signedToUint128();", negative_text)
        self.assertIn("rewardIndex = indexSnapshot + rewardDelta;", negative_text)
        self.assertIn("balanceSnapshot += balanceSnapshot + delta;", negative_text)
