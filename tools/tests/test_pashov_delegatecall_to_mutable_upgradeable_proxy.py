...
PATTERN = "pashov-delegatecall-to-mutable-upgradeable-proxy"
DETECTOR = ROOT / "detectors" / "wave17" / "pashov_delegatecall_to_mutable_upgradeable_proxy.py"
REFERENCE = ROOT / "reference" / "patterns.dsl" / f"{PATTERN}.yaml"
FIXTURE_DIR = ROOT / "detectors" / "fixtures" / "pashov_delegatecall_to_mutable_upgradeable_proxy"
ALT_FIXTURE_DIR = ROOT / "detectors" / "fixtures" / "pashov-delegatecall-to-mutable-upgradeable-proxy"
...
class PashovDelegatecallToMutableUpgradeableProxyTest(unittest.TestCase):
    ...
    def test_detector_reference_and_fixture_scope_stay_aligned(self) -> None:
        ...
        self.assertIn("_DELEGATECALL_RE", detector_text)
        self.assertIn("_MUTABLE_PROXY_SETTER_RE", detector_text)
        ...
        self.assertIn("function setRewardsProxy(address newRewardsProxy)", positive_text)
        self.assertIn("_rewardsProxy.delegatecall", positive_text)
        self.assertIn("upgradeToAndCall", positive_text)

        self.assertIn("address public immutable rewardsProxy;", clean_text)
        self.assertIn("rewardsProxy.call", clean_text)
        self.assertNotIn("delegatecall", clean_text)
        ...
        self.assertIn("mutable `_rewardsProxy`", payload["limitation_note"])
