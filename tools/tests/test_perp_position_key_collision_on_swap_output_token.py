...
PATTERN = "perp-position-key-collision-on-swap-output-token"
DETECTOR = ROOT / "detectors" / "wave17" / "perp_position_key_collision_on_swap_output_token.py"
REFERENCE = ROOT / "reference" / "patterns.dsl" / f"{PATTERN}.yaml"
FIXTURE_DIR = ROOT / "detectors" / "fixtures" / "perp_position_key_collision_on_swap_output_token"
...
class PerpPositionKeyCollisionOnSwapOutputTokenTest(unittest.TestCase):
    ...
    def test_detector_reference_and_fixture_metadata(self) -> None:
        ...
        self.assertIn("Position\\\\.getPositionKey", detector_text)
        self.assertIn("order.initialCollateralToken", detector_text)
        self.assertIn("SwapUtils", detector_text)
        ...
        self.assertIn("contract PerpPositionKeyCollisionPositive", positive_text)
        self.assertIn("Position.getPositionKey", positive_text)
        self.assertIn("order.initialCollateralToken()", positive_text)
        self.assertIn("positionLastSrcChainId[positionKey]", positive_text)
        self.assertNotIn("SwapUtils.getOutputToken", positive_text)

        self.assertIn("contract PerpPositionKeyCollisionClean", clean_text)
        self.assertIn("SwapUtils.getOutputToken", clean_text)
        self.assertIn("order.swapPath()", clean_text)
        self.assertIn("order.initialCollateralToken()", clean_text)
