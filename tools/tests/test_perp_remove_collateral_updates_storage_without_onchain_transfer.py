...
PATTERN = "perp-remove-collateral-updates-storage-without-onchain-transfer"
DETECTOR = ROOT / "detectors" / "wave17" / "perp_remove_collateral_updates_storage_without_onchain_transfer.py"
REFERENCE = ROOT / "reference" / "patterns.dsl" / f"{PATTERN}.yaml"
FIXTURE_DIR = ROOT / "detectors" / "fixtures" / "perp_remove_collateral_updates_storage_without_onchain_transfer"
...
class PerpRemoveCollateralUpdatesStorageWithoutOnchainTransferTest(unittest.TestCase):
    ...
    def test_detector_reference_and_fixture_metadata(self) -> None:
        ...
        self.assertIn(r"(usedFunds|totalCollateral|positionData\\.totalCollateral|collateralAmount)\\s*-=", detector_text)
        self.assertNotIn("collateralAmount)\\\\s*-=|", detector_text)
        ...
        self.assertIn("contract KangarooVaultPositive", positive_text)
        self.assertIn("function removeCollateral(uint256 amount) external", positive_text)
        self.assertIn("positionData.totalCollateral -= amount;", positive_text)
        self.assertIn("usedFunds -= amount;", positive_text)
        self.assertNotIn("EXCHANGE.removeCollateral(positionId, amount);", positive_text)

        self.assertIn("contract KangarooVaultClean", clean_text)
        self.assertIn("function removeCollateral(uint256 amount) external", clean_text)
        self.assertIn("EXCHANGE.removeCollateral(positionId, amount);", clean_text)
        self.assertIn("positionData.totalCollateral -= amount;", clean_text)
