"""Unit tests for ``tools/hackerman-etl-from-lending-protocols.py``.
...
* Map severity / impact / actor correctly for lending-domain advisories.
...
TOOL_PATH = REPO_ROOT / "tools" / "hackerman-etl-from-lending-protocols.py"
VALIDATOR_PATH = REPO_ROOT / "tools" / "hackerman-record-validate.py"


def _load_tool():
    name = "_hackerman_etl_from_lending_protocols"
...
def _load_validator():
    name = "_hackerman_record_validate_for_lending_test"
...
def _sample_advisory(
    *,
    ...
    severity: str = "high",
    summary: str = "Reentrancy in withdraw allows depositor fund theft",
    description: str = (
        "A flash-loan-assisted reentrancy in the lending pool's withdraw path "
        "allows an unprivileged attacker to drain collateral from other "
        "depositors."
    ),
    cve_id: str = "CVE-2024-12345",
    package_name: str = "@aave/aave-v3-core",
    patched_versions: str = ">=1.19.3",
    html_url: str = "https://github.com/aave/aave-v3-core/security/advisories/GHSA-aaaa-bbbb-cccc",
    ...
    return {
        ...
        "cwes": [{"cwe_id": "CWE-841", "name": "Improper Enforcement of Behavioral Workflow"}],
    }
...
class HackermanEtlFromLendingProtocolsTests(unittest.TestCase):
    @classmethod
    ...
    def test_target_repos_cover_required_set(self) -> None:
        ...
        required = {
            ...
            "spark-protocol/spark-bond",
            "radiant-capital/radiant-v2",
            "benqi-fi/benqi-protocol",
            "liquity/dev",
            "liquity/liquity2",
        }
        ...
        # Every entry must be (solidity, lending) - schema enums.
        for repo, lang, domain in self.tool.TARGET_REPOS:
            self.assertEqual(lang, "solidity", msg=f"{repo} lang wrong")
            self.assertEqual(domain, "lending", msg=f"{repo} domain wrong")
    ...
    def test_severity_maps_dollar_class_and_impact_actor(self) -> None:
        adv = _sample_advisory(severity="critical")
        summary = self._run({"aave/aave-v3-core": [adv]})
        ...
        # "drain collateral" routes to theft (lending-specific keyword set).
        self.assertEqual(rec["impact_class"], "theft")
        # Theft on lending protocols hits depositors.
    ...
    def test_moderate_severity_maps_medium(self) -> None:
        adv = _sample_advisory(
            severity="moderate",
            summary="Precision rounding in share-accounting math",
            description=(
                "Rounding in the share-accounting math produces "
                "off-by-one errors when computing pool share balances."
            ),
        )
        self._run({"compound-finance/comet": [adv]})
        sub = next(p for p in self.out_dir.iterdir() if p.is_dir())
        rec = json.loads((sub / "record.json").read_text())
        self.assertEqual(rec["severity_at_finding"], "medium")
        self.assertEqual(rec["impact_dollar_class"], "$10K-$100K")
        # "Precision loss"/"Rounding" route to precision-loss before yield.
        self.assertEqual(rec["impact_class"], "precision-loss")

    def test_honest_zero_repos_reported_not_fabricated(self) -> None:
        # Only 1 repo with an advisory; the other 19 in TARGET_REPOS are
        # honest zeros and should NOT appear as records.
        cache = {"aave/aave-v3-core": [_sample_advisory()]}
        self._write_cache(cache)
        summary = self.tool.convert(
            self.out_dir, cache_file=self.cache_path, dry_run=True
        )
        self.assertEqual(summary["records_emitted"], 1)
        # 20 repos in TARGET_REPOS, 1 with data, 19 honest zeros.
        self.assertEqual(len(summary["repos_with_zero_advisories"]),
                         len(self.tool.TARGET_REPOS) - 1)
        self.assertNotIn("aave/aave-v3-core",
                         summary["repos_with_zero_advisories"])

    def test_filter_repo_restricts_output(self) -> None:
        cache = _build_cache(
            repos=("aave/aave-v3-core", "compound-finance/comet"),
        )
        self._write_cache(cache)
        summary = self.tool.convert(
            self.out_dir,
            cache_file=self.cache_path,
            filter_repo="aave/aave-v3-core",
            dry_run=True,
        )
    ...
    def test_state_filter_skips_unpublished_advisories(self) -> None:
        ...
        draft["state"] = "draft"
        self._run({"aave/aave-v3-core": [published, draft]})
        sub_ids = [
    ...
    def test_slug_uses_double_underscore_separator(self) -> None:
        ...
        self.assertTrue(subs[0].startswith("aave__aave-v3-core__"),
    ...
    def test_dedupe_collapses_same_ghsa(self) -> None:
        ...
        summary = self._run({"aave/aave-v3-core": [adv, adv]})
        self.assertEqual(summary["records_emitted"], 1)
        self.assertEqual(summary["errors"], [])
