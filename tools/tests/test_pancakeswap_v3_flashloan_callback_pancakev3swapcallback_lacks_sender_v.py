...
PATTERN = "pancakeswap-v3-flashloan-callback-pancakev3swapcallback-lacks-sender-v"
DETECTOR = (
    ROOT
    / "detectors"
    / "wave_graveyard"
    / "wave13_broken"
    / "pancakeswap_v3_flashloan_callback_pancakev3swapcallback_lacks_sender_v.py"
)
...
FIXTURE_DIR = (
    ...
    / "detectors"
    / "fixtures"
    / "pancakeswap_v3_flashloan_callback_pancakev3swapcallback_lacks_sender_v"
)
ALT_FIXTURE_DIR = (
    ROOT
    / "detectors"
    / "fixtures"
    / "pancakeswap-v3-flashloan-callback-pancakev3swapcallback-lacks-sender-v"
)
...
class PancakeswapV3FlashloanCallbackSenderValidationTest(unittest.TestCase):
    def _hits(self, fixture: Path) -> int:
        ...
        self.assertIn("cache: off", proc.stdout)
    ...
    def test_detector_reference_and_fixture_scope_stay_aligned(self) -> None:
        ...
        self.assertIn("verifyCallback-style", detector_text)
        ...
        self.assertIn("function pancakeV3SwapCallback(", positive_text)
        self.assertIn("transfer(msg.sender, amountToPay)", positive_text)
        self.assertNotIn("require(msg.sender == expectedPool", positive_text)

        self.assertIn("function pancakeV3SwapCallback(", clean_text)
        self.assertIn('require(msg.sender == expectedPool, "caller not pool");', clean_text)
        ...
        self.assertIn("AUDITOOOR_SLITHER_NOCACHE=1", payload["positive_command"])
    ...
    def test_hyphenated_fixture_mirror_stays_in_sync(self) -> None:
        ...
        self.assertEqual(alt_payload["submission_posture"], "NOT_SUBMIT_READY")
