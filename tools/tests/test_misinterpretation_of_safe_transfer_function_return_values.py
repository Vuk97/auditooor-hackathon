...
PATTERN = "misinterpretation-of-safe-transfer-function-return-values"
DETECTOR = (
    ROOT
    / "detectors"
    / "wave17"
    / "misinterpretation_of_safe_transfer_function_return_values.py"
)
...
FIXTURE_DIR = (
    ...
    / "detectors"
    / "fixtures"
    / "misinterpretation_of_safe_transfer_function_return_values"
)
ALT_FIXTURE_DIR = (
    ROOT
    / "detectors"
...
class MisinterpretationOfSafeTransferFunctionReturnValuesTest(unittest.TestCase):
    ...
    def test_detector_reference_and_fixture_scope_stay_aligned(self) -> None:
        ...
        self.assertIn("parent.parent", detector_text)
        self.assertIn("NOT_SUBMIT_READY", detector_text)
        self.assertIn("safeTransfer-style helper result", detector_text)
        ...
        self.assertIn("function sweep(address token, address to, uint256 amount) external", positive_text)
        self.assertIn("bool ok = IERC20(token).safeTransfer(to, amount);", positive_text)
        self.assertIn("require(ok, \"safe transfer failed\");", positive_text)

        self.assertIn("function sweep(address token, address to, uint256 amount) external", clean_text)
        self.assertIn("IERC20(token).safeTransfer(to, amount);", clean_text)
        self.assertNotIn("bool ok = IERC20(token).safeTransfer(to, amount);", clean_text)
        self.assertIn("abi.decode(data, (bool))", clean_text)
        ...
        self.assertIn("safeTransfer-style helper result", payload["limitation_note"])
    ...
    def test_hyphenated_fixture_mirror_stays_in_sync(self) -> None:
        ...
        self.assertEqual(SMOKE.read_text(encoding="utf-8"), ALT_SMOKE.read_text(encoding="utf-8"))
