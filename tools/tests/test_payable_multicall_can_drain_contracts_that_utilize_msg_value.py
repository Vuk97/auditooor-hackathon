...
PATTERN = "payable-multicall-can-drain-contracts-that-utilize-msg-value"
DETECTOR = (
    ROOT
    / "detectors"
    / "wave_graveyard"
    / "wave13_broken"
    / "payable_multicall_can_drain_contracts_that_utilize_msg_value.py"
)
REFERENCE = (
    ROOT
    / "reference"
    / "patterns.dsl"
    / "payable-multicall-can-drain-contracts-that-utilize-msg-value.yaml"
)
FIXTURE_DIR = (
    ROOT
    / "detectors"
    / "fixtures"
    / "payable_multicall_can_drain_contracts_that_utilize_msg_value"
)
ALT_FIXTURE_DIR = (
    ROOT
    / "detectors"
    / "fixtures"
    / "payable-multicall-can-drain-contracts-that-utilize-msg-value"
)
...
class PayableMulticallCanDrainContractsThatUtilizeMsgValueTest(unittest.TestCase):
    ...
    def test_detector_reference_and_fixture_scope_stay_aligned(self) -> None:
        ...
        self.assertIn("_MSG_VALUE_GATE_RE", detector_text)
        self.assertIn("_AUTHORIZATION_EFFECT_RE", detector_text)
        self.assertIn("_SAFE_VALUE_ACCOUNTING_RE", detector_text)
        ...
        self.assertIn("contract SeatReservationVault is PayableMulticall", positive_text)
        self.assertIn('require(msg.value >= seatPrice, "price");', positive_text)
        self.assertIn("seatBooked[seatId] = true;", positive_text)

        self.assertIn("contract SeatReservationVaultSafe is PayableMulticall", clean_text)
        self.assertIn(
            'require(address(this).balance >= accountedBalance + seatPrice, "price");',
            clean_text,
        )
        self.assertIn("accountedBalance += seatPrice;", clean_text)
        self.assertNotIn('require(msg.value >= seatPrice, "price");', clean_text)
        ...
        self.assertIn("accountedBalance", payload["limitation_note"])
        self.assertIn("seatPrice", payload["limitation_note"])
