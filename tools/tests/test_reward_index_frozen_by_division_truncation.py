...
PATTERN = "reward-index-frozen-by-division-truncation"
DETECTOR = ROOT / "detectors" / "wave17" / "reward_index_frozen_by_division_truncation.py"
REFERENCE = ROOT / "reference" / "patterns.dsl" / f"{PATTERN}.yaml"
FIXTURE_DIR = ROOT / "detectors" / "fixtures" / "reward_index_frozen_by_division_truncation"
MIRROR_DIR = ROOT / "detectors" / "fixtures" / "reward-index-frozen-by-division-truncation"
...
class RewardIndexFrozenByDivisionTruncationTest(unittest.TestCase):
    ...
    def test_pattern_compile_round_trip_matches_generated_detector(self) -> None:
        ...
        with tempfile.TemporaryDirectory(prefix=".pattern_compile_reward_index_frozen_", dir=ROOT) as tmp:
    ...
    def test_detector_reference_and_fixture_metadata_stay_advisory(self) -> None:
        ...
        self.assertNotIn("!function.body_contains_regex", detector_text)
        ...
        self.assertIn("uint256 deltaIndex = accrued / totalSupply;", positive_text)
        self.assertIn("lastRewardBalance += accrued;", positive_text)
        self.assertIn("if (deltaIndex == 0) return;", clean_text)
        self.assertIn("lastRewardBalance += deltaIndex * totalSupply;", clean_text)
        self.assertIn("Bait only", bait_text)
        self.assertIn("_ceilDiv", bait_text)
        ...
        self.assertIn("reward-balance watermark", payload["limitation_note"])
