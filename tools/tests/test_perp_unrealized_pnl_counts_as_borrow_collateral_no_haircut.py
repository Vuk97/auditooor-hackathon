...
PATTERN = "perp-unrealized-pnl-counts-as-borrow-collateral-no-haircut"
DETECTOR = ROOT / "detectors" / "wave17" / "perp_unrealized_pnl_counts_as_borrow_collateral_no_haircut.py"
REFERENCE = ROOT / "reference" / "patterns.dsl" / f"{PATTERN}.yaml"
FIXTURE_DIR = ROOT / "detectors" / "fixtures" / "perp_unrealized_pnl_counts_as_borrow_collateral_no_haircut"
...
class PerpUnrealizedPnlCountsAsBorrowCollateralNoHaircutTest(unittest.TestCase):
    ...
    def test_pattern_compile_round_trip_matches_generated_detector(self) -> None:
        ...
        with tempfile.TemporaryDirectory(
            prefix=".pattern_compile_perp_unrealized_pnl_",
            dir=ROOT,
        ) as tmp:
    ...
    def test_detector_reference_and_fixture_metadata(self) -> None:
        ...
        self.assertIn("getMaxBorrow", detector_text)
        self.assertIn("unrealized(Pnl|PnL|Profit)", detector_text)
        self.assertIn("pnlHaircut", detector_text)
        ...
        self.assertIn("contract PerpUnrealizedPnlCollateralPositive", positive_text)
        self.assertIn("function getMaxBorrow(address account, uint256 markPrice) external returns (uint256)", positive_text)
        self.assertIn("uint256 unrealizedPnl = (markPrice - position.entryPrice) * position.size;", positive_text)
        self.assertIn("uint256 borrowable = position.collateral + unrealizedPnl;", positive_text)
        self.assertNotIn("pnlHaircutBps", positive_text)

        self.assertIn("contract PerpUnrealizedPnlCollateralClean", clean_text)
        self.assertIn("uint256 internal constant pnlHaircutBps = 5_000;", clean_text)
        self.assertIn("uint256 effectivePnl = (unrealizedPnl * (10_000 - pnlHaircutBps)) / 10_000;", clean_text)
        self.assertIn("uint256 borrowable = position.collateral + effectivePnl;", clean_text)
