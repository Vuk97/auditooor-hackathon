"""Tests for VaultQuery.vault_verify_session_token callable (W3.8 coverage push).
...
Verifies:
  ...
  3. Sample kwargs payload accepted: token='nosuchtoken'.
...
Notes: invalid token -> valid=false
"""
...
class TestVaultVerifySessionToken(unittest.TestCase):

    def setUp(self):
        self.tmp = tempfile.TemporaryDirectory(prefix="w38-verify_session_token-")
    ...
    def test_happy_path_returns_dict(self):
        # synthetic_fixture: true
        result = self.query.vault_verify_session_token(token='nosuchtoken')
    ...
    def test_no_kwargs_does_not_raise(self):
        # synthetic_fixture: true
        result = self.query.vault_verify_session_token()
        self.assertIsInstance(result, dict)
    ...
    def test_schema_or_error_field_present(self):
        ...
        has_known = any(k in result for k in ['schema', 'valid'])
        self.assertTrue(
    ...
    def test_cli_dispatch_exits_zero(self):
        # synthetic_fixture: true
        args_json = json.dumps({ "token": 'nosuchtoken' })
        proc = subprocess.run(
            [sys.executable, str(MODULE_PATH), "--repo-root", str(self.root),
             "--vault-dir", str(self.vault), "--call", "vault_verify_session_token",
             "--args", args_json],
    ...
    def test_callable_in_tool_schemas(self):
        ...
        self.assertIn("vault_verify_session_token", names)
