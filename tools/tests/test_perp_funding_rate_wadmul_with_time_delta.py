...
PATTERN = "perp-funding-rate-wadmul-with-time-delta"
DETECTOR = ROOT / "detectors" / "wave17" / "perp_funding_rate_wadmul_with_time_delta.py"
REFERENCE = ROOT / "reference" / "patterns.dsl" / f"{PATTERN}.yaml"
FIXTURE_DIR = ROOT / "detectors" / "fixtures" / "perp_funding_rate_wadmul_with_time_delta"
ALT_FIXTURE_DIR = ROOT / "detectors" / "fixtures" / "perp-funding-rate-wadmul-with-time-delta"
...
class PerpFundingRateWadmulWithTimeDeltaTest(unittest.TestCase):
    ...
    def test_detector_reference_and_fixture_scope_stay_aligned(self) -> None:
        ...
        self.assertIn("wadMul(fundingRatePerSecond, dt)", detector_text)
        self.assertIn("block.timestamp - lastFundingTime", detector_text)
        ...
        self.assertIn("uint256 dt = block.timestamp - lastFundingTime;", positive_text)
        self.assertIn("totalFunding = wadMul(fundingRatePerSecond, dt);", positive_text)
        self.assertNotIn("totalFunding = fundingRatePerSecond * dt;", positive_text)

        self.assertIn("uint256 dt = block.timestamp - lastFundingTime;", clean_text)
        self.assertIn("totalFunding = fundingRatePerSecond * dt;", clean_text)
        self.assertNotIn("wadMul(fundingRatePerSecond, dt)", clean_text)
