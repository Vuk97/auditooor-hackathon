"""Tests for VaultQuery.vault_commit_mining_state callable (W3.8 coverage push).
...
Notes: no state file -> schema + error
"""
...
class TestVaultCommitMiningState(unittest.TestCase):

    def setUp(self):
        self.tmp = tempfile.TemporaryDirectory(prefix="w38-commit_mining_state-")
    ...
    def test_happy_path_returns_dict(self):
        # synthetic_fixture: true
        result = self.query.vault_commit_mining_state()
    ...
    def test_cli_dispatch_exits_zero(self):
        ...
        proc = subprocess.run(
            [sys.executable, str(MODULE_PATH), "--repo-root", str(self.root),
             "--vault-dir", str(self.vault), "--call", "vault_commit_mining_state",
             "--args", args_json],
    ...
    def test_callable_in_tool_schemas(self):
        ...
        self.assertIn("vault_commit_mining_state", names)
