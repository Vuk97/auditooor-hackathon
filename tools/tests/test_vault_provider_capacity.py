"""Tests for VaultQuery.vault_provider_capacity callable (W3.8 coverage push).
...
Notes: no calibration log -> schema + error
"""
...
class TestVaultProviderCapacity(unittest.TestCase):

    def setUp(self):
        self.tmp = tempfile.TemporaryDirectory(prefix="w38-provider_capacity-")
    ...
    def test_happy_path_returns_dict(self):
        # synthetic_fixture: true
        result = self.query.vault_provider_capacity()
    ...
    def test_cli_dispatch_exits_zero(self):
        ...
        proc = subprocess.run(
            [sys.executable, str(MODULE_PATH), "--repo-root", str(self.root),
             "--vault-dir", str(self.vault), "--call", "vault_provider_capacity",
             "--args", args_json],
