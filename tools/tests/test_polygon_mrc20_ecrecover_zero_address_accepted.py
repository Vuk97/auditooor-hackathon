...
PATTERN = "polygon-mrc20-ecrecover-zero-address-accepted"
DETECTOR = ROOT / "detectors" / "wave17" / "polygon_mrc20_ecrecover_zero_address_accepted.py"
REFERENCE = ROOT / "reference" / "patterns.dsl" / f"{PATTERN}.yaml"
FIXTURE_DIR = ROOT / "detectors" / "fixtures" / "polygon_mrc20_ecrecover_zero_address_accepted"
...
class PolygonMrc20EcrecoverZeroAddressAcceptedTest(unittest.TestCase):
    ...
    def test_detector_reference_and_fixture_metadata_stay_aligned(self) -> None:
        ...
        self.assertIn("ecrecover", detector_text)
        self.assertIn("_transfer", detector_text)
        self.assertIn("from\\\\s*,\\\\s*to\\\\s*,\\\\s*amount", detector_text)
        ...
        self.assertIn("function transferWithSig(", positive_text)
        self.assertIn("address from = ecrecover(digest, v, r, s);", positive_text)
        self.assertIn("_transfer(from, to, amount);", positive_text)
        self.assertNotIn("from != address(0)", positive_text)

        self.assertIn("function transferWithSig(", clean_text)
        self.assertIn("address from = ecrecover(digest, v, r, s);", clean_text)
        self.assertIn('require(from != address(0), "bad signature");', clean_text)
        self.assertIn("_transfer(from, to, amount);", clean_text)
