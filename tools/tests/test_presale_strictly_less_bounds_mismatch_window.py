...
PATTERN = "presale-strictly-less-bounds-mismatch-window"
DETECTOR = ROOT / "detectors" / "wave17" / "presale_strictly_less_bounds_mismatch_window.py"
REFERENCE = ROOT / "reference" / "patterns.dsl" / f"{PATTERN}.yaml"
FIXTURE_DIR = ROOT / "detectors" / "fixtures" / "presale_strictly_less_bounds_mismatch_window"
MIRROR_DIR = ROOT / "detectors" / "fixtures" / "presale-strictly-less-bounds-mismatch-window"
...
class PresaleStrictlyLessBoundsMismatchWindowTest(unittest.TestCase):
    ...
    def test_pattern_compile_round_trip_matches_generated_detector(self) -> None:
        ...
        with tempfile.TemporaryDirectory(
            prefix=".pattern_compile_presale_strictly_less_",
            dir=ROOT,
        ) as tmp:
    ...
    def test_detector_reference_and_fixtures_stay_row_local(self) -> None:
        ...
        self.assertIn("Presale start-time guard is inverted", detector_text)
        ...
        self.assertIn("function buyPresale(address subject, uint256 amount) external payable", positive_text)
        self.assertIn("presalesMeta[subject].startTime <= block.timestamp", positive_text)
        self.assertIn("revert PresaleUnavailable();", positive_text)

        self.assertIn("function buyPresale(address subject, uint256 amount) external payable", clean_text)
        self.assertIn("block.timestamp < presalesMeta[subject].startTime", clean_text)
        self.assertNotIn("presalesMeta[subject].startTime <= block.timestamp", clean_text)
        ...
        self.assertIn("start-time guard uses `<= block.timestamp`", payload["limitation_note"])
