...
PATTERN = "reentrant-modifier-misuse-nonreentrant-functions-calling-each-other-ca"
DETECTOR = (
    ROOT
    / "detectors"
    / "wave_graveyard"
    / "wave13_broken"
    / "reentrant_modifier_misuse_nonreentrant_functions_calling_each_other_ca.py"
)
REFERENCE = ROOT / "reference" / "patterns.dsl" / f"{PATTERN}.yaml"
FIXTURE_DIR = ROOT / "detectors" / "fixtures" / "reentrant_modifier_misuse_nonreentrant_functions_calling_each_other_ca"
FIXTURE_DIR_HYPHEN = ROOT / "detectors" / "fixtures" / PATTERN
POSITIVE = FIXTURE_DIR / "reentrant_modifier_misuse_nonreentrant_functions_calling_each_other_ca_vulnerable.sol"
CLEAN = FIXTURE_DIR / "reentrant_modifier_misuse_nonreentrant_functions_calling_each_other_ca_clean.sol"
SMOKE = FIXTURE_DIR / "smoke.json"
POSITIVE_HYPHEN = FIXTURE_DIR_HYPHEN / "reentrant_modifier_misuse_nonreentrant_functions_calling_each_other_ca_vulnerable.sol"
CLEAN_HYPHEN = FIXTURE_DIR_HYPHEN / "reentrant_modifier_misuse_nonreentrant_functions_calling_each_other_ca_clean.sol"
...
class ReentrantModifierMisuseNonreentrantFunctionsCallingEachOtherCaTest(unittest.TestCase):
    ...
    def test_detector_compiles_and_reference_points_at_owned_fixture_pair(self) -> None:
        ...
        self.assertIn("calls another `nonReentrant` function", detector_text)
        self.assertIn(str(POSITIVE.relative_to(ROOT)), reference_text)
        self.assertIn(str(CLEAN.relative_to(ROOT)), reference_text)
        self.assertIn("internal or high-level call to another function that is also marked nonReentrant", reference_text)
    ...
    def test_smoke_metadata_marks_advisory_only_posture(self) -> None:
        ...
        self.assertEqual(
            payload["detector_slug"],
            "reentrant_modifier_misuse_nonreentrant_functions_calling_each_other_ca",
        )
