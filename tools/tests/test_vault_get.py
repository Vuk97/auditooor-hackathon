"""Tests for VaultQuery.vault_get callable (W3.8 coverage push).
...
Verifies:
  ...
  3. Sample kwargs payload accepted: path='INDEX.md'.
...
Notes: happy path reads INDEX.md
"""
...
class TestVaultGet(unittest.TestCase):
    ...
    def setUp(self):
        self.tmp = tempfile.TemporaryDirectory(prefix="w38-get-")
    ...
    def test_happy_path_returns_dict(self):
        # synthetic_fixture: true
        result = self.query.vault_get(path='INDEX.md')
        self.assertIsInstance(result, dict)
        # At least one of the expected keys must be present
        expected = ['body', 'title', 'frontmatter', 'path']
    ...
    def test_missing_path_returns_error_envelope(self):
        # synthetic_fixture: true
        # vault_get requires a `path` positional arg; passing a non-existent
        # path must return the structured `not_found` error envelope.
        result = self.query.vault_get(path="does-not-exist.md")
        self.assertIsInstance(result, dict)
    ...
    def test_schema_or_error_field_present(self):
        ...
        has_known = any(k in result for k in ['body', 'title', 'frontmatter', 'path'])
        self.assertTrue(
    ...
    def test_cli_dispatch_exits_zero(self):
        # synthetic_fixture: true
        args_json = json.dumps({ "path": 'INDEX.md' })
        proc = subprocess.run(
            [sys.executable, str(MODULE_PATH), "--repo-root", str(self.root),
             "--vault-dir", str(self.vault), "--call", "vault_get",
             "--args", args_json],
