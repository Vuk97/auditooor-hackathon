"""
test_rust_wave1_solana_token_account_close_authority_unset.py

Tests for solana_token_account_close_authority_unset detector.

Run with:
    python3 -m unittest tools.tests.test_rust_wave1_solana_token_account_close_authority_unset
"""
...
DETECTOR_NAME = "solana_token_account_close_authority_unset"
...
class TestSolanaTokenAccountCloseAuthorityUnset(unittest.TestCase):
    ...
    def test_fires_on_positive_1(self):
        """Fires when initialize_account is called without set_authority(CloseAccount)."""
    ...
    def test_fires_on_positive_2(self):
        """Fires on Anchor context with initialize_account and no close authority."""
        hits = self._scan_fixture(f"{DETECTOR_NAME}_positive_2.rs")
        self.assertGreaterEqual(len(hits), 1, f"Expected >=1 hit, got: {hits}")

    def test_silent_on_negative_1(self):
        """Silent when set_authority(CloseAccount) follows initialize_account."""
    ...
    def test_silent_on_negative_2(self):
        """Silent when file has no initialize_account call."""
        hits = self._scan_fixture(f"{DETECTOR_NAME}_negative_2.rs")
        self.assertEqual(len(hits), 0, f"Expected 0 hits, got: {hits}")

    def test_scan_file_returns_required_fields(self):
        """scan_file() hit contains detector_id, severity=HIGH, fn_name."""
    ...
    def test_scan_file_emits_util_fields(self):
        """scan_file() emits at least one _util-style field."""
        fixture = FIXTURES_DIR / f"{DETECTOR_NAME}_positive_1.rs"
        if not fixture.exists():
            self.skipTest("positive_1 missing")
        hits = self.mod.scan_file(str(fixture))
