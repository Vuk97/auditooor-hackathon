...
PATTERN = "refinance-tokenid-not-matched-to-loan"
DETECTOR = ROOT / "detectors" / "wave17" / "refinance_tokenid_not_matched_to_loan.py"
REFERENCE = ROOT / "reference" / "patterns.dsl" / f"{PATTERN}.yaml"
FIXTURE_DIR = ROOT / "detectors" / "fixtures" / "refinance_tokenid_not_matched_to_loan"
...
class RefinanceTokenidNotMatchedToLoanTest(unittest.TestCase):
    ...
    def test_detector_fixture_and_smoke_metadata(self) -> None:
        ...
        self.assertIn("executionData", detector_text)
        self.assertIn("nftCollateralTokenId", detector_text)

        self.assertIn("pattern: refinance-tokenid-not-matched-to-loan", reference_text)
        self.assertIn("executionData.tokenId == oldLoan.nftCollateralTokenId", reference_text)

        self.assertIn("function refinanceFromLoanExecutionData(", positive_text)
        self.assertIn("executionData.tokenId", positive_text)
        self.assertIn("loan.nftCollateralTokenId", positive_text)
        self.assertNotIn("executionData.tokenId == loan.nftCollateralTokenId", positive_text)
        ...
        self.assertIn("executionData.tokenId", clean_text)
