"""
test_rust_wave1_solana_program_id_check_missing.py

Tests for solana_program_id_check_missing detector.

Run with:
    python3 -m unittest tools.tests.test_rust_wave1_solana_program_id_check_missing
"""
...
DETECTOR_NAME = "solana_program_id_check_missing"
...
class TestSolanaProgramIdCheckMissing(unittest.TestCase):
    ...
    def test_fires_on_positive_1(self):
        """Fires when AccountInfo data borrowed without owner check."""
    ...
    def test_fires_on_positive_2(self):
        """Fires when from_account_info used without owner check."""
        hits = self._scan_fixture(f"{DETECTOR_NAME}_positive_2.rs")
        self.assertGreaterEqual(len(hits), 1, f"Expected >=1 hit, got: {hits}")

    def test_silent_on_negative_1(self):
        """Silent when account.owner check precedes data borrow."""
    ...
    def test_silent_on_negative_2(self):
        """Silent when file has no AccountInfo parameters."""
        hits = self._scan_fixture(f"{DETECTOR_NAME}_negative_2.rs")
        self.assertEqual(len(hits), 0, f"Expected 0 hits, got: {hits}")

    def test_scan_file_returns_required_fields(self):
        """scan_file() hit has detector_id, severity=HIGH, borrow_line."""
