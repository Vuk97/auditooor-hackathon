...
PATTERN = "perp-liquidation-guard-inverted-comparison"
DETECTOR = ROOT / "detectors" / "wave17" / "perp_liquidation_guard_inverted_comparison.py"
REFERENCE = ROOT / "reference" / "patterns.dsl" / f"{PATTERN}.yaml"
FIXTURE_DIR = ROOT / "detectors" / "fixtures" / "perp_liquidation_guard_inverted_comparison"
...
class PerpLiquidationGuardInvertedComparisonTest(unittest.TestCase):
    ...
    def test_detector_reference_and_fixture_metadata(self) -> None:
        ...
        self.assertIn("getMarginRate(...) >= maintenanceMarginRate(...)", detector_text)
        self.assertIn("MarginUnsafe", detector_text)
        ...
        self.assertIn("contract PerpLiquidationEnginePositive", positive_text)
        self.assertIn("getMarginRate(account) >= maintenanceMarginRate(account)", positive_text)
        self.assertIn('"MarginUnsafe"', positive_text)
        self.assertNotIn(" < maintenanceMarginRate(account)", positive_text)

        self.assertIn("contract PerpLiquidationEngineClean", clean_text)
        self.assertIn("getMarginRate(account) < maintenanceMarginRate(account)", clean_text)
        self.assertIn('"MarginSafe"', clean_text)
        self.assertNotIn(">= maintenanceMarginRate(account)", clean_text)
