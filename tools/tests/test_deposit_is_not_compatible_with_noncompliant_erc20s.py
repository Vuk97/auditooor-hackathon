...
PATTERN = "deposit-is-not-compatible-with-noncompliant-erc20s"
REFERENCE = ROOT / "reference" / "patterns.dsl" / f"{PATTERN}.yaml"
DETECTOR = ROOT / "detectors" / "wave17" / "deposit_is_not_compatible_with_noncompliant_erc20s.py"
FIXTURE_DIR = ROOT / "detectors" / "fixtures" / "deposit_is_not_compatible_with_noncompliant_erc20s"
...
class DepositIsNotCompatibleWithNoncompliantErc20sTest(unittest.TestCase):
    ...
    def test_pattern_compile_round_trip_matches_generated_detector(self) -> None:
        ...
        with tempfile.TemporaryDirectory(
            prefix=".pattern_compile_deposit_noncompliant_erc20_",
            dir=ROOT,
        ) as tmp:
    ...
    def test_reference_yaml_points_at_owned_fixture_pair(self) -> None:
        ...
        self.assertIn(
            "vuln: detectors/fixtures/deposit_is_not_compatible_with_noncompliant_erc20s/positive.sol",
            text,
        )
        self.assertIn(
            "clean: detectors/fixtures/deposit_is_not_compatible_with_noncompliant_erc20s/clean.sol",
            text,
        )
