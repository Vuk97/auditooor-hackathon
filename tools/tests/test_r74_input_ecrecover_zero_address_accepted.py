...
PATTERN = "r74-input-ecrecover-zero-address-accepted"
DETECTOR = ROOT / "detectors" / "wave17" / "r74_input_ecrecover_zero_address_accepted.py"
REFERENCE = ROOT / "reference" / "patterns.dsl" / f"{PATTERN}.yaml"
FIXTURE_DIR = ROOT / "detectors" / "fixtures" / "r74_input_ecrecover_zero_address_accepted"
...
class R74InputEcrecoverZeroAddressAcceptedTest(unittest.TestCase):
    ...
    def test_pattern_compile_round_trip_matches_generated_detector(self) -> None:
        ...
        with tempfile.TemporaryDirectory(prefix=".pattern_compile_r74_ecrecover_", dir=ROOT) as tmp:
    ...
    def test_reference_and_fixture_metadata_stay_not_submit_ready(self) -> None:
        ...
        self.assertIn("address signer = ecrecover(digest, v, r, s);", positive_text)
        self.assertIn('require(signer == owner, "bad signer");', positive_text)
        self.assertNotIn("signer != address(0)", positive_text)
        self.assertIn("allowance[owner][spender] = value;", positive_text)

        self.assertIn("address signer = ecrecover(digest, v, r, s);", clean_text)
        self.assertIn('require(signer != address(0), "bad signature");', clean_text)
        self.assertIn('require(signer == owner, "bad signer");', clean_text)
