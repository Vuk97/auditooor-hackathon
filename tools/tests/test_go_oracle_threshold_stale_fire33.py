...
DETECTOR = ROOT / "detectors" / "go_wave1" / "go-oracle-threshold-stale-fire33.py"
FIXTURE_DIR = ROOT / "detectors" / "go_wave1" / "test_fixtures"
PATTERN = "go-oracle-threshold-stale-fire33"
...
class GoOracleThresholdStaleFire33Test(unittest.TestCase):
    def _hits(self, fixture: Path) -> tuple[int, str]:
        ...
        with tempfile.NamedTemporaryFile(prefix=".go_oracle_threshold_fire33_", suffix=".log") as tmp:
            proc = subprocess.run(
                [
                    python_ast,
                    str(TOOL),
    ...
    def test_positive_fixture_fires_and_negative_fixture_is_silent(self) -> None:
        ...
        self.assertIn("SettleLiquidationFromRawOracle", positive_log)
        self.assertIn("ApplyMarginFromMedianIndex", positive_log)
        self.assertIn("UpdateReservesFromThreshold", positive_log)
        self.assertIn("SettleFundingFromCachedQuote", positive_log)
    ...
    def test_false_positive_boundaries_are_locked(self) -> None:
        ...
        self.assertIn("ValidateOracleQuote", clean)
        self.assertIn("quote.Value <= 0", clean)
        self.assertIn("quote.Confidence > k.maxConfidence", clean)
        self.assertIn("ctx.BlockTimeUnix()-quote.UpdatedAt > k.maxAge", clean)
        self.assertIn("deviationBps(quote.Value, position.LastPrice) > k.maxDeviationBps", clean)
        self.assertIn("StoreOraclePriceMetric", clean)
        self.assertIn("tier-3-synthetic-taxonomy-anchored", detector)
