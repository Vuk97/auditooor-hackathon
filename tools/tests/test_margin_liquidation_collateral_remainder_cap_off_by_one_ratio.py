...
PATTERN = "margin-liquidation-collateral-remainder-cap-off-by-one-ratio"
DETECTOR = ROOT / "detectors" / "wave17" / "margin_liquidation_collateral_remainder_cap_off_by_one_ratio.py"
REFERENCE = ROOT / "reference" / "patterns.dsl" / f"{PATTERN}.yaml"
FIXTURE_DIR = ROOT / "detectors" / "fixtures" / "margin_liquidation_collateral_remainder_cap_off_by_one_ratio"
...
class MarginLiquidationCollateralRemainderCapOffByOneRatioTest(unittest.TestCase):
    ...
    def test_detector_reference_and_fixture_metadata_stay_scoped(self) -> None:
        ...
        self.assertIn("fixture-smoke/source-shape evidence", detector_text)
        self.assertIn("crLiquidation - PERCENT", detector_text)
        ...
        self.assertIn("state.riskConfig.crLiquidation,", positive_text)
        self.assertNotIn("state.riskConfig.crLiquidation - PERCENT", positive_text)

        self.assertIn("function executeLiquidate", clean_text)
        self.assertIn("state.riskConfig.crLiquidation - PERCENT", clean_text)
