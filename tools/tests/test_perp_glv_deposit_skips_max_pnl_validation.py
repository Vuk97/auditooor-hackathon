...
PATTERN = "perp-glv-deposit-skips-max-pnl-validation"
DETECTOR = ROOT / "detectors" / "wave17" / "perp_glv_deposit_skips_max_pnl_validation.py"
REFERENCE = ROOT / "reference" / "patterns.dsl" / f"{PATTERN}.yaml"
FIXTURE_DIR = ROOT / "detectors" / "fixtures" / "perp_glv_deposit_skips_max_pnl_validation"
...
class PerpGlvDepositSkipsMaxPnlValidationTest(unittest.TestCase):
    ...
    def test_detector_reference_and_fixture_metadata(self) -> None:
        ...
        self.assertIn("validateMaxPnl", detector_text)
        self.assertIn("isMarketTokenDeposit", detector_text)
        ...
        self.assertIn("contract GlvDepositExecutorPositive", positive_text)
        self.assertIn("if (deposit.isMarketTokenDeposit())", positive_text)
        self.assertIn("MarketUtils.validateMaxPnl", positive_text)
        self.assertRegex(
            positive_text,
            r"if \(deposit\.isMarketTokenDeposit\(\)\) \{\s+MarketUtils\.validateMaxPnl",
        )

        self.assertIn("contract GlvDepositExecutorClean", clean_text)
        self.assertIn("MarketUtils.validateMaxPnl", clean_text)
        self.assertRegex(
            clean_text,
            r"MarketUtils\.validateMaxPnl[^\n]*;\s+if \(deposit\.isMarketTokenDeposit\(\)\)",
        )
