...
PATTERN = "public-nonce-invalidator-enables-order-block-dos"
DETECTOR = ROOT / "detectors" / "wave17" / "public_nonce_invalidator_enables_order_block_dos.py"
REFERENCE = ROOT / "reference" / "patterns.dsl" / f"{PATTERN}.yaml"
FIXTURE_DIR = ROOT / "detectors" / "fixtures" / "public_nonce_invalidator_enables_order_block_dos"
...
class PublicNonceInvalidatorEnablesOrderBlockDosTest(unittest.TestCase):
    ...
    def test_pattern_compile_round_trip_matches_generated_detector(self) -> None:
        ...
        with tempfile.TemporaryDirectory(
            prefix=".pattern_compile_public_nonce_invalidator_",
            dir=ROOT,
        ) as tmp:
    ...
    def test_detector_reference_and_fixture_metadata(self) -> None:
        ...
        self.assertIn("ECDSA\\\\.recover|ecrecover|isValidSignature|SignatureChecker", detector_text)
        ...
        self.assertIn("contract BebopNonceBookPositive", positive_text)
        self.assertIn("function assertAndInvalidateAggregateOrder(address maker, uint256 nonce) external", positive_text)
        self.assertIn("nonceBitmap[maker] |= bit;", positive_text)
        self.assertNotIn("require(msg.sender == maker", positive_text)

        self.assertIn("contract BebopNonceBookClean", clean_text)
        self.assertIn('require(msg.sender == maker, "maker only");', clean_text)
        self.assertIn("nonceBitmap[maker] |= bit;", clean_text)
