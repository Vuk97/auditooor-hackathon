...
PATTERN = "restaking-slash-finalize-after-operator-unregister"
DETECTOR = ROOT / "detectors" / "wave17" / "restaking_slash_finalize_after_operator_unregister.py"
REFERENCE = ROOT / "reference" / "patterns.dsl" / f"{PATTERN}.yaml"
FIXTURE_DIR = ROOT / "detectors" / "fixtures" / "restaking_slash_finalize_after_operator_unregister"
...
class RestakingSlashFinalizeAfterOperatorUnregisterTest(unittest.TestCase):
    ...
    def test_detector_reference_fixture_and_smoke_metadata(self) -> None:
        ...
        self.assertIn("queuedSlashing|earmarkedStakes", detector_text)
        self.assertIn("isOperatorRegisteredToDSS", detector_text)
        ...
        self.assertIn("vuln: detectors/fixtures/restaking_slash_finalize_after_operator_unregister/positive.sol", reference_text)
        self.assertIn("clean: detectors/fixtures/restaking_slash_finalize_after_operator_unregister/clean.sol", reference_text)

        self.assertIn("function finalizeSlashing(bytes32 slashId) external", positive_text)
        self.assertIn("earmarkedStakes[queuedSlashing.operator][queuedSlashing.dss]", positive_text)
        self.assertNotIn("isOperatorRegisteredToDSS(operator, queuedSlashing.dss)", positive_text)
        self.assertIn("require(", clean_text)
        self.assertIn("isOperatorRegisteredToDSS(operator, queuedSlashing.dss)", clean_text)
