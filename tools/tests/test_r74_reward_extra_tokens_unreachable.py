...
PATTERN = "r74-reward-extra-tokens-unreachable"
DETECTOR = ROOT / "detectors" / "wave17" / "r74_reward_extra_tokens_unreachable.py"
REFERENCE = ROOT / "reference" / "patterns.dsl" / f"{PATTERN}.yaml"
FIXTURE_DIR = ROOT / "detectors" / "fixtures" / "r74_reward_extra_tokens_unreachable"
...
class R74RewardExtraTokensUnreachableTest(unittest.TestCase):
    ...
    def test_pattern_compile_round_trip_matches_generated_detector(self) -> None:
        ...
        with tempfile.TemporaryDirectory(
            prefix=".pattern_compile_r74_reward_extra_tokens_",
            dir=ROOT,
        ) as tmp:
    ...
    def test_detector_reference_and_fixture_metadata(self) -> None:
        ...
        self.assertIn("forwardExtra", detector_text)
        ...
        self.assertIn("contract RewardExtraTokensUnreachablePositive", positive_text)
        self.assertIn("IERC20Like[] public rewardTokens;", positive_text)
        self.assertIn("function harvest() external", positive_text)
        self.assertIn("rewardTokens.length", positive_text)
        self.assertNotIn("forwardExtra(", positive_text)

        self.assertIn("contract RewardExtraTokensUnreachableClean", clean_text)
        self.assertIn("forwardExtra(airdropToken, distributor);", clean_text)
        self.assertIn("function forwardExtra(IERC20Like extraReward, address recipient) internal", clean_text)
