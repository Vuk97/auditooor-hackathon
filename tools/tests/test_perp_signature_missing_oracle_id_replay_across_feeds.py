...
PATTERN = "perp-signature-missing-oracle-id-replay-across-feeds"
DETECTOR = ROOT / "detectors" / "wave17" / "perp_signature_missing_oracle_id_replay_across_feeds.py"
REFERENCE = ROOT / "reference" / "patterns.dsl" / f"{PATTERN}.yaml"
FIXTURE_DIR = ROOT / "detectors" / "fixtures" / "perp_signature_missing_oracle_id_replay_across_feeds"
...
class PerpSignatureMissingOracleIdReplayAcrossFeedsTest(unittest.TestCase):
    ...
    def test_pattern_compile_round_trip_matches_generated_detector(self) -> None:
        ...
        with tempfile.TemporaryDirectory(
            prefix=".pattern_compile_perp_signature_oracle_id_",
            dir=ROOT,
        ) as tmp:
    ...
    def test_detector_reference_and_fixture_metadata(self) -> None:
        ...
        self.assertIn("oracleId|feedId|priceId|marketId", detector_text)
        ...
        self.assertIn("contract MuxPriceProviderPositive", positive_text)
        self.assertIn("abi.encodePacked(block.chainid, sequence, price, timestamp)", positive_text)
        self.assertIn("ECDSA.recover", positive_text)
        self.assertNotIn("oracleId, block.chainid", positive_text)

        self.assertIn("contract MuxPriceProviderClean", clean_text)
        self.assertIn("abi.encodePacked(oracleId, block.chainid, sequence, price, timestamp)", clean_text)
        self.assertIn("ECDSA.recover", clean_text)
        ...
        self.assertIn("owned Mux-style `setPrice` shape", payload["limitation_note"])
