...
PATTERN = "perp-autoclose-autohedge-missing-signature-check"
DETECTOR = ROOT / "detectors" / "wave17" / "perp_autoclose_autohedge_missing_signature_check.py"
REFERENCE = ROOT / "reference" / "patterns.dsl" / f"{PATTERN}.yaml"
FIXTURE_DIR = ROOT / "detectors" / "fixtures" / "perp_autoclose_autohedge_missing_signature_check"
ALT_FIXTURE_DIR = ROOT / "detectors" / "fixtures" / "perp-autoclose-autohedge-missing-signature-check"
...
class PerpAutocloseAutohedgeMissingSignatureCheckTest(unittest.TestCase):
    ...
    def test_detector_reference_and_fixture_scope_stay_aligned(self) -> None:
        ...
        self.assertIn("_SETTLEMENT_PARAM_RE", detector_text)
        self.assertIn("owner signature check", detector_text)
        ...
        self.assertIn("function autoClose(uint256 positionId, SettlementParams calldata settlementParams) external", positive_text)
        self.assertIn("_executeClose(positionId, settlementParams.swapData, settlementParams.minAmountOut, settlementParams.maxSlippageBps);", positive_text)
        self.assertNotIn("ownerSignature", positive_text)

        self.assertIn("bytes calldata ownerSignature", clean_text)
        self.assertIn("owner.isValidSignatureNow", clean_text)
        self.assertIn("keccak256(settlementParams.swapData)", clean_text)
        ...
        self.assertIn("caller-chosen settlement params", payload["limitation_note"])
