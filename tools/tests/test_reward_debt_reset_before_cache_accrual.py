...
PATTERN = "reward-debt-reset-before-cache-accrual"
DETECTOR = ROOT / "detectors" / "wave17" / "reward_debt_reset_before_cache_accrual.py"
REFERENCE = ROOT / "reference" / "patterns.dsl" / f"{PATTERN}.yaml"
FIXTURE_DIR = ROOT / "detectors" / "fixtures" / "reward_debt_reset_before_cache_accrual"
MIRROR_DIR = ROOT / "detectors" / "fixtures" / "reward-debt-reset-before-cache-accrual"
...
class RewardDebtResetBeforeCacheAccrualTest(unittest.TestCase):
    ...
    def test_detector_reference_and_fixture_smoke_metadata_stay_advisory(self) -> None:
        ...
        self.assertIn("userRewardDebts[user][token] = 0;", positive_text)
        self.assertIn(
            "cachedUserRewards[user][token] += rewardDebtDiff - userRewardDebts[user][token];",
            positive_text,
        )
        self.assertLess(
            positive_text.index("userRewardDebts[user][token] = 0;"),
            positive_text.index("cachedUserRewards[user][token] += rewardDebtDiff"),
        )

        self.assertLess(
            clean_text.index("cachedUserRewards[user][token] += rewardDebtDiff"),
            clean_text.index("userRewardDebts[user][token] = 0;"),
        )
        ...
        self.assertIn("same-slot/user/token proof", payload["limitation_note"])
