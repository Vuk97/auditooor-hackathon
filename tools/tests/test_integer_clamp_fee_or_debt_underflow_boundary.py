...
PATTERN = "integer-clamp-fee-or-debt-underflow-boundary"
DETECTOR = ROOT / "detectors" / "wave17" / "integer_clamp_fee_or_debt_underflow_boundary.py"
...
class IntegerClampFeeOrDebtUnderflowBoundaryTest(unittest.TestCase):
    ...
    def test_pattern_compile_round_trip_matches_generated_detector(self) -> None:
        ...
        with tempfile.TemporaryDirectory(prefix=".pattern_compile_integer_boundary_", dir=ROOT) as tmp:
    ...
    def test_reference_and_fixtures_pin_confirmed_integer_clamp_subset(self) -> None:
        ast.parse(DETECTOR.read_text(encoding="utf-8"))
        ...
        self.assertEqual(
            spec["replay_scope"]["cross_class_replayed_and_rejected"],
            [
                "input-missing-zero-address-check",
        ...
        self.assertIn("uint256 swapFee = protocolFee + lpFee;", positive_text)
        self.assertIn("return lastDebt - decay;", positive_text)
        self.assertIn("market.totalDebt -= decay;", positive_text)
        self.assertNotIn("unchecked", positive_text)
        self.assertIn("if (swapFee == protocolFee)", clean_text)
        self.assertIn("quoteProtocolFeeOnly", clean_text)
        self.assertIn("return decay > lastDebt ? 0 : lastDebt - decay;", clean_text)
        self.assertIn("ClampMath.min", clean_text)
