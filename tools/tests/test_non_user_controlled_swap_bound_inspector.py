...
PATTERN = "non-user-controlled-swap-bound-inspector"
DETECTOR = (
    ROOT
    / "detectors"
    / "wave_graveyard"
    / "wave13_broken"
    / "non_user_controlled_swap_bound_inspector.py"
)
REFERENCE = ROOT / "reference" / "patterns.dsl" / f"{PATTERN}.yaml"
FIXTURE_DIR = ROOT / "detectors" / "fixtures" / "non_user_controlled_swap_bound_inspector"
ALT_FIXTURE_DIR = ROOT / "detectors" / "fixtures" / "non-user-controlled-swap-bound-inspector"
...
class NonUserControlledSwapBoundInspectorTest(unittest.TestCase):
    ...
    def test_detector_reference_and_fixture_scope_stay_aligned(self) -> None:
        ...
        self.assertIn("amountOutMinimum", detector_text)
        self.assertIn("_has_user_controlled_bound_param", detector_text)
        self.assertIn("quote", detector_text)
        ...
        self.assertIn("function rebalance(address tokenIn, address tokenOut, uint256 amountIn)", positive_text)
        self.assertIn("uint256 protocolMinOut = quotedOut * (10_000 - slippageBps) / 10_000;", positive_text)
        self.assertIn("amountOutMinimum: protocolMinOut", positive_text)
        self.assertNotIn("userMinAmountOut", positive_text)

        self.assertIn("uint256 userMinAmountOut", clean_text)
        self.assertIn('require(userMinAmountOut > 0, "missing user bound");', clean_text)
        self.assertIn("amountOutMinimum: userMinAmountOut", clean_text)
        ...
        self.assertIn("protocolMinOut", payload["limitation_note"])
        self.assertIn("userMinAmountOut", payload["limitation_note"])
