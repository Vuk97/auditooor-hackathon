...
PATTERN = "go-bridge-message-recipient-validation-missing"
...
class GoBridgeMessageRecipientValidationMissingTest(unittest.TestCase):
    def _hits(self, fixture: Path) -> tuple[int, str]:
        ...
        with tempfile.NamedTemporaryFile(prefix=".go_bridge_message_recipient_", suffix=".log") as tmp:
            proc = subprocess.run(
                [
                    python_ast,
                    str(TOOL),
