"""Tests for VaultQuery.vault_dupe_rejection_context callable (W3.8 coverage push).
...
class TestVaultDupeRejectionContext(unittest.TestCase):
    ...
    def setUp(self):
        self.tmp = tempfile.TemporaryDirectory(prefix="w38-dupe_rejection_context-")
    ...
    def test_happy_path_returns_dict(self):
        # synthetic_fixture: true
        result = self.query.vault_dupe_rejection_context()
    ...
    def test_cli_dispatch_exits_zero(self):
        ...
        proc = subprocess.run(
            [sys.executable, str(MODULE_PATH), "--repo-root", str(self.root),
             "--vault-dir", str(self.vault), "--call", "vault_dupe_rejection_context",
             "--args", args_json],
