...
PATTERN = "perp-max-pnl-bypass-via-partial-close"
DETECTOR = ROOT / "detectors" / "wave17" / "perp_max_pnl_bypass_via_partial_close.py"
REFERENCE = ROOT / "reference" / "patterns.dsl" / f"{PATTERN}.yaml"
FIXTURE_DIR = ROOT / "detectors" / "fixtures" / "perp_max_pnl_bypass_via_partial_close"
...
class PerpMaxPnlBypassViaPartialCloseTest(unittest.TestCase):
    ...
    def test_detector_reference_and_fixture_metadata(self) -> None:
        ...
        self.assertIn("_toMint\\\\s*>\\\\s*_trade\\\\.margin", detector_text)
        self.assertIn("_partialMargin\\\\s*\\\\*\\\\s*maxWinPercent", detector_text)
        ...
        self.assertIn("contract PerpPositionBookPositive", positive_text)
        self.assertIn("if (_toMint > _trade.margin * maxWinPercent / DIVISION_CONSTANT)", positive_text)
        self.assertNotIn("_partialMargin * maxWinPercent", positive_text)

        self.assertIn("contract PerpPositionBookClean", clean_text)
        self.assertIn("uint256 _partialMargin = _trade.margin * _percent / DIVISION_CONSTANT", clean_text)
        self.assertIn("if (_toMint > _partialMargin * maxWinPercent / DIVISION_CONSTANT)", clean_text)
