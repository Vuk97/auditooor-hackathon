...
PATTERN = "r74-oracle-liquidation-factor-above-storefront"
DETECTOR = ROOT / "detectors" / "wave17" / "r74_oracle_liquidation_factor_above_storefront.py"
REFERENCE = ROOT / "reference" / "patterns.dsl" / f"{PATTERN}.yaml"
FIXTURE_DIR = ROOT / "detectors" / "fixtures" / "r74_oracle_liquidation_factor_above_storefront"
...
class R74OracleLiquidationFactorAboveStorefrontTest(unittest.TestCase):
    ...
    def test_pattern_compile_round_trip_matches_generated_detector(self) -> None:
        ...
        with tempfile.TemporaryDirectory(
            prefix=".pattern_compile_r74_oracle_liquidation_",
            dir=ROOT,
        ) as tmp:
    ...
    def test_detector_reference_and_fixture_metadata(self) -> None:
        ...
        self.assertIn("liquidationFactor|storeFrontPriceFactor", detector_text)
        ...
        self.assertIn("contract CometRiskParameterPositive", positive_text)
        self.assertIn("function setLiquidationFactor", positive_text)
        self.assertIn("function setStoreFrontPriceFactor", positive_text)
        self.assertNotIn("newLiquidationFactor < storeFrontPriceFactor", positive_text)

        self.assertIn("contract CometRiskParameterClean", clean_text)
        self.assertIn("require(newLiquidationFactor < storeFrontPriceFactor, \"no-loss\");", clean_text)
        self.assertIn("require(liquidationFactor < newStoreFrontPriceFactor, \"no-loss\");", clean_text)
