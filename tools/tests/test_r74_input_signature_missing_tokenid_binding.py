...
PATTERN = "r74-input-signature-missing-tokenid-binding"
DETECTOR = ROOT / "detectors" / "wave17" / "r74_input_signature_missing_tokenid_binding.py"
REFERENCE = ROOT / "reference" / "patterns.dsl" / f"{PATTERN}.yaml"
FIXTURE_DIR = ROOT / "detectors" / "fixtures" / "r74_input_signature_missing_tokenid_binding"
...
class R74InputSignatureMissingTokenidBindingTest(unittest.TestCase):
    ...
    def test_pattern_compile_round_trip_matches_generated_detector(self) -> None:
        ...
        with tempfile.TemporaryDirectory(prefix=".pattern_compile_r74_input_", dir=ROOT) as tmp:
    ...
    def test_reference_and_fixture_metadata_stay_not_submit_ready(self) -> None:
        ...
        self.assertIn("has_param_name_matching", detector_text)
        self.assertIn("ownerOf", detector_text)
        ...
        self.assertIn("function castVoteBySig(", positive_text)
        self.assertIn("abi.encode(VOTE_TYPEHASH, voter, proposalId, support, nonce, deadline)", positive_text)
        self.assertIn("ownerOf(tokenId) == signer", positive_text)
        self.assertNotIn("abi.encode(VOTE_TYPEHASH, tokenId, voter, proposalId, support, nonce, deadline)", positive_text)

        self.assertIn("function castVoteBySig(", clean_text)
        self.assertIn("abi.encode(VOTE_TYPEHASH, tokenId, voter, proposalId, support, nonce, deadline)", clean_text)
        self.assertIn("ownerOf(tokenId) == signer", clean_text)
