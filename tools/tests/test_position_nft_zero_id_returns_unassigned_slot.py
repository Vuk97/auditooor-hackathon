...
PATTERN = "position-nft-zero-id-returns-unassigned-slot"
DETECTOR = ROOT / "detectors" / "wave17" / "position_nft_zero_id_returns_unassigned_slot.py"
REFERENCE = ROOT / "reference" / "patterns.dsl" / f"{PATTERN}.yaml"
FIXTURE_DIR = ROOT / "detectors" / "fixtures" / "position_nft_zero_id_returns_unassigned_slot"
...
class PositionNftZeroIdReturnsUnassignedSlotTest(unittest.TestCase):
    ...
    def test_detector_reference_and_fixture_metadata(self) -> None:
        ...
        self.assertIn('WIKI_TITLE = "keyId == 0 bypass on position-NFT ownership check"', detector_text)
        self.assertIn("reservedKeys", detector_text)
        self.assertIn("availableNFTs", detector_text)
        ...
        self.assertIn("contract PositionNftZeroIdPositive", positive_text)
        self.assertIn("function exitFarm(uint256 keyId) external returns (uint256 nftId)", positive_text)
        self.assertIn("reservedKeys[msg.sender] == keyId", positive_text)
        self.assertIn("nftId = availableNFTs[keyId];", positive_text)
        self.assertNotIn("keyId == 0", positive_text)

        self.assertIn("contract PositionNftZeroIdClean", clean_text)
        self.assertIn("if (keyId == 0)", clean_text)
        self.assertIn("revert InvalidKeyId();", clean_text)
        self.assertIn("reservedKeys[msg.sender] == keyId", clean_text)
