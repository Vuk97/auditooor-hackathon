...
PATTERN = "r94-loop-unchecked-erc20-return"
DETECTOR = ROOT / "detectors" / "wave17" / "r94_loop_unchecked_erc20_return.py"
REFERENCE = ROOT / "reference" / "patterns.dsl" / f"{PATTERN}.yaml"
FIXTURE_DIR = ROOT / "detectors" / "fixtures" / "r94_loop_unchecked_erc20_return"
...
class R94LoopUncheckedErc20ReturnTest(unittest.TestCase):
    ...
    def test_detector_reference_and_fixture_smoke_metadata_stay_advisory(self) -> None:
        ...
        self.assertIn("contract UncheckedErc20ReturnPositive", positive_text)
        self.assertIn("IERC20(token).transfer(to, amount);", positive_text)
        self.assertNotIn("require(IERC20(token).transfer", positive_text)

        self.assertIn("contract UncheckedErc20ReturnClean", clean_text)
        self.assertIn("bool ok = IERC20(token).transfer(to, amount);", clean_text)
        self.assertIn('require(ok, "transfer failed");', clean_text)
        self.assertNotRegex(clean_text, r"\n\s*IERC20\(token\)\.transfer\(to, amount\);\n")
        ...
        self.assertIn("bare approve/transfer/transferFrom", payload["limitation_note"])
