...
PATTERN = "missing-zero-address-validation-in-constructor"
DETECTOR = (
    ROOT
    / "detectors"
    / "wave_graveyard"
    / "wave13_broken"
    / "missing_zero_address_validation_in_constructor.py"
)
REFERENCE = ROOT / "reference" / "patterns.dsl" / f"{PATTERN}.yaml"
FIXTURE_DIR = ROOT / "detectors" / "fixtures" / "missing_zero_address_validation_in_constructor"
ALT_FIXTURE_DIR = ROOT / "detectors" / "fixtures" / "missing-zero-address-validation-in-constructor"
...
class MissingZeroAddressValidationInConstructorTest(unittest.TestCase):
    ...
    def test_detector_reference_and_fixture_scope_stay_aligned(self) -> None:
        ...
        self.assertIn("visible zero-address guard", detector_text)
        ...
        self.assertIn("constructor(address newOwner, address newTreasury)", positive_text)
        self.assertIn("owner = newOwner;", positive_text)
        self.assertIn("treasury = newTreasury;", positive_text)
        self.assertNotIn("require(newOwner != address(0)", positive_text)

        self.assertIn("constructor(address newOwner, address newTreasury)", clean_text)
        self.assertIn('require(newOwner != address(0), "owner zero");', clean_text)
        self.assertIn('require(newTreasury != address(0), "treasury zero");', clean_text)
        ...
        self.assertIn("owner = newOwner;", payload["limitation_note"])
