...
PATTERN = "malformed-equate-statement"
DETECTOR = (
    ROOT
    / "detectors"
    / "wave_graveyard"
    / "wave13_broken"
    / "malformed_equate_statement.py"
)
REFERENCE = ROOT / "reference" / "patterns.dsl" / f"{PATTERN}.yaml"
FIXTURE_DIR = ROOT / "detectors" / "fixtures" / "malformed_equate_statement"
ALT_FIXTURE_DIR = ROOT / "detectors" / "fixtures" / "malformed-equate-statement"
...
class MalformedEquateStatementTest(unittest.TestCase):
    ...
    def test_detector_reference_and_fixture_scope_stay_aligned(self) -> None:
        ...
        self.assertIn("standalone `stateVar == value;`", detector_text)
        ...
        self.assertIn("function setThreshold(uint256 newThreshold) external", positive_text)
        self.assertIn("threshold == newThreshold;", positive_text)
        self.assertNotIn("threshold = newThreshold;", positive_text)

        self.assertIn("function setThreshold(uint256 newThreshold) external", clean_text)
        self.assertIn("threshold = newThreshold;", clean_text)
        self.assertNotIn("threshold == newThreshold;", clean_text)
        ...
        self.assertIn("standalone `threshold == newThreshold;`", payload["limitation_note"])
