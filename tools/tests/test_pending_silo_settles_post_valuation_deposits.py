...
PATTERN = "pending-silo-settles-post-valuation-deposits"
DETECTOR = ROOT / "detectors" / "wave17" / "pending_silo_settles_post_valuation_deposits.py"
REFERENCE = ROOT / "reference" / "patterns.dsl" / f"{PATTERN}.yaml"
FIXTURE_DIR = ROOT / "detectors" / "fixtures" / "pending_silo_settles_post_valuation_deposits"
ALT_FIXTURE_DIR = ROOT / "detectors" / "fixtures" / "pending-silo-settles-post-valuation-deposits"
...
class PendingSiloSettlesPostValuationDepositsTest(unittest.TestCase):
    ...
    def test_detector_reference_and_fixture_scope_stay_aligned(self) -> None:
        ...
        self.assertIn("valuationTimestamp", detector_text)
        self.assertIn("balanceOf", detector_text)
        self.assertIn("assetsAtValuation", detector_text)
        ...
        self.assertIn("function settleDeposit() external", positive_text)
        self.assertIn("uint256 pendingAssets = asset.balanceOf(pendingSilo);", positive_text)
        self.assertIn("asset.transferFrom(pendingSilo, address(this), pendingAssets);", positive_text)
        self.assertNotIn("requestTime <= valuationTimestamp", positive_text)

        self.assertIn("function settleDeposit() external", clean_text)
        self.assertIn('require(requestTime <= valuationTimestamp, "post-valuation deposits wait");', clean_text)
        self.assertIn("uint256 assetsAtValuation = pendingAssetsAtValuation[currentEpoch];", clean_text)
        ...
        self.assertEqual(manifest["materialization_status"], "materialized_and_smoke_passed")
