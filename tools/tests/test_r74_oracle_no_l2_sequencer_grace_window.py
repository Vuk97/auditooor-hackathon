...
PATTERN = "r74-oracle-no-l2-sequencer-grace-window"
DETECTOR = ROOT / "detectors" / "wave17" / "r74_oracle_no_l2_sequencer_grace_window.py"
REFERENCE = ROOT / "reference" / "patterns.dsl" / f"{PATTERN}.yaml"
FIXTURE_DIR = ROOT / "detectors" / "fixtures" / "r74_oracle_no_l2_sequencer_grace_window"
...
class R74OracleNoL2SequencerGraceWindowTest(unittest.TestCase):
    ...
    def test_pattern_compile_round_trip_matches_generated_detector(self) -> None:
        ...
        with tempfile.TemporaryDirectory(prefix=".pattern_compile_r74_oracle_l2_", dir=ROOT) as tmp:
    ...
    def test_reference_and_fixture_metadata_stay_not_submit_ready(self) -> None:
        ...
        self.assertIn("sequencerFeed.latestRoundData()", positive_text)
        self.assertIn("priceFeed.latestRoundData()", positive_text)
        self.assertNotIn("GRACE_PERIOD", positive_text)
        self.assertNotIn("block.timestamp - startedAt", positive_text)

        self.assertIn("uint256 internal constant GRACE_PERIOD = 3600;", clean_text)
        self.assertIn("block.timestamp - startedAt > GRACE_PERIOD", clean_text)
        self.assertIn("priceFeed.latestRoundData()", clean_text)
