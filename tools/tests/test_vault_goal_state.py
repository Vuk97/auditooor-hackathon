"""Tests for VaultQuery.vault_goal_state callable (W3.8 coverage push).
...
Notes: empty vault -> status field present
"""
...
class TestVaultGoalState(unittest.TestCase):

    def setUp(self):
        self.tmp = tempfile.TemporaryDirectory(prefix="w38-goal_state-")
    ...
    def test_happy_path_returns_dict(self):
        # synthetic_fixture: true
        result = self.query.vault_goal_state()
    ...
    def test_schema_or_error_field_present(self):
        ...
        has_known = any(k in result for k in ['status'])
        self.assertTrue(
    ...
    def test_cli_dispatch_exits_zero(self):
        ...
        proc = subprocess.run(
            [sys.executable, str(MODULE_PATH), "--repo-root", str(self.root),
             "--vault-dir", str(self.vault), "--call", "vault_goal_state",
             "--args", args_json],
    ...
    def test_callable_in_tool_schemas(self):
        ...
        self.assertIn("vault_goal_state", names)
