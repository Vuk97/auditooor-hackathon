"""Tests for VaultQuery.vault_next_loop callable (W3.8 coverage push).
...
Verifies:
  ...
  3. Sample kwargs payload accepted: limit=3.
...
Notes: NEXT_LOOP.md missing -> error envelope
"""
...
class TestVaultNextLoop(unittest.TestCase):

    def setUp(self):
        self.tmp = tempfile.TemporaryDirectory(prefix="w38-next_loop-")
    ...
    def test_happy_path_returns_dict(self):
        # synthetic_fixture: true
        result = self.query.vault_next_loop(limit=3)
    ...
    def test_no_kwargs_does_not_raise(self):
        # synthetic_fixture: true
        result = self.query.vault_next_loop()
        self.assertIsInstance(result, dict)
    ...
    def test_schema_or_error_field_present(self):
        ...
        has_known = any(k in result for k in ['error', 'path'])
        self.assertTrue(
    ...
    def test_cli_dispatch_exits_zero(self):
        ...
        proc = subprocess.run(
            [sys.executable, str(MODULE_PATH), "--repo-root", str(self.root),
             "--vault-dir", str(self.vault), "--call", "vault_next_loop",
             "--args", args_json],
    ...
    def test_callable_in_tool_schemas(self):
        ...
        self.assertIn("vault_next_loop", names)
