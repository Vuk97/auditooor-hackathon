...
PATTERN = "profit-loss-else-if-drops-one-branch"
DETECTOR = ROOT / "detectors" / "wave17" / "profit_loss_else_if_drops_one_branch.py"
REFERENCE = ROOT / "reference" / "patterns.dsl" / f"{PATTERN}.yaml"
FIXTURE_DIR = ROOT / "detectors" / "fixtures" / "profit_loss_else_if_drops_one_branch"
ALT_FIXTURE_DIR = ROOT / "detectors" / "fixtures" / "profit-loss-else-if-drops-one-branch"
...
class ProfitLossElseIfDropsOneBranchTest(unittest.TestCase):
    ...
    def test_detector_reference_and_fixture_scope_stay_aligned(self) -> None:
        ...
        self.assertIn("else\\\\s+if", detector_text)
        self.assertIn("_burn|burn|burnLoss", detector_text)
        ...
        self.assertIn("function repayCreditAccount(uint256 debt, uint256 profit, uint256 loss) external", positive_text)
        self.assertIn("} else if (loss > 0) {", positive_text)
        self.assertIn("_burnTreasury(loss);", positive_text)
        self.assertIn("function repayCreditAccount(uint256 debt, uint256 profit, uint256 loss) external", clean_text)
        self.assertIn("if (loss > 0) {", clean_text)
        self.assertNotIn("} else if (loss > 0) {", clean_text)
