...
DETECTOR = ROOT / "detectors" / "go_wave1" / "go-rewards-sink-policy-missing.py"
FIXTURE_DIR = ROOT / "detectors" / "go_wave1" / "test_fixtures"
PATTERN = "go-rewards-sink-policy-missing"
POSITIVE = FIXTURE_DIR / f"{PATTERN}_positive.go"
CLEAN = FIXTURE_DIR / f"{PATTERN}_negative.go"
FIRE14_POSITIVE = FIXTURE_DIR / "go-rewards-distribution-skew-blocked-module-recipient_positive.go"
FIRE14_CLEAN = FIXTURE_DIR / "go-rewards-distribution-skew-blocked-module-recipient_negative.go"
...
class GoRewardsSinkPolicyMissingTest(unittest.TestCase):
    def _hits(self, fixture: Path) -> tuple[int, str]:
        ...
        with tempfile.NamedTemporaryFile(prefix=".go_rewards_sink_policy_", suffix=".log") as tmp:
            proc = subprocess.run(
                [
                    python_ast,
                    str(TOOL),
    ...
    def test_positive_fixture_fires_and_clean_fixture_is_silent(self) -> None:
        ...
        self.assertEqual(positive_hits, 2, positive_stdout)
        self.assertEqual(clean_hits, 0, clean_stdout)

    def test_fire14_rewards_miss_is_covered(self) -> None:
        positive_hits, positive_stdout = self._hits(FIRE14_POSITIVE)
        clean_hits, clean_stdout = self._hits(FIRE14_CLEAN)
