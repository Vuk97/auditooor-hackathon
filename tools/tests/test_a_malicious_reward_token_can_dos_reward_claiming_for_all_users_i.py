"""Smoke regression for the malicious reward-claiming DoS detector."""
...
DETECTOR = "a-malicious-reward-token-can-dos-reward-claiming-for-all-users-i"
FIXTURE_DIR = (
    REPO
    / "detectors"
    / "fixtures"
    / "a_malicious_reward_token_can_dos_reward_claiming_for_all_users_i"
)
FIXTURE_VULN = FIXTURE_DIR / "ssi-fix-030_positive.sol"
FIXTURE_CLEAN = FIXTURE_DIR / "ssi-fix-030_clean.sol"
SMOKE = FIXTURE_DIR / "ssi-fix-030_smoke.json"
...
class MaliciousRewardTokenDosSmokeTest(unittest.TestCase):
    def test_vuln_fixture_hits_and_clean_fixture_does_not(self) -> None:
        ...
        with tempfile.TemporaryDirectory(prefix="malicious_reward_token_dos_") as tmp:
            scratch = Path(tmp)
