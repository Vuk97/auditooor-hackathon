...
PATTERN = "prediction-market-oracle-void-outcome-minus-one-payouts-never-set"
DETECTOR = ROOT / "detectors" / "wave17" / "prediction_market_oracle_void_outcome_minus_one_payouts_never_set.py"
REFERENCE = ROOT / "reference" / "patterns.dsl" / f"{PATTERN}.yaml"
FIXTURE_DIR = ROOT / "detectors" / "fixtures" / "prediction_market_oracle_void_outcome_minus_one_payouts_never_set"
ALT_FIXTURE_DIR = ROOT / "detectors" / "fixtures" / "prediction-market-oracle-void-outcome-minus-one-payouts-never-set"
...
class PredictionMarketOracleVoidOutcomeMinusOnePayoutsNeverSetTest(unittest.TestCase):
    ...
    def test_detector_reference_and_fixture_metadata_stay_scoped(self) -> None:
        ...
        self.assertIn("outcome\\\\s*==\\\\s*-1", detector_text)
        self.assertIn("voidedPayouts", detector_text)
        ...
        self.assertIn("contract PredictionMarketOracleManagerPositive", positive_text)
        self.assertIn("if (outcome == -1)", positive_text)
        self.assertIn("markets[marketId].state = MarketState.Resolved;", positive_text)
        self.assertNotIn("adminVoidMarket", positive_text)

        self.assertIn("contract PredictionMarketOracleManagerClean", clean_text)
        self.assertIn("require(outcome == 0 || outcome == 1", clean_text)
        self.assertNotIn("if (outcome == -1)", clean_text)
        ...
        self.assertIn("source-shape proof", smoke["limitation_note"])
