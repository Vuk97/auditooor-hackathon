...
DETECTOR = "matching_engine_misprice_orderbook_fire23"
...
def _run_fixture(fixture: Path) -> tuple[int, str]:
    with tempfile.NamedTemporaryFile(prefix=".rust_matching_fire23_", suffix=".log") as tmp:
        proc = subprocess.run(
            [
                sys.executable,
                str(RUST_DETECT),
...
class RustMatchingEngineMispriceOrderbookFire23Tests(unittest.TestCase):
    def test_detector_and_test_compile(self) -> None:
        py_compile.compile(str(DETECTOR_PATH), doraise=True)
        py_compile.compile(str(Path(__file__)), doraise=True)

    def test_positive_fixture_fires_on_orderbook_replay_shapes(self) -> None:
        fixture_text = POSITIVE.read_text(encoding="utf-8")
        self.assertIn("self.orderbook.last_px", fixture_text)
        self.assertIn("Side::Buy => self.orderbook.best_bid()", fixture_text)
        self.assertIn("maintenance_margin_from_raw_book_ask", fixture_text)

        hits, log_text = _run_fixture(POSITIVE)
        self.assertEqual(hits, 3, log_text)
        self.assertIn("stale orderbook last price", log_text)
        self.assertIn("wrong-side bid/ask selection", log_text)
        self.assertIn("orderbook price without fresh snapshot guard", log_text)
        self.assertIn("matching-engine-misprice", log_text)

    def test_negative_fixture_uses_snapshot_or_oracle_and_is_silent(self) -> None:
        fixture_text = NEGATIVE.read_text(encoding="utf-8")
        self.assertIn("checked_mark_price", fixture_text)
        self.assertIn("snapshot.assert_fresh", fixture_text)
