...
PATTERN = "vote-power-source-switch-without-prior-receipt-debit"
REFERENCE = ROOT / "reference" / "patterns.dsl" / f"{PATTERN}.yaml"
DETECTOR = ROOT / "detectors" / "wave17" / "vote_power_source_switch_without_prior_receipt_debit.py"
FIXTURE_DIR = ROOT / "detectors" / "fixtures" / "vote_power_source_switch_without_prior_receipt_debit"
POSITIVE = FIXTURE_DIR / "positive.sol"
CLEAN = FIXTURE_DIR / "clean.sol"
SMOKE = FIXTURE_DIR / "smoke.json"

SELF_DELEGATION_POSITIVE = (
    ROOT / "detectors" / "fixtures" / "voting_power_self_delegation_double_count" / "positive.sol"
)
SELF_DELEGATION_CLEAN = (
    ROOT / "detectors" / "fixtures" / "voting_power_self_delegation_double_count" / "clean.sol"
)
W68_DIRECT_POSITIVE = (
    ROOT / "detectors" / "fixtures" / "w68_zero_coverage" / "vote_double_count_positive.sol"
)
W68_DIRECT_CLEAN = (
    ROOT / "detectors" / "fixtures" / "w68_zero_coverage" / "vote_double_count_clean.sol"
)
OLD_SOURCE_POSITIVE = (
...
class VotePowerSourceSwitchWithoutPriorReceiptDebitTest(unittest.TestCase):
    ...
    def test_pattern_compile_round_trip_matches_generated_detector(self) -> None:
        ...
        with tempfile.TemporaryDirectory(
            prefix=".pattern_compile_vote_power_source_switch_",
            dir=ROOT,
        ) as tmp:
    ...
    def test_reference_yaml_and_smoke_metadata_stay_aligned(self) -> None:
        ...
        self.assertIn("vuln: detectors/fixtures/vote_power_source_switch_without_prior_receipt_debit/positive.sol", yaml_text)
        self.assertIn("clean: detectors/fixtures/vote_power_source_switch_without_prior_receipt_debit/clean.sol", yaml_text)
        self.assertIn("_balances[voter] + voteCheckpoints[delegates[voter]][snapshot]", positive_text)
        self.assertNotIn("hasVoted[proposalId][voter] = true;", positive_text)
        self.assertIn("require(delegatee != msg.sender", clean_text)
        self.assertIn("hasVoted[proposalId][voter] = true;", clean_text)
    ...
    def test_named_vote_double_count_misses_and_controls(self) -> None:
        ...
        self.assertEqual(hits, 0, log_text)
        hits, log_text = self._hits(OLD_SOURCE_POSITIVE)
        self.assertEqual(hits, 0, log_text)
        hits, log_text = self._hits(OLD_SOURCE_CLEAN)
        self.assertEqual(hits, 0, log_text)
