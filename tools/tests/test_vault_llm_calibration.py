"""Tests for VaultQuery.vault_llm_calibration callable (W3.8 coverage push).
...
Notes: always emits schema; error when log missing
"""
...
class TestVaultLlmCalibration(unittest.TestCase):
    ...
    def setUp(self):
        self.tmp = tempfile.TemporaryDirectory(prefix="w38-llm_calibration-")
    ...
    def test_happy_path_returns_dict(self):
        # synthetic_fixture: true
        result = self.query.vault_llm_calibration()
    ...
    def test_cli_dispatch_exits_zero(self):
        ...
        proc = subprocess.run(
            [sys.executable, str(MODULE_PATH), "--repo-root", str(self.root),
             "--vault-dir", str(self.vault), "--call", "vault_llm_calibration",
             "--args", args_json],
    ...
    def test_callable_in_tool_schemas(self):
        ...
        self.assertIn("vault_llm_calibration", names)
