...
PATTERN = "restaking-slash-target-mismatch-operator-deploys-unslashable-vault"
DETECTOR = ROOT / "detectors" / "wave17" / "restaking_slash_target_mismatch_operator_deploys_unslashable_vault.py"
REFERENCE = ROOT / "reference" / "patterns.dsl" / f"{PATTERN}.yaml"
FIXTURE_DIR = ROOT / "detectors" / "fixtures" / "restaking_slash_target_mismatch_operator_deploys_unslashable_vault"
...
class RestakingSlashTargetMismatchOperatorDeploysUnslashableVaultTest(unittest.TestCase):
    ...
    def test_detector_reference_fixture_and_smoke_metadata(self) -> None:
        ...
        self.assertIn("slashingHandler", detector_text)
        self.assertIn("slashStore", detector_text)
        self.assertIn("DetectorClassification.LOW", detector_text)
        self.assertIn("manual proof", detector_text)

        self.assertIn(f"pattern: {PATTERN}", reference_text)
        self.assertIn("severity: LOW", reference_text)
        self.assertIn("confidence: LOW", reference_text)
        ...
        self.assertIn("backend_scope: solidity_fixture_port_only", reference_text)
        self.assertIn("review_lead_only: true", reference_text)
        self.assertIn(
            "vuln: detectors/fixtures/restaking_slash_target_mismatch_operator_deploys_unslashable_vault/positive.sol",
            reference_text,
        )
        self.assertIn(
            "clean: detectors/fixtures/restaking_slash_target_mismatch_operator_deploys_unslashable_vault/clean.sol",
            reference_text,
        )

        self.assertIn("function slashAssets(uint256 amount, address slashingHandler) external", positive_text)
        self.assertIn("slashingHandler != self.slashStore", positive_text)
        self.assertIn("revert NotSlashStore()", positive_text)
        self.assertIn("operatorChosenSlashStore", positive_text)
        self.assertIn("assetSlashingHandlers[initialAsset] = canonicalHandler", clean_text)
        self.assertIn("require(slashingHandler == canonicalHandler", clean_text)
        self.assertNotIn("revert NotSlashStore()", clean_text)
        ...
        self.assertEqual(smoke["backend_scope"], "solidity_fixture_port_only")
        self.assertTrue(smoke["review_lead_only"])
        ...
        self.assertTrue(manifest["review_lead_only"])
