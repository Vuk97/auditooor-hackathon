"""
test_rust_wave1_anchor_cpi_reentrancy_invoke_signed_mid_state.py

Tests for anchor_cpi_reentrancy_invoke_signed_mid_state detector.

Run with:
    python3 -m unittest tools.tests.test_rust_wave1_anchor_cpi_reentrancy_invoke_signed_mid_state
"""
...
DETECTOR_NAME = "anchor_cpi_reentrancy_invoke_signed_mid_state"
...
class TestAnchorCpiReentrancyInvokeSignedMidState(unittest.TestCase):
    ...
    def test_fires_on_positive_1(self):
        """Fires when state mutation (balance -=) precedes invoke_signed."""
    ...
    def test_fires_on_positive_2(self):
        """Fires when pending_rewards cleared before token::transfer CPI."""
        hits = self._scan_fixture(f"{DETECTOR_NAME}_positive_2.rs")
        self.assertGreaterEqual(len(hits), 1, f"Expected >=1 hit, got: {hits}")

    def test_silent_on_negative_1(self):
        """Silent when CPI comes before state write (correct CEI order)."""
    ...
    def test_silent_on_negative_2(self):
        """Silent when reentrancy_guard is present before CPI."""
        hits = self._scan_fixture(f"{DETECTOR_NAME}_negative_2.rs")
        self.assertEqual(len(hits), 0, f"Expected 0 hits, got: {hits}")

    def test_scan_file_returns_dict_fields(self):
        """scan_file() returns dicts with required fields."""
    ...
    def test_scan_file_emits_util_fields(self):
        """scan_file() emits module_path or fn_signature (best-effort)."""
        fixture = FIXTURES_DIR / f"{DETECTOR_NAME}_positive_1.rs"
        if not fixture.exists():
            self.skipTest("positive_1 fixture missing")
        hits = self.mod.scan_file(str(fixture))
        self.assertGreaterEqual(len(hits), 1)
        h = hits[0]
        has_util = "module_path" in h or "fn_signature" in h or "crate_name" in h
