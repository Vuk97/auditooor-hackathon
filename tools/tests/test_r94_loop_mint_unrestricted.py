...
PATTERN = "r94-loop-mint-unrestricted"
DETECTOR = ROOT / "detectors" / "wave17" / "r94_loop_mint_unrestricted.py"
REFERENCE = ROOT / "reference" / "patterns.dsl" / f"{PATTERN}.yaml"
FIXTURE_DIR = ROOT / "detectors" / "fixtures" / "r94_loop_mint_unrestricted"
MIRROR_DIR = ROOT / "detectors" / "fixtures" / "r94-loop-mint-unrestricted"
...
class R94LoopMintUnrestrictedTest(unittest.TestCase):
    ...
    def test_detector_reference_and_fixture_smoke_metadata_stay_advisory(self) -> None:
        ...
        self.assertIn("contract MintUnrestrictedPositive", positive_text)
        self.assertIn("function mint(address to, uint256 amount) external", positive_text)
        self.assertIn("_mint(to, amount);", positive_text)
        self.assertNotIn("onlyOwner", positive_text)

        self.assertIn("contract MintUnrestrictedClean", clean_text)
        self.assertIn("modifier onlyOwner()", clean_text)
        self.assertIn("function mint(address to, uint256 amount) external onlyOwner", clean_text)
        self.assertIn("_mint(to, amount);", clean_text)
        ...
        self.assertIn("unrestricted external/public entrypoint", payload["limitation_note"])
