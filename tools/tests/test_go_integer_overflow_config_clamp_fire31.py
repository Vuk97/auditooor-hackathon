...
DETECTOR = ROOT / "detectors" / "go_wave1" / "go-integer-overflow-config-clamp-fire31.py"
FIXTURE_DIR = ROOT / "detectors" / "go_wave1" / "test_fixtures"
PATTERN = "go-integer-overflow-config-clamp-fire31"
...
class GoIntegerOverflowConfigClampFire31Test(unittest.TestCase):
    def _hits(self, fixture: Path) -> tuple[int, str]:
        ...
        with tempfile.NamedTemporaryFile(prefix=".go_integer_overflow_config_clamp_", suffix=".log") as tmp:
            proc = subprocess.run(
                [
                    python_ast,
                    str(TOOL),
