...
PATTERN = "r94-loop-fiat-shamir-missing-observation"
DETECTOR = ROOT / "detectors" / "wave17" / "r94_loop_fiat_shamir_missing_observation.py"
REFERENCE = ROOT / "reference" / "patterns.dsl" / f"{PATTERN}.yaml"
FIXTURE_DIR = ROOT / "detectors" / "fixtures" / "r94_loop_fiat_shamir_missing_observation"
ALT_FIXTURE_DIR = ROOT / "detectors" / "fixtures" / "r94-loop-fiat-shamir-missing-observation"
...
class R94LoopFiatShamirMissingObservationTest(unittest.TestCase):
    ...
    def test_detector_reference_and_fixture_scope_stay_aligned(self) -> None:
        ...
        self.assertIn("challenge without prior observation", detector_text)
        ...
        self.assertIn("function verifyTranscript(bytes32 publicInput) external returns (bytes32)", positive_text)
        self.assertIn("return transcript.challenge();", positive_text)
        self.assertNotIn("transcript.observe(publicInput);", positive_text)

        self.assertIn("function verifyTranscript(bytes32 publicInput) external returns (bytes32)", clean_text)
        self.assertIn("transcript.observe(publicInput);", clean_text)
        self.assertIn("return transcript.challenge();", clean_text)
        ...
        self.assertIn("observe/absorb/append", payload["notes"])
