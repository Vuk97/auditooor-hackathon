...
DETECTOR = ROOT / "detectors" / "go_wave1" / "go-bridge-transferout-recipient-binding-missing.py"
FIXTURE_DIR = ROOT / "detectors" / "go_wave1" / "test_fixtures"
PATTERN = "go-bridge-transferout-recipient-binding-missing"
...
class GoBridgeTransferoutRecipientBindingMissingTest(unittest.TestCase):
    def test_detector_ids_route_to_missing_recipient_validation(self) -> None:
        ...
        self.assertEqual(detector_map[PATTERN][0], "missing-recipient-validation")
    ...
    def _hits(self, fixture: Path) -> tuple[int, str]:
        ...
        with tempfile.NamedTemporaryFile(prefix=".go_bridge_binding_", suffix=".log") as tmp:
            proc = subprocess.run(
                [
                    python_ast,
                    str(TOOL),
