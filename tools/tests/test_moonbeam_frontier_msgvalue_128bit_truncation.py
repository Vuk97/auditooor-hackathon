...
PATTERN = "moonbeam-frontier-msgvalue-128bit-truncation"
DETECTOR = ROOT / "detectors" / "wave17" / "moonbeam_frontier_msgvalue_128bit_truncation.py"
REFERENCE = ROOT / "reference" / "patterns.dsl" / f"{PATTERN}.yaml"
FIXTURE_DIR = ROOT / "detectors" / "fixtures" / "moonbeam_frontier_msgvalue_128bit_truncation"
ALT_FIXTURE_DIR = ROOT / "detectors" / "fixtures" / "moonbeam-frontier-msgvalue-128bit-truncation"
...
class MoonbeamFrontierMsgvalue128bitTruncationTest(unittest.TestCase):
    ...
    def test_detector_reference_and_fixture_scope_stay_aligned(self) -> None:
        ...
        self.assertIn("_MSG_VALUE_U128_TRUNCATION_RE", detector_text)
        self.assertIn("_FULL_MSG_VALUE_CREDIT_RE", detector_text)
        self.assertIn("_VISIBLE_U128_GUARD_RE", detector_text)
        self.assertNotIn("rust_fn_runtime", detector_text)
        ...
        self.assertIn("function depositToFrontierRuntime() external payable", positive_text)
        self.assertIn("uint128 runtimeValue = uint128(msg.value);", positive_text)
        self.assertIn("wrappedBalance[msg.sender] += msg.value;", positive_text)
        self.assertNotIn("type(uint128).max", positive_text)

        self.assertIn("function depositToFrontierRuntime() external payable", clean_text)
        self.assertIn('require(msg.value <= type(uint128).max, "frontier value overflow");', clean_text)
        self.assertIn("uint128 runtimeValue = uint128(msg.value);", clean_text)
