...
PATTERN = "optimism-withdrawal-finalize-race-window"
DETECTOR = ROOT / "detectors" / "wave17" / "optimism_withdrawal_finalize_race_window.py"
REFERENCE = ROOT / "reference" / "patterns.dsl" / f"{PATTERN}.yaml"
FIXTURE_DIR = ROOT / "detectors" / "fixtures" / "optimism_withdrawal_finalize_race_window"
ALT_FIXTURE_DIR = ROOT / "detectors" / "fixtures" / "optimism-withdrawal-finalize-race-window"
...
class OptimismWithdrawalFinalizeRaceWindowTest(unittest.TestCase):
    ...
    def test_detector_reference_and_fixture_scope_stay_aligned(self) -> None:
        ...
        self.assertIn("_LEGACY_TIMESTAMP_GATE_RE", detector_text)
        self.assertIn("_FAULT_PROOF_FINALITY_GUARD_RE", detector_text)
        ...
        self.assertIn("function finalizeWithdrawal(", positive_text)
        self.assertIn("block.timestamp >= provenAt + finalizationPeriodSeconds", positive_text)
        self.assertIn("optimismPortal.finalizeWithdrawalTransaction(withdrawalTx);", positive_text)
        self.assertNotIn("faultDisputeGame.status()", positive_text)

        self.assertIn("function finalizeWithdrawal(", clean_text)
        self.assertIn("proofMaturityDelaySeconds", clean_text)
        self.assertIn("disputeGameFinalityDelaySeconds", clean_text)
        self.assertIn("faultDisputeGame.status() == DEFENDER_WINS", clean_text)
