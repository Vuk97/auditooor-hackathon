"""
test_rust_wave1_anchor_account_aliasing_writable_vs_readonly.py

Tests for anchor_account_aliasing_writable_vs_readonly detector.

Run with:
    python3 -m unittest tools.tests.test_rust_wave1_anchor_account_aliasing_writable_vs_readonly
"""
...
DETECTOR_NAME = "anchor_account_aliasing_writable_vs_readonly"
...
class TestAnchorAccountAliasingWritableVsReadonly(unittest.TestCase):
    ...
    def test_fires_on_positive_1(self):
        """Fires on #[account(mut)] AccountInfo without constraint."""
    ...
    def test_fires_on_positive_2(self):
        """Fires on alias constraint with mut field."""
        hits = self._scan_fixture(f"{DETECTOR_NAME}_positive_2.rs")
        self.assertGreaterEqual(len(hits), 1, f"Expected >=1 hit, got: {hits}")

    def test_silent_on_negative_1(self):
        """Silent when /// CHECK: marker is present (acknowledged)."""
    ...
    def test_scan_file_emits_util_fields(self):
        """scan_file() emits at least one _util field."""
        fixture = FIXTURES_DIR / f"{DETECTOR_NAME}_positive_1.rs"
        if not fixture.exists():
            self.skipTest("positive_1 missing")
        hits = self.mod.scan_file(str(fixture))
        self.assertGreaterEqual(len(hits), 1)
        h = hits[0]
        has_util = "module_path" in h or "crate_name" in h or "fn_name" in h
