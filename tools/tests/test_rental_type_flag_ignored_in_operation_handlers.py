...
PATTERN = "rental-type-flag-ignored-in-operation-handlers"
DETECTOR = ROOT / "detectors" / "wave17" / "rental_type_flag_ignored_in_operation_handlers.py"
REFERENCE = ROOT / "reference" / "patterns.dsl" / f"{PATTERN}.yaml"
FIXTURE_DIR = ROOT / "detectors" / "fixtures" / "rental_type_flag_ignored_in_operation_handlers"
...
class RentalTypeFlagIgnoredInOperationHandlersTest(unittest.TestCase):
    ...
    def test_detector_reference_fixture_and_smoke_metadata(self) -> None:
        ...
        self.assertIn("(finalize|cancel)\\\\w*Rental", detector_text)
        self.assertIn("\\\\.rentals\\\\[", detector_text)
        self.assertIn("landlord|renter|owner", detector_text)
        self.assertIn("rentalType|\\\\.kind|is_short", detector_text)
        ...
        self.assertNotIn("preconditions: []", reference_text)
        self.assertIn("contract.source_matches_regex", reference_text)
        self.assertIn("vuln: detectors/fixtures/rental_type_flag_ignored_in_operation_handlers/positive.sol", reference_text)
        self.assertIn("clean: detectors/fixtures/rental_type_flag_ignored_in_operation_handlers/clean.sol", reference_text)

        self.assertIn("function finalizeLongTermRental(uint256 tokenId, uint256 period) external", positive_text)
        self.assertIn("asset.rentals[i]", positive_text)
        self.assertIn("item.tenant == msg.sender", positive_text)
        self.assertIn("item.period == period", positive_text)
        self.assertIn("paidOut[msg.sender] += item.amount;", positive_text)
        self.assertNotIn("rentalType == false", positive_text)

        self.assertIn("function finalizeLongTermRental(uint256 tokenId, uint256 period) external", clean_text)
        self.assertIn("if (item.rentalType == false)", clean_text)
        ...
        self.assertEqual(manifest["backend_scope"], "solidity_fixture_port_only")
