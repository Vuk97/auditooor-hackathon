...
PATTERN = "lack-of-user-access-control-in-stakev2"
DETECTOR = (
    ROOT
    / "detectors"
    / "wave17"
    / "lack_of_user_access_control_in_stakev2.py"
)
REFERENCE = ROOT / "reference" / "patterns.dsl" / f"{PATTERN}.yaml"
FIXTURE_DIR = ROOT / "detectors" / "fixtures" / "lack_of_user_access_control_in_stakev2"
...
class LackOfUserAccessControlInStakev2Test(unittest.TestCase):
    def _hits(self, fixture: Path) -> int:
        ...
        script = textwrap.dedent(
            ...
            spec = importlib.util.spec_from_file_location("stakev2_access_row_detector", detector_path)
            ...
            for compilation_unit in slither.compilation_units:
                detector = module.LackOfUserAccessControlInStakev2(
                    compilation_unit,
                    slither,
                    logger,
                )
    ...
    def test_smoke_record_captures_fixture_smoke_posture(self) -> None:
        ...
        self.assertEqual(payload["positive_hits"], 4)
        self.assertEqual(payload["clean_hits"], 0)
        self.assertFalse(payload["promotion_allowed"])
        self.assertEqual(payload["submission_posture"], "NOT_SUBMIT_READY")

    def test_positive_fixture_fires_and_clean_fixture_stays_quiet(self) -> None:
        self.assertEqual(self._hits(POSITIVE), 4)
        self.assertEqual(self._hits(CLEAN), 0)

    def test_manager_exists_target_state_modifier_does_not_count_as_auth(self) -> None:
        text = POSITIVE.read_text(encoding="utf-8")
        self.assertIn("modifier managerExists", text)
        self.assertEqual(self._hits(POSITIVE), 4)
