...
DETECTOR = ROOT / "detectors" / "go_wave1" / "go-bridge-domain-or-recipient-binding-fire19.py"
FIXTURE_DIR = ROOT / "detectors" / "go_wave1" / "test_fixtures"
PATTERN = "go-bridge-domain-or-recipient-binding-fire19"
...
class GoBridgeDomainOrRecipientBindingFire19Test(unittest.TestCase):
    def _hits(self, fixture: Path) -> tuple[int, str]:
        ...
        with tempfile.NamedTemporaryFile(prefix=".go_bridge_binding_fire19_", suffix=".log") as tmp:
            proc = subprocess.run(
                [
                    python_ast,
                    str(TOOL),
    ...
    def test_fixtures_lock_confirmed_source_shape(self) -> None:
        positive = POSITIVE.read_text(encoding="utf-8")
        negative = CLEAN.read_text(encoding="utf-8")
        detector = DETECTOR.read_text(encoding="utf-8")

        self.assertIn("RunBridgeDaemonTaskLoop", positive)
        self.assertIn("sha256.Sum256(message.MessageHash[:])", positive)
        self.assertIn("VerifyBridgeProof(proof, proofLeaf)", positive)
        self.assertIn("Credit(ctx, message.Recipient, message.Amount)", positive)
        self.assertIn("ValidateBridgeMessageBinding", negative)
        self.assertIn("BuildBridgeMessageDomainKey", negative)
        self.assertIn("legacy:dydx-hunt-iter-1_dydx-hunt-c3", detector)
