...
PATTERN = "pdt-can-be-set-as-a-reward-token-and-withdrawn-by-admin"
DETECTOR = (
    ROOT
    / "detectors"
    / "wave_graveyard"
    / "syntax_broken"
    / "pdt_can_be_set_as_a_reward_token_and_withdrawn_by_admin.py"
)
...
FIXTURE_DIR = (
    ...
    / "detectors"
    / "fixtures"
    / "pdt_can_be_set_as_a_reward_token_and_withdrawn_by_admin"
)
ALT_FIXTURE_DIR = (
    ROOT
    / "detectors"
...
class PdtCanBeSetAsARewardTokenAndWithdrawnByAdminTest(unittest.TestCase):
    ...
    def test_detector_reference_and_fixtures_stay_row_local(self) -> None:
        ...
        self.assertIn("_find_risky_admin_withdraw", detector_text)
        self.assertIn("_WITHDRAW_FN_RE", detector_text)
        ...
        self.assertIn("function registerNewRewardToken(address newRewardToken)", positive_text)
        self.assertIn("function withdrawRewardToken(", positive_text)
        self.assertIn("IERC20Like(token).transfer(to, amount);", positive_text)
        self.assertNotIn("newRewardToken != pdtToken", positive_text)

        self.assertIn("function registerNewRewardToken(address newRewardToken)", clean_text)
        self.assertIn("require(newRewardToken != pdtToken", clean_text)
        self.assertIn("require(token != pdtToken", clean_text)
        ...
        self.assertIn("reward-token registration and admin token withdrawal", payload["limitation_note"])
