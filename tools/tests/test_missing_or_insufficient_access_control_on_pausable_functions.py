...
PATTERN = "missing-or-insufficient-access-control-on-pausable-functions"
DETECTOR = (
    ROOT
    / "detectors"
    / "wave_graveyard"
    / "wave13_broken"
    / "missing_or_insufficient_access_control_on_pausable_functions.py"
)
REFERENCE = ROOT / "reference" / "patterns.dsl" / f"{PATTERN}.yaml"
FIXTURE_DIR = (
    ROOT / "detectors" / "fixtures" / "missing_or_insufficient_access_control_on_pausable_functions"
)
ALT_FIXTURE_DIR = (
    ROOT / "detectors" / "fixtures" / "missing-or-insufficient-access-control-on-pausable-functions"
)
...
class MissingOrInsufficientAccessControlOnPausableFunctionsTest(unittest.TestCase):
    ...
    def test_detector_reference_and_smoke_posture_stay_aligned(self) -> None:
        ...
        self.assertIn("pause-state wrapper", detector_text)
        ...
        self.assertIn("function pause() external", positive_text)
        self.assertIn("function unpause() external", positive_text)
        ...
        self.assertIn("public pause/unpause wrappers", payload["limitation_note"])
